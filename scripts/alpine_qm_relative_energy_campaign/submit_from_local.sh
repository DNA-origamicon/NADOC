...
archive="$archive_root/alpine-relative-energy-campaign-v1.tar.gz"; checksum="$archive.sha256"
remote_root=/scratch/alpine/jojo6687/nadoc_qm_campaigns/tt-cpd-relative-energy-v1
[[ -f "$archive" && -f "$checksum" ]] || exit 2
ssh -S "$control_socket" -O check "$host" >/dev/null 2>&1 || exit 3
ssh -S "$control_socket" "$host" "set -e; if [ -e '$remote_root/bundle' ] || [ -e '$remote_root/submission.json' ]; then exit 4; fi; mkdir -p '$remote_root/logs'"
...
root=Path(sys.argv[1]); record={'schema':'nadoc.photoproduct-alpine-relative-energy-submission.v1','status':'submitted','gate_effect':'none','simulation_ready':False,'submitted_at':datetime.now(timezone.utc).isoformat(),'slurm_job_id':sys.argv[2],'slurm_array':'0-7','remote_root':sys.argv[3]}
