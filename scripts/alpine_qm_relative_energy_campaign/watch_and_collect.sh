...
submission="$campaign_root/submission.json"; remote_results="$campaign_root/remote-results"; log="$campaign_root/watch-and-collect.log"
...
while true; do
  ...
  if [[ "$active" == "0" ]]; then
    ...
    uv run python scripts/alpine_qm_relative_energy_campaign/collect_campaign.py --campaign-root "$campaign_root" --remote-results "$remote_results" >> "$log" 2>&1
    exit 0
  fi
  sleep 60
done
