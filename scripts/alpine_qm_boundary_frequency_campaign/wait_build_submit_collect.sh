...
output_root=${4:?frequency campaign output root is required}
remote_root=${5:-/scratch/alpine/jojo6687/nadoc_qm_campaigns/tt-cpd-boundary-frequency-v1}
...
printf '%s collections present; building fail-closed frequency campaign\n' "$(date --iso-8601=seconds)" >> "$log"
uv run python scripts/alpine_qm_boundary_frequency_campaign/build_campaign.py \
...
bash scripts/alpine_qm_boundary_frequency_campaign/submit_from_local.sh \
  "$output_root" "$remote_root" >> "$log" 2>&1
printf '%s frequency campaign submitted; collecting\n' "$(date --iso-8601=seconds)" >> "$log"
exec bash scripts/alpine_qm_boundary_frequency_campaign/watch_and_collect.sh "$output_root"
