#!/usr/bin/env python3
"""Import and audit Alpine full d(TpT) harmonic-frequency results."""
...
def collect(*, campaign_root: Path, remote_results: Path) -> dict[str, object]:
    ...
    if (
        campaign.get("schema")
        != "nadoc.photoproduct-alpine-boundary-frequency-campaign.v1"
        or campaign.get("product_count") != len(campaign.get("products") or [])
    ):
        raise ValueError("local boundary-frequency campaign manifest is invalid")
    products = []
    ...
    for record in campaign["products"]:
        ...
        valid_receipt = (
            ...
            == "nadoc.photoproduct-alpine-boundary-frequency-case-completion.v1"
        ...
        products.append(
            {
                ...
                "frequency_audit": {
                    "path": str((local_job / "frequency_audit.json").resolve()),
                    "sha256": _sha256(local_job / "frequency_audit.json"),
                },
            }
        )
    passed_statuses = {"passed_harmonic_minimum", "passed_candidate_harmonic_minimum"}
    passed = sum(item["status"] in passed_statuses for item in products)
    report = {
        "schema": "nadoc.photoproduct-alpine-boundary-frequency-collection.v1",
        "status": "passed_harmonic_minimum_audits" if passed == len(products) else "incomplete_or_failed",
        "gate_effect": "none",
        ...
        "interpretation": (
            "Full boundary harmonic evidence only; parameter fitting and NAMD validation remain."
        ),
    }
