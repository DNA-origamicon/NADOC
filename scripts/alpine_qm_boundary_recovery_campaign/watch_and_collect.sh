...
while true; do
  ...
  rsync -a --partial -e "ssh -S $control_socket" "$host:$remote_root/results/" "$remote_results/" 2>/dev/null || true
  if [[ "$active" == "0" ]]; then
    uv run python scripts/alpine_qm_boundary_recovery_campaign/collect_campaign.py \
      --campaign-root "$campaign_root" --remote-results "$remote_results" >> "$log" 2>&1
    exit 0
  fi
  sleep 60
done
