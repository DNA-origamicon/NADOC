...
output_root=${4:?water campaign output root is required}
remote_root=${5:-/scratch/alpine/jojo6687/nadoc_qm_campaigns/tt-cpd-boundary-water-v1}
...
printf '%s collections present; building fail-closed boundary-water campaign\n' "$(date --iso-8601=seconds)" >> "$log"
uv run python scripts/alpine_qm_boundary_water_campaign/build_campaign.py \
  --repository . \
...
printf '%s boundary-water campaign submitted; collecting\n' "$(date --iso-8601=seconds)" >> "$log"
exec bash scripts/alpine_qm_water_campaign/watch_and_collect.sh "$output_root"
