...
remote_root=${NADOC_CAMPAIGN_REMOTE_ROOT:-/scratch/alpine/jojo6687/nadoc_qm_campaigns/tt-cpd-boundary-opt-v1}
...
work_root="$SLURM_SCRATCH/nadoc-cpd-boundary-${SLURM_JOB_ID}-${SLURM_ARRAY_TASK_ID}"
...
export PSI_SCRATCH="$SLURM_SCRATCH/psi4-boundary-${SLURM_JOB_ID}-${SLURM_ARRAY_TASK_ID}"
mkdir -p "$PSI_SCRATCH"
...
grep -Eq 'Optimization is complete|Optimizer: Optimization complete' "$job_dir/output.dat"
