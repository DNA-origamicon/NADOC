#!/usr/bin/env python3
"""Import and audit Alpine d(TpT) optimization results without hiding failures."""
...
def collect(*, campaign_root: Path, remote_results: Path) -> dict[str, object]:
    ...
    if (
        campaign.get("schema")
        != "nadoc.photoproduct-alpine-boundary-optimization-campaign.v1"
        or campaign.get("product_count") != len(campaign.get("products") or [])
    ):
        raise ValueError("local boundary campaign manifest is invalid")
    ...
    for record in campaign["products"]:
        ...
        candidates = sorted(remote_results.glob(f"{product_id}/*/case_completion.json"))
        if len(candidates) != 1:
            products.append(
                {"product_id": product_id, "status": "missing_or_ambiguous_completion", "completion_count": len(candidates)}
            )
            continue
        completion_path = candidates[0]
        completion = json.loads(completion_path.read_text())
        remote_case = completion_path.parent / "case"
        remote_job = remote_case / "job"
        valid_receipt = (
            completion.get("schema")
            == "nadoc.photoproduct-alpine-boundary-optimization-case-completion.v1"
        ...
        if not valid_receipt:
            products.append({"product_id": product_id, "status": "invalid_completion_receipt"})
            continue
        ...
        if any(
            ...
            or _sha256(remote_job / name) != (output_records.get(name) or {}).get("sha256")
            for name in required
        ):
            products.append({"product_id": product_id, "status": "output_hash_mismatch"})
            continue
        local_job = local_case / "job"
        if any((local_job / name).exists() for name in (*required, "run_manifest.json", "optimized_model_audit.json")):
        ...
        run = {
            ...
            "status": "completed_unreviewed" if parsed["passed_execution_checks"] else "failed",
            "gate_effect": "none",
    ...
    report = {
        ...
        "status": "passed_import_and_identity_audit" if passed == len(products) else "incomplete_or_failed",
        "gate_effect": "none",
        "simulation_ready": False,
        "product_count": len(products),
        "passed_product_count": passed,
        "products": products,
        "source_campaign": {"path": str(campaign_path.resolve()), "sha256": _sha256(campaign_path)},
        "next_required_step": "Run matching frequency calculations and require no material imaginary modes before fitting.",
    }
