...
let app, tunnel, expiryTimer, closing = false, output = ''
...
async function stop(error) {
  if (closing) return
  closing = true; clearTimeout(expiryTimer); app?.stop()
...
async function main() {
  ...
  app = await createPreparedHost({ dist: resolve(flags['--dist']), publicOrigin: origin, lifetimeMs: Number(flags['--minutes'] || 120) * 60000 })
  ...
  tunnel = spawn(tailscale, ['funnel', '--yes', '--https=443', '--bg=false', target], { windowsHide: true, stdio: ['ignore', 'pipe', 'pipe'] })
  ...
  for (let i = 0; i < 80 && !closing; i++) {
    if (hasInternetRoute(JSON.parse(await ts(['serve', 'status', '--json'])), origin, target)) {
      ...
      await report({ state: 'ready', origin }); return
    }
