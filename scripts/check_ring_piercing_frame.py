...
This screens what a job actually ran: the packaged seed, or any frame of its trajectory. Use it to decide
...
def _rings_and_bonds(universe, selection: str):
    ...
    for res in dna.residues:
        ...
        for kind, ring in ring_names_for(names):
            rings.append(
                (
                    f"{res.segid}:{res.resname}{res.resid}",
                    kind,
    ...
    bonds = [
        ...
        for a, b in ((bd.atoms[0], bd.atoms[1]) for bd in universe.bonds)
        if a.index in idx
        and b.index in idx
        and not a.name.startswith("H")
        and not b.name.startswith("H")
    ]
...
def _scan_frame(u, rings, bonds, label: str, quiet: bool) -> list[str]:
    ...
    for bi, cands in enumerate(near):
        ...
        for ri in cands:
            ...
            hits.append(
                f"{a.segid}:{a.resname}{a.resid}:{a.name}"
...
def main(argv=None) -> int:
    ...
    ap.add_argument(
        "--select",
        default="nucleic or (not resname TIP3 HOH SOD CLA MG MGH)",
        help="atom selection to treat as DNA",
    )
    ap.add_argument(
        "--stride", type=int, default=1, help="frame stride for trajectories"
    )
