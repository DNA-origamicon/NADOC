...
remote_root=${NADOC_CAMPAIGN_REMOTE_ROOT:-/scratch/alpine/jojo6687/nadoc_qm_campaigns/tt-cpd-boundary-esp-v1}
...
work_root="$SLURM_SCRATCH/nadoc-cpd-boundary-esp-${SLURM_JOB_ID}-${SLURM_ARRAY_TASK_ID}"
...
import json,sys
p=json.load(open(sys.argv[1])); print(p['job_manifest_sha256']); print(p['input_sha256']); print(p['grid_sha256'])
PY
)
...
[[ $(sha256sum "$job_dir/grid.dat" | awk '{print $1}') == "${expected[2]}" ]] || exit 8
sync_results() { rsync -a "$case_root/" "$result_root/case/" 2>/dev/null || true; }
trap sync_results EXIT
export PSI_SCRATCH="$SLURM_SCRATCH/psi4-boundary-esp-${SLURM_JOB_ID}-${SLURM_ARRAY_TASK_ID}"
mkdir -p "$PSI_SCRATCH"
