...
try {
  if (!/^\/host\/(shares(?:\/[a-f0-9]{32}(?:\/broadcast\/(?:start|camera|scene|pause|heartbeat)|\/(?:trajectory|content))?)?|stop)$/.test(path) || !['GET', 'POST', 'DELETE'].includes(method)) throw new Error('Invalid host action')
