...
| `M-JOB-PLANNER` | chain jobs unattended (stage-spec + executor + planner UI) | pending — **P1+P2+P3 shipped 2026-07-08**. P1 = `MdPipeline` stage-spec + pure builder. **P2 = chain EXECUTOR** (`md_chain_executor.py`): stage N spawns SEEDED FROM N-1's realised child on completion; failure HALTS; `resume_chain` retries-only-failed. **P3 = cross-engine output→input**: a `cross_engine` hop (oxDNA/mrDNA root → NAMD stage) reconstructs the atomistic start structure from the CG relaxed frame via the create-time converter (`build_namd_seed`/`_from_mrdna`, nm↔sim-units↔Å already handled) instead of the same-engine `.coor/.xsc` restart — `cross_engine_seed` resolver + `_chain_spawn` branch + `create_md_chain` accepts a CG root; CHAIN oracle 6 pure + 7 route (incl. E2E oxDNA→NAMD chain + halt/resume). Only **P4** (Plan Run UI) remains |
| `M-DEPOSITION-CHAIN` | E-field→surface→anchors→field-sweep from one Plan Run | pending (P1✓ + **P2✓** + **P3✓** + P4 + U2✓) — the executor now chains deposition→immobilize→sweep stages AND hands one engine's coords to another cross-engine; still needs P4 (planner UI to author the plan) + forces-into-reseed-conf wiring |
