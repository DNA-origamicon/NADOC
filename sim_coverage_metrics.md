...
| `M-MRDNA-FORCES-UI` | mrDNA field/anchors reachable from the unified panel + a new mrDNA hard-surface (M6+M7+M8) | in_progress — **M6 shipped 2026-07-10**: the shared U2 Forces + Anchors cards are mounted in the mrDNA panel (`mrdna-efield-*`/`mrdna-anchors-*` DOM + `forces_card.js` `mrdna` variant + `engine_capabilities` efield/anchors flipped `off→on`), and `_launch` threads `field:{field_pN,dir}` + `anchors:[…]` into `POST /mrdna/jobs` via the pure `buildMrdnaLaunchBody` (byte-identical to CanDo; server-side field-needs-anchor guard mirrored client-side). PARITY oracle `mrdna_jobs_panel.test.js` +7 (card output byte-equals payload; census flip enforced vs live index.html). main.js Δ=0; live gesture owes MV-34. Remaining: **M7** (ARBD repulsion-plane hard-surface backend, headless) → **M8** (surface UI card). |
