...
- **VALIDATED: 11** — **MV-CLUSTYLE (2026-08-01, user-confirmed in app)** — per-cluster colour + opacity
  ...
  the backend ships a per-vertex nucleotide table (2026-08-01), so the surface resolves per
  nucleotide like everything else. Worth one re-look in app: with two clusters coloured, a
  scaffold-spanning SURFACE should show both colours and match the bead view.
  MV-1, MV-2, MV-3, MV-4, MV-6, MV-7 (all 2026-06-07), MV-LNK (2026-06-07),
...
- **MV-SCRYWITNESS** — ScryWrite live VR Witness Mode — run the canonical witness-menu script against `native/vr_viewer/examples/triangle.nadocvr` on the Vive; confirm the physical HMD stays freely observer-controlled, the ghost controllers and corrected 72-degree/16:9 scripted-head frustum face the expected direction, the head-following actor-eye monitor is upright and binocularly comfortable, Options → Tools → Move Rotate is visible and highlighted in sequence, left-menu pause/resume and right-menu single-step work, and a deliberately wrong `expect hover` freezes with a legible red failure marker and no held scripted button. Automated replay/state/hit checks plus the strict `Chiral_test`-derived projection export cover deterministic pose and framing, but cannot establish stereo correctness, physical scale, legibility, or comfort. Physical left-eye mirror motion and through-lens parity are now hand-confirmed for the room grid and framed asymmetric origami; SteamVR idle suppression has a labeled fallback. The live mirror now adds application-side pixel sampling and predicted-time JSONL with `BLACK`, `STABLE`, `FROZEN?`, and `CHANGING` states plus same-pass design/grid/overlay stencil coverage. A live mutation distinguished `GRID ONLY` from the framed `DESIGN+GRID` case. `just scrywrite-frame` now provides reproducible view-relative placement with named orientations and recorded parameters; paired front/top runs are retained. Remaining checks: right-eye behavior, menu/Witness-panel inclusion, aspect-preserving resize, actor monitor comfort, and deliberate moved-head validation of `PX CHANGING`. Per-object IDs/depth, compositor acknowledgement, and side-by-side capture are not implemented.
...
- **MV-ION-VFIELD** — NAMD nanopore ion vector field visual quality on a real trajectory. Select a completed nanopore production job with crossings, choose **View ion vector field**, and compare Na⁺ and Cl⁻ at low/default/high arrow density in both **Arrow size** and **Color coded** magnitude modes. Verify: (a) arrows expose the dominant through-pore route without visually overwhelming the cyan RMSF-average origami or grey graphene/aperture; (b) direction and colour/size gradients are legible from several camera angles; (c) graphene representation/visibility remains identical while toggling paths ↔ vector field; (d) before/after window changes show the full staged progress, while repeated view toggles preserve every setting and do not issue another `/ion-paths` request. Pure binning, scene ownership, WebGL rendering, local controls, and the single-fetch invariant are automated; the scientific visual legibility on a representative real trajectory remains human-eye only.
