...
NAMD_MS_PER_STEP = 7.5      # measured: 17,827-atom solvated duplex, RTX 2080 (0.0075 s/step)
NAMD_REF_N = 17_827
...
class PaiNNLite(nn.Module):
    ...
    def __init__(self, hidden: int = 128, n_layers: int = 3, n_rbf: int = 20,
        ...
        self.vel_gate = nn.Sequential(nn.Linear(hidden, hidden), nn.SiLU(),
    ...
    def _rbf(self, d: torch.Tensor) -> torch.Tensor:
        ...
        return torch.exp(-((d[:, None] - centers[None, :]) ** 2) / (self.cutoff / self.n_rbf) ** 2)

    def forward(self, z, pos, vel, edge_index):
        i, j = edge_index[0], edge_index[1]
        s = self.embed(z)                              # [N,H] scalar
        V = vel[:, :, None] * self.vel_gate(s)[:, None, :]   # [N,3,H] equivariant, seeded by velocity
        r = pos[j] - pos[i]                            # [E,3]
        d = r.norm(dim=-1) + 1e-8
        fc = 0.5 * (torch.cos(np.pi * d.clamp(max=self.cutoff) / self.cutoff) + 1.0)  # cutoff envelope
        unit = r / d[:, None]
...
def speed_benchmark(sizes=(1_000, 5_000, 17_827, 50_000, 100_000, 200_000),
                    cutoff: float = 5.0, hidden: int = 128, n_layers: int = 3,
                    reps: int = 20, device: str = "cuda") -> list[dict]:
    ...
    rows = []
    print(f"=== GNN per-step speed vs NAMD (device={dev}, cutoff={cutoff} Å, "
          f"hidden={hidden}, layers={n_layers}) ===")
    print(f"NAMD baseline: {NAMD_MS_PER_STEP:.2f} ms/step @ {NAMD_REF_N} atoms "
          f"(full force+PME+integrate, RTX 2080)")
    print(f"{'N atoms':>9} {'edges':>11} {'nbr/atom':>8} {'GNN ms/step':>12} "
          f"{'NAMD~ms':>9} {'GNN/NAMD':>9}")
    for n in sizes:
        z, pos, vel, _L = _random_system(n, )
        ...
        namd_scaled = NAMD_MS_PER_STEP * (n * np.log2(max(n, 2))) / (
            NAMD_REF_N * np.log2(NAMD_REF_N))
        row = {"n": n, "edges": int(edges.shape[1]), "nbr_per_atom": edges.shape[1] / n,
               "gnn_ms": ms, "namd_ms_scaled": namd_scaled, "ratio": ms / namd_scaled}
        rows.append(row)
        print(f"{n:>9} {row['edges']:>11} {row['nbr_per_atom']:>8.1f} {ms:>12.3f} "
