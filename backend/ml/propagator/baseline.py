...
def _pair_indices(
    n_frames: int, segment_starts: np.ndarray, stride: int = 1
) -> np.ndarray:
...
def fit_verlet(
    disp: np.ndarray, vel: np.ndarray, acc: np.ndarray
) -> tuple[float, float]:
...
def evaluate(npz_path: str | Path, *, train_frac: float = 0.7, stride: int = 1) -> dict:
    ...
    z = data["z"]
    a, b = fit_verlet(
        dispP[tr].reshape(-1, 3), velP[tr].reshape(-1, 3), accP[tr].reshape(-1, 3)
    )
    a_vonly, _ = fit_verlet(
        dispP[tr].reshape(-1, 3),
        velP[tr].reshape(-1, 3),
    ...
    inertial_a = (
        NAMD_VEL_TO_A_PER_PS / 1000.0
    ) * dt_fs  # v(Å/fs)·dt(fs), correct units
    ...
    out = {
        ...
        "elem_coeffs": {
            str(zz): {"a": ab[0], "b": ab[1]} for zz, ab in elem_coeffs.items()
        },
    ...
    out["skill_vs_zero_motion"] = {
        "global": (1.0 - out["one_step_rmse_A"]["fitted_verlet_global"] / e0)
        if e0
        else None,
        "per_element": (1.0 - out["one_step_rmse_A"]["fitted_verlet_per_element"] / e0)
        if e0
        else None,
    }
...
def _teacher_forced_rollout(
    data: dict, a: float, b: float, *, max_steps: int = 50
) -> dict:
...
def report(npz_path: str | Path) -> dict:
    ...
    print(
        f"atoms={m['n_atoms']}  pairs={m['n_pairs_total']} "
        f"(train {m['n_train_pairs']}/test {m['n_test_pairs']})  dt={m['dt_fs']:.1f} fs"
    )
    print(
        f"mean true one-step displacement: {m['mean_true_step_displacement_A']:.4f} Å"
    )
    vr = m["velocity_regime"]
    print(
        f"velocity regime: fitted a={vr['fitted_a_vel_only']:.4g} vs ballistic "
    ...
    print(
        f"skill vs zero-motion: global {sk['global']:.3f}  per-element {sk['per_element']:.3f}"
    )
    print(
        "per-element vel coeff a (Z→a): "
        + "  ".join(f"{z}:{c['a']:.3g}" for z, c in m["elem_coeffs"].items())
    )
    if rl["horizon"]:
        print(
            f"teacher-forced rollout: {rl['horizon']} steps, "
...
def macrostep_sweep(npz_path: str | Path, strides=(1, 2, 3, 5, 10)) -> list[dict]:
    ...
    print(
        f"{'dt(fs)':>7} {'disp(Å)':>9} {'zero':>8} {'inertial':>9} "
    ...
    for s in strides:
        ...
        row = {
            ...
            "zero": r["zero_motion"],
            "inertial": r["inertial_vdt"],
            "fit_global": r["fitted_verlet_global"],
            "fit_elem": r["fitted_verlet_per_element"],
            "skill_elem": m["skill_vs_zero_motion"]["per_element"],
        }
        rows.append(row)
        print(
            f"{m['dt_fs']:7.0f} {row['mean_disp_A']:9.4f} {row['zero']:8.4f} "
