...
def coating_blocks(design):
    ...
    for p in design.nanoparticles:
        for index, pose in enumerate(p.coating.poses):
            matrix = p.pose.to_array() @ pose.to_array()
            att = ProteinAttachment(
                id=f"{p.id}:strep:{index}",
                asset_id=p.coating.protein.id,
                target={"kind": "free"},
                pose=Mat4x4(values=matrix.ravel().tolist()),
            )
            attachments.append(att)
            blocks.append(protein_beads(p.coating.protein, att))
...
def fixed_core_forces(design, attachments, blocks, geometry):
    ...
    parts = []
    base = 0
    resolved = seed_fixed_dna(
        design, oxdna_native_seed_map(design, resolved_nuc_map(design, geometry))
    )
    coating_map = {
        f"{p.id}:strep:{i}": (p, i)
        for p in design.nanoparticles
        for i in range(len(p.coating.poses))
    }
    for att, beads in zip(attachments, blocks):
        p, tetramer_index = coating_map.get(att.id, (None, None))
        if p is None:
            base += len(beads)
            continue
        ...
        radius = (p.diameter_nm / 2 - 0.8) * NM_TO_OXDNA
        if tetramer_index == 0:
            parts.append(
            ...
            chosen.append(
                int(
                    max(
                        candidates,
                        key=lambda i: min(
                            np.linalg.norm(xyz[i] - xyz[j]) for j in chosen
                        ),
                    )
                )
            )
        for i in chosen:
            parts.append(anchor_trap_block(base + i, xyz[i] * NM_TO_OXDNA, 10.0))
        for record in p.biotin_dna:
            if record.tetramer_index != tetramer_index:
                continue
            anchor, _ = pocket_geometry(p, record.chain, record.tetramer_index)
            local = min(
                (i for i, b in enumerate(beads) if b.chain_id == record.chain),
                key=lambda i: np.linalg.norm(xyz[i] - anchor),
            )
            ...
            key = next(
                (
                    ...
                    if k[0] == record.helix_id and k[1] == 0
                ),
                None,
            )
            particle = (
                dna_particle_index(design, key, offset) if key is not None else None
            )
            if particle is None or key not in resolved:
                raise ValueError("Cannot resolve the biotinylated DNA 5′ terminus")
            ...
            parts.append(
                conjugation_trap_text(base + local, particle, 1.424, float(rest))
            )
        base += len(beads)
    return "".join(parts)
...
def seed_fixed_dna(design, resolved):
    ...
    for p in design.nanoparticles:
        if not p.oxdna_fixed_core:
            continue
        for record in p.biotin_dna:
            h = next(h for h in design.helices if h.id == record.helix_id)
            if record.placement_version == 2:
                from backend.core.gold_strep_dna import extended_dna_seed

                positions, normal, axis = extended_dna_seed(h)
                for key, nuc in resolved.items():
                    if key[0] == record.helix_id:
                        result[key] = {
                            **nuc,
                            "backbone_position": positions[key[1]].tolist(),
                            "base_normal": normal.tolist(),
                            "axis_tangent": axis.tolist(),
                        }
                continue
            axis = h.axis_end.to_array() - h.axis_start.to_array()
            axis /= np.linalg.norm(axis)
            normal = np.cross(
                axis, [1.0, 0.0, 0.0] if abs(axis[0]) < 0.9 else [0.0, 1.0, 0.0]
            )
            ...
            for key, nuc in resolved.items():
                ...
                result[key] = {
                    **nuc,
                    "backbone_position": (
                        h.axis_start.to_array() + axis * key[1] * 0.7564 / NM_TO_OXDNA
                    ).tolist(),
                    "base_normal": normal.tolist(),
                    "axis_tangent": axis.tolist(),
                }
