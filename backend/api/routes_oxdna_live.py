...
class LiveStartRequest(BaseModel):
    ...
    burst_steps:  int = Field(500, ge=1, le=200_000,
...
def _prepare_live_rundir(design, seed_conf, rundir, *, field, wall, anchors,
    ...
    info = write_run_forces(rundir / "field_forces.txt", design, rundir / "conf.dat",
                            field=field, wall=wall, anchors=anchors,
...
async def start_oxdna_live(body: LiveStartRequest) -> dict:
    ...
    if design is None:
        raise HTTPException(500, "design.json snapshot missing; cannot start live session.")
