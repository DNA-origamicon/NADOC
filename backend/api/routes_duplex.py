...
from backend.core.duplex import (
    ...
    classify_duplex_pairing,
    overhang_pairing_map,
    connect_register,
    longest_driver,
...
def _end(body: DuplexEndBody) -> DuplexEnd:
    return DuplexEnd(
        overhang_id=body.overhang_id, start_bp=body.start_bp, end_bp=body.end_bp
    )


def _validate_placement(
    design: Design, dx: Duplex, *, ignore_id: Optional[str] = None
) -> None:
    ...
    for side, end in (("left", dx.left), ("right", dx.right)):
        if end.overhang_id not in overhang_ids:
            raise HTTPException(
                404, detail=f"{side} overhang {end.overhang_id!r} not found."
            )
        _, dom = _overhang_backing_domain(design, end.overhang_id)
        if dom is None:
            raise HTTPException(
                404, detail=f"{side} overhang {end.overhang_id} has no backing domain."
            )
...
def _propagate_driver_to_binding(design: Design, duplex: Duplex) -> Design:
    ...
    driver_oh = (
        duplex.left.overhang_id if duplex.driver == "left" else duplex.right.overhang_id
    )
    driven_oh = (
        duplex.right.overhang_id if duplex.driver == "left" else duplex.left.overhang_id
    )
    pair = {duplex.left.overhang_id, duplex.right.overhang_id}
    changed = False
    new_bindings = []
    for b in design.overhang_bindings:
        if {b.overhang_a_id, b.overhang_b_id} == pair and (
            b.driver_oh_id != driver_oh or b.driven_oh_id != driven_oh
        ):
            new_bindings.append(
                b.model_copy(
                    update={"driver_oh_id": driver_oh, "driven_oh_id": driven_oh}
                )
            )
            changed = True
        else:
    ...
    return (
        design.model_copy(update={"overhang_bindings": new_bindings})
        if changed
        else design
    )
...
def connect_duplex(body: ConnectDuplexBody) -> dict:
    ...
    for dx in design.duplexes:
        if {dx.left.overhang_id, dx.right.overhang_id} == {
            body.overhang_a_id,
            body.overhang_b_id,
        }:
            raise HTTPException(
    ...
    has_binding = any(
        {b.overhang_a_id, b.overhang_b_id} == pair for b in updated.overhang_bindings
    )
    if not has_binding:
...
def patch_duplex(duplex_id: str, body: PatchDuplexBody) -> dict:
    ...
    if "driver" in update:
        ...
        linked = next(
            (
                bb
                for bb in updated.overhang_bindings
                if {bb.overhang_a_id, bb.overhang_b_id} == pair
            ),
            None,
        )
...
def get_overhang_pairing_map(overhang_id: str) -> dict:
    ...
    return {
        ...
        "pairing_map": {
            str(bp): st for bp, st in overhang_pairing_map(design, overhang_id).items()
        },
    }
