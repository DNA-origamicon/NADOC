...
async def create_lammps_job(body: CreateLammpsJobRequest) -> dict:
    ...
    if not avail["available"]:
        raise HTTPException(
            400,
            "LAMMPS not found. Build it via the MD Engines panel or "
            "scripts/lammps_doctor.py --fix (see docs/lammps_setup.md).",
        )
    if not avail["cgdna_capable"]:
        raise HTTPException(
            400,
            "LAMMPS is installed but was built without the CG-DNA package, so it "
    ...
    job = new_lammps_job(
        name,
        steps=body.steps,
        dump_every=body.dump_every,
        temperature=body.temperature,
        salt_molar=body.salt_molar,
        ranks=body.ranks,
...
async def get_lammps_trajectory(job_id: str, align: bool = True) -> dict:
    ...
    result = await run_in_threadpool(
        composite_trajectory, design, stages, ref, align=align
    )
