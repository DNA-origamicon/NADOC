...
class AutorefineStartRequest(BaseModel):
    ...
    initial_period: int | None = None   # None => seed from the design (48 if it has no skips)
    max_iterations: int = 6
    production_steps: int = 8_000_000   # long rounds for decorrelated frames near tolerance
    ...
    finetune: bool = True              # default: fine-tune from the analytical pattern (CanDo-style)
...
def _run(run_id: str, design, workspace, params: AutorefineStartRequest) -> None:
    ...
    try:
        result = autorefine_sq_design(
            design, workspace, on_progress=on_progress,
            should_stop=stop_event.is_set, on_job=on_job,
            tol_nm=params.tol_nm, tol_twist_deg=params.tol_twist_deg,
            min_confidence=params.min_confidence,
            initial_period=params.initial_period, max_iterations=params.max_iterations,
            production_steps=params.production_steps,
            max_production_rounds=params.max_production_rounds,
            regional=params.regional, w_dev=params.w_dev, w_strain=params.w_strain,
            min_spacing=params.min_spacing,
            finetune=params.finetune, finetune_max_edits=params.finetune_max_edits,
...
def start_autorefine(req: AutorefineStartRequest) -> dict:
    ...
    if design.lattice_type != LatticeType.SQUARE:
        raise HTTPException(
            400, detail="Autorefine skips/loops currently supports SQUARE lattice "
    ...
    if not any(s.strand_type.value == "scaffold" for s in design.strands):
        raise HTTPException(
            400, detail="Route a scaffold first — autorefine needs a fully routed, "
    ...
    threading.Thread(target=_run, args=(run_id, snapshot, _workspace(), req),
                     daemon=True).start()
...
def apply_autorefine_skips(run_id: str, period: int | None = None) -> dict:
    ...
    from backend.api.skip_twist_tuning import (
        build_explicit_skip_from_design, build_sq_skip_from_design,
    )
    ...
    apply_explicit = bool(period is None and run.get("state") in ("done", "stopped")
                          and explicit_skips)
    ...
    if apply_explicit:
        ...
        params = {"placement": placement, "skip_period": result.get("converged_period"),
                  "source": "autorefine", "run_id": run_id, "resequenced": True}
    else:
        period = int(period)
        refined = build_sq_skip_from_design(design, period)
        label = f"Autorefine skips (period {period} bp)"
        params = {"skip_period": period, "source": "autorefine", "run_id": run_id,
                  "resequenced": True}
