...
def build_sq_skip_design(cells, length_bp: int, skip_period,
                         *, scaffold: str | None = None) -> Design:
    ...
    with hb.scratch_session(LatticeType.SQUARE):
        ...
        if skip_period is not None:
            mods = sq_lattice_periodic_skips(design_state.get_or_404(), int(skip_period))
...
def core_reference_geometry(design) -> list[dict]:
    ...
    from backend.physics.oxdna_interface import (
        _XB_SENTINEL, _strand_nucleotide_order, resolved_nuc_map,
    )

    resolved = resolved_nuc_map(design, _geometry_for_design(design, compact_skips=True))
    recs: list[tuple] = []
    strands_at: dict = {}
...
class PeriodAdjuster:
    ...
    def __init__(self, *, p_min: int = 8, p_max: int = 400, explore_max: float = 0.5,
                 explore_gain: float = 0.015, explore_min: float = 0.03):
        self.p_min, self.p_max = p_min, p_max
        self.explore_max, self.explore_gain, self.explore_min = (
            explore_max, explore_gain, explore_min)
    ...
    def _explore_step(self, period: int, residual: float) -> int:
        ...
        frac = min(self.explore_max, max(self.explore_min, abs(residual) * self.explore_gain))
...
def build_sq_skip_from_design(base_design, skip_period, *, scaffold: str | None = None):
    ...
    with hb.scratch_session(LatticeType.SQUARE):
        ...
        hb.full_sequence(scaffold_name=_select_scaffold(design_state.get_or_404(), scaffold))
        return design_state.get_or_404().model_copy(deep=True)


def build_regional_skip_design(base_design, skip_period, deviation_by_bp, strain_by_bp,
                              *, w_dev: float = 1.0, w_strain: float = 0.25,
                              min_spacing: int = 4, scaffold: str | None = None):
    ...
    from backend.core.regional_skip_placer import (
        budget_from_uniform_period, place_regional_skips,
    )
    ...
    with hb.scratch_session(LatticeType.SQUARE):
        ...
        if skip_period is not None:
            ...
            mods = place_regional_skips(
                d, budget, deviation_by_bp or {}, strain_by_bp or {},
                w_dev=w_dev, w_strain=w_strain, min_spacing=min_spacing)
...
def build_explicit_skip_from_design(base_design, skips_by_helix, *, scaffold: str | None = None):
    ...
    with hb.scratch_session(LatticeType.SQUARE):
        design_state.set_design(base_design.model_copy(deep=True))
        d = clear_all_loop_skips(design_state.get_or_404())
        mods = {hid: [LoopSkip(bp_index=int(bp), delta=-1) for bp in bps]
                for hid, bps in (skips_by_helix or {}).items() if bps}
        if mods:
            d = apply_loop_skips(d, mods)
        design_state.set_design(d)
        hb.full_sequence(scaffold_name=_select_scaffold(design_state.get_or_404(), scaffold))
...
def measure_skip_fields(job, workspace, design, reference):
    ...
    try:
        ...
        if confs:
            strain = backbone_strain_field(design, read_configuration_full(
                Path(confs[-1]), design))
    except Exception:
        strain = {}
...
def _finetune_measure(design, workspace, *, n_prod, production_steps, timeout, on_job=None,
                      **relax_params):
    ...
    from backend.api.headless_oxdna_build import (
        append_production, read_flexibility_map, run_relaxation, wait_for_terminal,
    )
    from backend.core.oxdna_health import (
        _filter_to_reference_core, geometry_deviation_map, measure_bundle_twist,
        measure_bundle_twist_profile,
    )
    from backend.core.regional_skip_placer import (
        aggregate_deviation_per_bp, detrend_error_profile,
    )
    ...
    shape = detrend_error_profile(list(zip(st.tolist(), (sv - np.interp(st, at, av)).tolist())))
    devs = [p["deviation"] for p in dmap["positions"]]
    return {"twist": measure_bundle_twist(core), "dev_max": max(devs),
            "dev_mean": sum(devs) / len(devs),
            "deviation_by_bp": aggregate_deviation_per_bp(dmap), "shape_profile": shape}


def greedy_finetune_skips(converged_design, workspace, *, max_edits: int = 5,
                          tol_twist_deg: float = 5.0, dev_improve_nm: float = 0.1,
                          sigma: float = 1.0, min_spacing: int = 8,
                          n_prod: int = 3, production_steps: int = 2_000_000,
                          on_progress=None, should_stop=None, on_job=None,
    ...
    kw = dict(n_prod=n_prod, production_steps=production_steps, timeout=timeout,
              on_job=on_job, **relax_params)
    ...
    if before is None:
        return {"status": "error", "error": "baseline simulation failed",
                "edits_kept": [], "n_candidates": 0}
    emit({"phase": "baseline", "twist": before["twist"], "dev_max": before["dev_max"]})

    candidates = identify_finetune_edits(
        base, before["deviation_by_bp"], before["shape_profile"],
        max_edits=max_edits, sigma=sigma, min_spacing=min_spacing)
    ...
    for i, edit in enumerate(candidates):
        ...
        if m is None:
            continue
        accept = (abs(m["twist"]) <= tol_twist_deg
                  and m["dev_max"] <= best["dev_max"] - dev_improve_nm)
        emit({"phase": "edit", "i": i, **edit, "twist": m["twist"], "dev_max": m["dev_max"],
              "accepted": accept})
        if accept:
            cur_skips, best = trial_skips, m
            kept.append({**edit, "twist": m["twist"], "dev_max": m["dev_max"]})

    out = {"status": "done", "n_candidates": len(candidates), "edits_kept": kept,
           "before": {"twist": before["twist"], "dev_max": before["dev_max"],
                      "dev_mean": before["dev_mean"]},
           "after": {"twist": best["twist"], "dev_max": best["dev_max"],
...
def seed_skip_period(design, *, default: int = SQ_SKIP_PERIOD_DEFAULT,
                     p_min: int = 8, p_max: int = 400) -> int:
...
def _metrics_from_verdict(verdict) -> dict:
    ...
    steering = (verdict or {}).get("steering") or {}
    return {"rmsd_nm": steering.get("geometry_rmsd_nm", (verdict or {}).get("measured_nm")),
            "twist_residual_deg": steering.get("bundle_twist_residual_deg")}


def measure_design_self_consistency(design, workspace, *, tol_nm: float,
                                    min_confidence: int, production_steps: int,
                                    max_production_rounds: int, timeout: float,
                                    screen_steps=None, should_stop=None, on_job=None,
                                    **relax_params) -> dict:
    ...
    if on_job:
        on_job(job)
    if job.status.value != "completed":
        return {"error": job.error or "relaxation did not complete", "job_id": job.job_id,
                "stopped": bool(should_stop and should_stop())}
    parsed = parse_constraint_spec({**_GEOMETRY_CONSTRAINT, "tol_nm": tol_nm,
                                    "min_confidence": min_confidence})
    verdict, _rounds = _pool_until_conclusive(
        job, workspace, parsed, production_steps=production_steps,
        max_production_rounds=max_production_rounds, timeout=timeout, reference=reference,
        should_stop=should_stop, screen_steps=screen_steps)
    return {**_metrics_from_verdict(verdict), "n_frames": verdict["n_frames"],
            "status": verdict["status"], "job_id": job.job_id}


def autorefine_sq_design(base_design, workspace, *, on_progress=None, should_stop=None,
                         on_job=None, tol_nm: float = 2.0, tol_twist_deg: float = 5.0,
                         min_confidence: int = 400, baseline_min_confidence: int = 100,
                         initial_period: int | None = None, max_iterations: int = 6,
                         production_steps: int = 8_000_000, screen_steps: int = 2_000_000,
                         max_production_rounds: int = 6, early_reject_factor: float = 3.0,
                         regional: bool = False, w_dev: float = 1.0, w_strain: float = 0.25,
                         min_spacing: int = 4,
                         finetune: bool = False, finetune_max_edits: int = 5,
                         finetune_sigma: float = 1.0,
                         equilibration_steps: int = 10_000_000,
                         timeout: float = 14400.0, **relax_params) -> dict:
    ...
    """
    from backend.api.headless_oxdna_build import (
        STANDARD_RELAX_PARAMS, iterate_to_constraint,
    )
    ...
    relax_params = {**STANDARD_RELAX_PARAMS, "equil_steps": equilibration_steps, **relax_params}
    ...
    seed_period = int(initial_period) if initial_period is not None \
    ...
    constraint = ({"measure": "bundle_twist", "target_nm": 0.0, "tol_nm": tol_twist_deg,
                   "min_confidence": min_confidence} if plain_sq else
                  {"measure": "geometry_match", "target_nm": 0.0, "tol_nm": tol_nm,
                   "min_confidence": min_confidence})
    ...
    before = measure_design_self_consistency(
        base_design, workspace, tol_nm=tol_nm, min_confidence=baseline_min_confidence,
        production_steps=production_steps, max_production_rounds=max_production_rounds,
        timeout=timeout, screen_steps=screen_steps,
        should_stop=should_stop, on_job=on_job, **relax_params)
    if on_progress:
        on_progress({"phase": "before", **before})
    if should_stop and should_stop():          # stopped during the baseline measurement
        out = {"status": "stopped", "converged_period": None, "primary_metric": primary,
               "before": before, "after": {}, "iterations": []}
        if on_progress:
    ...
    if regional:
        def _build(period):
            return build_regional_skip_design(
                base_design, period, _state["deviation"], _state["strain"],
                w_dev=w_dev, w_strain=w_strain, min_spacing=min_spacing)
        ...
        build_fn, on_measure = (lambda p: build_sq_skip_from_design(base_design, p)), None

    result = iterate_to_constraint(
        build_fn=build_fn,
        adjust_fn=PeriodAdjuster(),
        constraint=constraint, workspace=workspace, initial_knob=seed_period,
        reference_fn=core_reference_geometry, max_iterations=max_iterations,
        production_steps=production_steps, max_production_rounds=max_production_rounds,
        timeout=timeout, should_stop=should_stop, on_job=on_job,
        early_reject_factor=early_reject_factor, screen_steps=screen_steps,
        on_measure=on_measure,
        on_iteration=(lambda rec: on_progress({"phase": "iteration",
                                               "period": rec["knob"],
                                               **(rec.get("verdict") or {})}))
    ...
    after = {**_metrics_from_verdict(av), "n_frames": av.get("n_frames"),
             "status": av.get("status"), "period": result.get("knob")}
    out = {"status": result["status"], "converged_period": result["knob"],
           "primary_metric": primary, "placement": "regional" if regional else "uniform",
           "before": before, "after": after,
           "iterations": [{"period": it["knob"],
    ...
    if regional and _state.get("design") is not None:
        ...
        out["converged_skips"] = {
            ...
            for h in fd.helices if any(ls.delta == -1 for ls in h.loop_skips)}
    ...
    if (finetune and not regional and result.get("knob") is not None
            and out["status"] in ("met", "exhausted")
            and not (should_stop and should_stop())):
        def _ft_progress(ev):
            if on_progress:
                on_progress({"phase": "finetune", "ft_phase": ev.get("phase"),
                             "i": ev.get("i"), "op": ev.get("op"),
                             "helix_id": ev.get("helix_id"), "bp_index": ev.get("bp_index"),
                             "accepted": ev.get("accepted"), "dev_max": ev.get("dev_max"),
                             "twist": ev.get("twist"), "n": ev.get("n")})

        converged_design = build_sq_skip_from_design(base_design, int(result["knob"]))
        ft = greedy_finetune_skips(
            converged_design, workspace, max_edits=finetune_max_edits,
            tol_twist_deg=tol_twist_deg, sigma=finetune_sigma,
            on_progress=_ft_progress, should_stop=should_stop, on_job=on_job,
            timeout=timeout, **relax_params)
        out["finetune"] = {k: ft.get(k) for k in
                           ("status", "n_candidates", "edits_kept", "before", "after")}
        out["placement"] = "finetuned"
        if ft.get("converged_skips"):           # final fine-tuned pattern → apply lays it verbatim
...
def iterate_sq_skips(cells, length_bp: int, workspace, *, tol_twist_deg: float = 5.0,
                     min_confidence: int = 400,
                     initial_period: int = SQ_SKIP_PERIOD_DEFAULT,
                     max_iterations: int = 6, scaffold: str | None = None,
                     p_min: int = 8, p_max: int = 400, early_reject_factor: float = 3.0,
                     **iterate_kwargs) -> dict:
    ...
    constraint = {"measure": "bundle_twist", "target_nm": 0.0,
                  "tol_nm": tol_twist_deg, "min_confidence": min_confidence}
