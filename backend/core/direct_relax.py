...
def duplex_midpoint_placement(
    design: Design, driver_oh_id: str, driven_oh_id: str,
...
def _min_clash_rotation(nucs: list[dict], moving_overhang_ids: set,
                        origin: np.ndarray, axis: np.ndarray,
                        *, threshold_nm: float = 1.8, n_samples: int = 24) -> float:
...
def _root_anchors(design: Design, nucs: list[dict], driver_oh_id: str,
                  driven_oh_id: str):
...
def relax_direct_binding(
    ...
    *,
    ...
    from backend.core.duplex_cluster import (
        dematerialize_duplex_cluster, duplex_cluster_for, materialize_duplex_cluster,
    )
    ...
    if len(selected) == 1:
        ...
        thetas = np.array([_optimize_chord_angle(
            moving, fixed, origin, axis, target_chord, tmin, tmax)])
    elif len(selected) > 1:
        def _loss(th: np.ndarray) -> float:
            pa, pb = _place(th)
            return ((float(np.linalg.norm(pa - pb)) - target_chord) ** 2
                    + _THETA_REG_LAMBDA * float(np.sum(th * th)))
    ...
    design_motion = design.model_copy(update={
    ...
    pa2, _ca2, pb2, _cb2 = _root_anchors(design_seated, nucs2, driver_oh_id, driven_oh_id)
    ...
    clash_theta = 0.0
    if (pa2 is not None and pb2 is not None and p0u is not None
    ...
    if _had_cluster:
        ...
        if dcl is not None:
            seek_log = list(updated.feature_log) + [ClusterOpLogEntry(
                cluster_id=dcl.id, translation=list(dcl.translation),
                rotation=list(dcl.rotation), pivot=list(dcl.pivot),
    ...
    _s, _i2, td2, rd2, cb_bp2, cr_bp2 = _find_driven_tip_and_root(design_seated, driven_oh_id)
    ...
    final_chord = float(np.linalg.norm(fb - fr)) if (fb is not None and fr is not None) else float("nan")
    final_root_chord = float(np.linalg.norm(pb2 - pa2)) if (pa2 is not None and pb2 is not None) else float("nan")
