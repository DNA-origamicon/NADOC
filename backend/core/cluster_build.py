...
DEFAULT_MODULES = ("gcc/11.2.0", "cuda/12.1.1", "cmake/3.27.7", "fftw/3.3.10",
                   "tcltk/8.6.11")
...
CHARM_KEEP = {"src", "contrib", "doc", "docs", "examples", "tests", "benchmarks",
              "tools", "util", "bin", "include", "lib", "cmake", "buildold"}
...
def validated_gencodes(gencodes) -> list[str]:
    ...
    """
    return [_check(g, _GENCODE_RE, "cuda gencode")
...
def namd_bin_path(build_dir: str, src_dir_name: str, namd_arch: str = "Linux-x86_64-g++") -> str:
...
def make_source_tarball(source_dir: Path, out_path: Path) -> Path:
    ...
    if not (source_dir / "config").is_file():
        raise ValueError(f"{source_dir} does not look like a NAMD source tree (no ./config)")
    ...
    def _skip(info: tarfile.TarInfo):
        ...
        if (len(rel) >= 2 and rel[0].startswith("charm-")
...
async def run_namd_build(
    ...
    *,
    ...
    _state.clear()
    _state.update({"phase": "packing", "build_dir": build_dir, "started_at": time.time()})
    ...
    logger.info("namd build: packed %s (%.0f MB)", tar_path, tar_path.stat().st_size / 1e6)

    await conn.mkdir_p(build_dir)
    await conn.sftp_put(str(tar_path), f"{build_dir}/{tar_name}")

    script = build_sbatch(
        build_dir=build_dir, src_dir_name=src_name, tar_name=tar_name,
        modules=modules, gencodes=gencodes, cores=cores,
    ...
    await conn.sftp_put_text(script, remote_sbatch) if hasattr(conn, "sftp_put_text") \
    ...
    if not m:
        _state.update({"phase": "failed", "error": (res.stderr or res.stdout or "")[:500]})
        raise RuntimeError(f"build sbatch failed: {(res.stderr or res.stdout or '').strip()[:400]}")

    slurm_id = m.group(1)
    _state.update({
        "phase": "queued", "slurm_job_id": slurm_id,
