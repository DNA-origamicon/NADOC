...
from backend.core.md_protocols import AKSIMENTIEV_STEPS_PER_CYCLE, _round_up_to_cycle
...
def predict_ns_per_day(n_atoms: int, *, gpu_resident: bool, gpu: bool = True) -> float:
    ...
    if n_atoms <= 0:
        return 0.0
    k = K_CPU if not gpu else (K_GPU_RESIDENT if gpu_resident else K_OFFLOAD)
    return k / n_atoms
...
def choose_water_shell(
    *,
    ...
    full_rate = predict_ns_per_day(full_atoms, gpu_resident=gpu, gpu=gpu) if full_ok else -1.0
    ...
    if carve_rate <= full_rate:
        ratio = full_atoms / max(1, carve_atoms)
        return (0.0, gpu,
                f"a {round(carve_shell * 10)} Å carve would only remove {ratio:.1f}x the atoms, "
                f"which does not pay for losing GPU-resident mode (needs >{CARVE_BREAKEVEN:.1f}x) — "
                f"this design largely fills its own bounding box")

    return (carve_shell, False,
            f"a {round(carve_shell * 10)} Å shell cuts {full_atoms:,} → {carve_atoms:,} atoms "
            f"({full_atoms / max(1, carve_atoms):.1f}x), which beats GPU-resident on the full box "
            f"(needs >{CARVE_BREAKEVEN:.1f}x)")
