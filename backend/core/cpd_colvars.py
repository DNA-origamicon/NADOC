...
def colvar_blocks(
    ...
    *,
    suffix: str = "",
    d_width: float = _WIDTH_D_ANG,
    d_extra: Sequence[str] = (),
) -> str:
...
def emit_colvars(
    ...
    *,
    ...
    target_ang: float | None = None,
    target_num_steps: int = 10_000_000,
    lower_ang: float = VDW_FLOOR_ANG,
    upper_ang: float = 12.0,
    ...
    * ``"eabf"``     — puts ``d_mid`` on an extended Lagrangian and applies ``abf``, so
    ...
    usable = [
        p
        for p in pairs or []
        if p.get("serials_resolved") is not False
    ...
    head = [
        "# Colvars for the designed extra-base UV weld (CPD).",
...
def umbrella_windows(
    d_start_ang: float,
    d_end_ang: float,
    *,
    spacing_ang: float = 0.5,
    wide_spacing_ang: float = 1.0,
