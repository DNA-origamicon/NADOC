...
def complete_psf(design: Design, model=None) -> str:
    ...
    tools (parmed, psfgen, VMD) are required.  Pass ``model`` (an
    AtomisticModel that already includes protein atoms) so the PSF matches a
...
def _complete_psf_from_stub(stub: str) -> str:
    ...
    for i in range(0, n_ang, 3):
        chunk = angles[i : i + 3]
        out.append(
            "".join(f"{serial(a):8d}{serial(b):8d}{serial(c):8d}" for a, b, c in chunk)
        )
    out.append("")
    ...
    for i in range(0, n_dih, 2):
        ...
        out.append(
            "".join(
                f"{serial(a):8d}{serial(b):8d}{serial(c):8d}{serial(d):8d}"
...
def _render_namd_conf(
    ...
    extrabonds_block = (
        ...
        "# plus the conjugation linker bond.  Generated by NADOC protein_enm.py.\n"
        "extraBonds         on\n"
