...
def _apply_cluster_transforms_domain_aware(
    ...
    # CHILD clusters (parent_cluster_id set) are applied FIRST, on the
    # un-transformed positions, masked to their own domains — so their local pose
    # composes INSIDE the parent: child domains end up at T_parent(T_child(p_rest)).
    # The child follows the parent rigidly yet its stored pose is drift-free. Normal
    ...
    # Step 1: CHILD clusters (inner, rest frame). Applied to their domain subset
    # before any parent transform; the parent below then poses everything on top.
    for cluster in child_clusters:
        mask = _domain_mask(cluster)
        if not mask.any():
            continue
    ...
    # child's already-locally-posed domains ride along rigidly with the parent.
    for cluster in helix_level_clusters:
        result = _apply_cluster_rigid_transform_arrays(result, cluster)
...
def _overhang_is_duplex_cluster_driver(design: "Design", ovhg_id: str) -> bool:
    """True if *ovhg_id* is the DRIVER of an auto-created overhang-DUPLEX cluster — i.e.
...
def _apply_cluster_transforms_to_point(
    ...
    #
    # An overhang-DUPLEX child cluster moves the centre-line ONLY when it FULLY covers this
    # helix (a DEDICATED overhang helix whose whole span IS the duplex — else the stick
    # stays at the lattice position while the beads move). When it covers only a bp SUB-range
    # (the duplex sits inline on a longer bundle helix), it is skipped here and the moved
    # sub-range is drawn by the per-domain `segments` path instead. CHILD clusters
    # (parent_cluster_id) apply FIRST so their local pose composes inside the parent.
...
def apply_overhang_rotation_if_needed(
    ...
    for ovhg in design.overhangs:
        ...
        # A non-identity rotation OR a non-zero translation drives the same
        # domain-aware synthetic transform (p' = R·(p−pivot)+pivot+translation).
        # The translation re-centers a bound DIRECT connection's relocated duplex
        # onto the midpoint of its two embedded-staple connections; the bound-driven
        # partner rides along via `partner_refs`, so the whole duplex shifts rigidly.
        if ovhg.rotation != _IDENTITY_QUAT or has_ovhg_trans:
...
def _apply_ovhg_rotations_to_axes(
    ...
    *nucleotides* (e.g. partial-helix response).
    ...
    for ovhg in design.overhangs:
        ...
        # Cluster-backed duplex: the pose lives on a child ClusterRigidTransform, so the
        # OverhangSpec overlay is cleared. Source ovhg_axes (the overhang SHAFT) from the
        # per-domain SEGMENT — already child-composed by deformed_helix_axes — so the shaft
        # follows the cluster + its beads instead of the inert overlay. Verified against the
        ...
        if pivot_raw is not None:
            # The junction pivot is read from the FINAL geometry, which ALREADY has
            # this overhang's rigid transform applied. Rotation leaves the junction
            # fixed, but the translation shifts it — so de-apply the translation to
            # recover the PRE-transform pivot the base axis samples are expressed in
            # (otherwise R·(sample−pivot)+pivot+trans double-counts the translation).
            pivot_arr = np.array(pivot_raw, dtype=float) - trans
        else:
            pivot_arr = np.array(ovhg.pivot, dtype=float)
        ...
        if h_obj is not None and h_obj.length_bp > 0:
            ...
            # Layer 1: legacy whole-overhang rotation + rigid translation across the
            # domain's bp range (samples outside the overhang domain — e.g. an inline
            # root portion — stay put, matching the domain-masked bead transform).
            if ovhg.rotation != _IDENTITY_QUAT or has_ovhg_trans:
                new_samples = [
                    (R @ (np.array(pt) - pivot_arr) + pivot_arr + trans).tolist()
