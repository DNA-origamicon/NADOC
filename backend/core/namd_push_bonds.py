"""Interhelical "push bonds" — the vacuum-phase repulsion surrogate.

Transcribed from ``mrdna/segmentmodel.py::write_atomic_ENM`` (the ``push_bonds`` block),
which is the same lab's successor to the ENRG-MD service the Aksimentiev origami tutorial
uses in §3.2.  These are the k = 1.0 kcal/mol/Å², r₀ = 31 Å phosphate-phosphate bonds that
hold helices at their proper spacing **in vacuum**, where Coulomb is truncated at 10 Å and
cannot do it.
...
bundle generates ZERO push bonds — they appear only where crossovers are sparse, which is
exactly where a structure would otherwise collapse without electrostatic support.  This
reproduces the end-weighted distribution measured on the tutorial's own hextube
(64 push bonds alongside 75,503 ENM terms).

Measured on NADOC designs (``experiments/exp48_vacuum_enrgmd/REPORT.md``, 2026-07-30):
2hb → 0 bonds, 6hb → 11, 24hb → 495, VoltronCore → 3143.  r₀ = 31 Å was validated as safe
on the 6hb (0 of 252 base pairs broken) even though it stretches those sites ~50 % past
...
def atom_resolver(design, pdb_text: str, names) -> dict:
...
def watson_crick_pairs(design, pdb_text: str) -> list:
    ...
    purine-N1 to pyrimidine-N3.  Verified on an idealised 2hb build: A-T lands at 2.60 Å
    and G-C at 3.39 Å, versus 4.7-6.6 Å for every mis-assigned combination.
    """
...
def _p_atom_resolver(design, pdb_text: str) -> dict:
...
def interhelical_push_bonds(design, pdb_text: str, *,
    ...
    SURROGATE — in vacuum, Coulomb is truncated at 10 Å, so nothing stops helices
    collapsing together, and the spring is deliberately longer than the equilibrium
    spacing to push them apart.  NADOC's honeycomb is 2.25 nm centre-to-centre and the
    phosphates this rule selects face each other at ~20.6 Å on an idealised 6hb build, so
    31 Å expands those sites ~50 %.  exp48 ran the 6hb both ways: 31 Å broke none of 252
    base pairs, and the shape-preserving arm drifted outward on its own (20.6 → 22.6 Å),
    so the structure wants to open in vacuum regardless.  Pass ``None`` to use each bond's
    MEASURED length instead, making the term shape-preserving rather than shape-setting.
    """
