...
from backend.core.models import (
    ...
    Design,
    DomainRef,
    _local_to_world_joint,  # noqa: F401
)
...
def conjugate_world_pose_into_parent_rest(
    ...
    pivot_world,
    t_world,
    parent: Optional[ClusterRigidTransform],
    ...
    for s in design.strands:
        for di, d in enumerate(s.domains):
            if d.overhang_id == driver_oh_id:
                ...
                return d.helix_id, [
                    DomainRef(strand_id=s.id, domain_index=di),
                    *partners,
                ]
    return None, []


def _owning_parent_cluster(
    design: Design, helix_id: str
    ...
    candidates = [
        ...
        if not c.domain_ids
        and c.overhang_duplex_driver_id is None
...
def duplex_cluster_for(
    design: Design, driver_oh_id: str
) -> Optional[ClusterRigidTransform]:
...
def materialize_duplex_cluster(
    ...
    *,
    ...
    cleared_specs = [
        o.model_copy(
            update={"rotation": _IDENTITY_QUAT, "translation": [0.0, 0.0, 0.0]}
        )
    ...
    cluster = ClusterRigidTransform(
        ...
        pivot=piv_c,
        translation=t_c,
        overhang_duplex_driver_id=driver_oh_id,
        auto_created=True,
        **kw,
    )
    out = cleared.model_copy(
        update={"cluster_transforms": [*design.cluster_transforms, cluster]}
    )
    return out, cluster.id


def _duplex_driven_overhang(
    design: Design, cluster: ClusterRigidTransform
) -> Optional[str]:
...
def duplex_cluster_rotation_points(
    design: Design, cluster: ClusterRigidTransform
) -> list[dict]:
    ...
    dup = [
        np.asarray(n.get("backbone_position") or n.get("base_position"), dtype=float)
        for n in nucs
        if n.get("overhang_id") in (driver, driven)
    ]
    if dup:
...
def duplex_cluster_tethers(
    ...
    for oh_id in (driver, driven):
        if not oh_id:
            continue
        try:
            _s, _idx, tip_dom, root_dom, c_bp, p_bp = _find_driven_tip_and_root(
                design, oh_id
            )
        ...
        out.append(
            {
                "moving": {
                    "helix_id": tip_dom.helix_id,
                    "bp": int(c_bp),
                    "direction": getattr(tip_dom.direction, "value", tip_dom.direction),
                },
                "fixed": {
                    "helix_id": root_dom.helix_id,
                    "bp": int(p_bp),
                    "direction": getattr(
                        root_dom.direction, "value", root_dom.direction
                    ),
                },
...
def set_duplex_cluster_pivot(design: Design, cluster_id: str, new_pivot) -> Design:
    ...
    new_cl = cl.model_copy(
        update={
            "pivot": [float(newp[0]), float(newp[1]), float(newp[2])],
...
def dematerialize_duplex_cluster(design: Design, driver_oh_id: str) -> Design:
    ...
    parent = (
        next(
            (c for c in design.cluster_transforms if c.id == cluster.parent_cluster_id),
            None,
        )
    ...
    r_w, _piv, t_w = _child_local_pose_to_world(
        design,
        driver_oh_id,
        cluster,
        parent,
    ...
    return design.model_copy(
        update={"cluster_transforms": new_clusters, "overhangs": new_overhangs}
    )


def _child_local_pose_to_world(
    design, driver_oh_id, child, parent, geom_fn, pivot_fn
) -> tuple[list[float], list[float], list[float]]:
    ...
    cleared = design.model_copy(
        update={"cluster_transforms": others, "overhangs": cleared_specs}
    )
