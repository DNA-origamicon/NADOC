---
name: overhang-duplex-foundation-archive
description: "ARCHIVE — the 2026-06-30/07-01 build history of the Duplex graph. Head is project_overhang_duplex_foundation.md; do not read this in a routine loop."
metadata:
  node_type: memory
  type: project
---

> **ARCHIVE (split 2026-07-30).** Build history only, as written during the phases. Current
> state, invariants and open items live in `project_overhang_duplex_foundation.md`. **Do not
> read this file in a routine loop** — open it only to mine one specific past decision.
> Known-stale here: the "relax 422s when the driven tip isn't on the driver helix" claim
> (it 422s on *no relocated tip domain found*), the "Phase 0: nothing consumes duplexes"
> framing, and every phase's `just test` pass count.
