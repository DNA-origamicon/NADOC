...
**Popup must carry troubleshooting details (2026-09-18).** Every working popup (`#op-progress` via `ui/op_progress.js`, and the `#file-load-progress` overlay) shows the process + a live elapsed timer and a **Copy details** button that puts operation, status, process, elapsed, and page URL on the clipboard. Any new working popup must do the same. For API-driven popups `client.js` passes `detail: "METHOD /path (request #id)"` and `startedAt` (request start, not popup-show time). `showOpProgress` returns a token; pass it to `hideOpProgress(token)` when concurrent operations can overlap.
