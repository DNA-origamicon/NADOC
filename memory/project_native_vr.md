...
- Next slice: map stable primitive identities to read-only desktop hover descriptors across selection levels, then specify a non-conflicting click gesture before allowing canonical selection writes. Phase 1 and Phase 3 remain pending their manual headset gates.
...
Map stable VR primitive identities to existing desktop preview descriptors and test every selection level without mutating canonical selection. Then decide the click gesture around the existing trigger-grab conflict. Phase 1 and Phase 3 stay at manual gates until the headset checklists above pass.
