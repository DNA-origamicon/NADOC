...
- Next slice: isolated generated fixtures for each Phase 1 topology fact plus one mixed-design golden snapshot; then wire the comparator into the smart test path. Phase 1 remains pending the manual headset gate.
...
Build isolated generated fixtures and one mixed-design golden snapshot, then wire stable-ID comparison into the smart test path while Phase 1 awaits the manual headset gate. Do not promote Phase 1 until both numeric parity and the evidence above pass.
