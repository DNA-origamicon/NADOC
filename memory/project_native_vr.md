...
- Selection-feedback checkpoint: click one valid target and confirm a larger green marker replaces ambiguity around the cyan hover cue after the desktop turns green; move/scale and hold Expanded View to confirm the marker follows. Click again to deselect, then try an invalid target at End and Crossover levels. **No-go** if green appears before canonical acknowledgement, survives deselection, attaches to a different primitive, lags noticeably, or appears for an invalid pair. Switching representations may temporarily omit the marker until cross-representation ownership lands.
...
- Next slice: add versioned owner-alias records to natural/Expanded scene primitives and resolve the first matching canonical feedback token after every representation switch; then complete the Phase 4 selection matrix and manual gate. Phase 1 and Phase 3 remain pending their manual headset gates.
...
Attach the transported canonical owner hierarchy to versioned scene primitives and activate cross-representation selected-marker fallback, preserving v4-v7 readers and exact natural/Expanded identity parity. Phase 1 and Phase 3 stay at manual gates until the headset checklists above pass.
