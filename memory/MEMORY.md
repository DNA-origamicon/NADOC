...
**atomistic_source_of_truth** (P0 — the stamp no longer reads the display bead; carries the RE-VERIFIED audit table of every CG coupling. Head + `_archive`. READ BEFORE any geometry, atomistic or sim-seed work) ·
**helical_site** (one site abstraction all reps project from; CLOSED, all phases 0-10 done. Head + `_archive`. Read with the above)
