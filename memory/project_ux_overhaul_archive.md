# UX Overhaul — Archive (shipped history, 2026-05-16 → 2026-07-16)

> **Do not read this in a routine loop.** History only. Current state, the live open
> items, and every correction from the 2026-07-31 plan audit live in
> `project_ux_overhaul.md` (the head).
>
> **Known stale (audited 2026-07-31):** two "SHIPPED" entries below describe code that
> was later deleted — `input/drag_scrub.js` (item 7) and `scene/overhang_binding_lines.js`
> (last row of the 2026-05-16 table). Both are **gone from the repo**. `originAxes.visible
> = false` (part-origin gizmo fix #2) was **reverted**. Many `main.js` symbols named below
> moved out during the carve-up. See the head's correction table.
...
## Shipped write-ups — 2026-05-17 session
...
## Batch B items 2–7 — original specs (all resolved; see head for status)
