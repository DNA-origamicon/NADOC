...
| Master jobs card | `frontend/src/ui/simulate_jobs.js` — `masterStepText`:187 (exported), `_stepTotal`:173 | live; **only importer is `main.js:207`**. Panels notify it by `window.dispatchEvent('nadoc:sim-jobs-changed')` → listener `simulate_jobs.js:654` |
