...
**UPDATE 2026-06-26 — autoscaffold now ROUTES THROUGH FLs (hinge case).** The "skip protected helices / preserve protected scaffold strands" rule above is no longer the only behavior. `auto_scaffold_seamed` now first tries `route_hinge` (`backend/core/hinge_router.py`), which treats FLs as **mandatory cross-gap scaffold bridges** and routes ONE strand through all of them (hinge primitives). It only triggers for clean cross-leaf FL pairings; a genuine one-off / intra-leaf manual FL still returns None → the old preserve pipeline. So FLs are still never touched by autocrossover/autobreak, but autoscaffold-seamed is now FL-aware. See [[project_hinge_autoscaffold]].
