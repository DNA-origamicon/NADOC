...
7. **OH rotation must be co-applied to LINKER complement domains** — `apply_overhang_rotation_if_needed` (deformation.py) builds the synthetic transform `domain_ids` from the OH domain PLUS any LINKER strand domain on the same helix overlapping the OH bp range with opposite direction. Without this the linker bridge anchor renders un-rotated even though the OH visibly rotates. Regression: `tests/test_overhang_linker_rotation.py`.
