# Overhang Duplex as a Cluster — ARCHIVE (design narrative + phase log, 2026-07-01)

**History only. Do not read this in a routine loop** — the current state, the probed code map,
and the live open items are in `project_overhang_duplex_cluster.md` (the head).

**Known stale (audited 2026-07-30):** every line below was written while the work was in flight,
so it mixes proposal tense with completion notes and its line anchors have drifted (`crud.py:7336`
is now `:7622`; `deformation.py:1808` is now `:1897`; `models.py:1070/1243` have shifted ~110 lines).
The §10 "Open questions for the user (before P0)" were all ANSWERED — see `## Decisions (user,
2026-07-01)` below. One claimed pin, `test_real_fixture_materialize_is_bead_and_shaft_neutral`,
**does not exist anywhere in the repo**. Mine this file for a specific past decision (the
frame-conjugation math, the DEAD END list, the two P2 bug root-causes); trust the head for state.
