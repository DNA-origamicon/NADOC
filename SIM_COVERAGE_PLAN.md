...
- ▶ STATE — **`U1` DONE 2026-07-08** (unified-panel foundation). `engine_capabilities.js` = the single source of truth: 5 engines × 8 cards, each enabled (panel renders it today) or unsupported-with-a-reason (the absent→greyed-with-tooltip shift U4 needs). PARITY census oracle, 19 FAST tests. main.js LOC-Δ=0 (data only, consumed by U2–U4). Feature tail (M4/N3/O2) parked. M-UNIFIED-PANEL still pending (U2/U3/U4 remain).
- ▶ NEXT — **stay on U/P.** Recommend **`U2`** (shared Forces card factory — now that U1 gives the per-engine card matrix, collapse the triplicated efield/anchors/surface DOM `efield_setup.js`/`cando_efield_setup.js`/`lammps_forces_setup.js` into one `initForcesCard({engine,deps})` over `efield_math`/`oxdna_floor_math`, greying unsupported sub-cards per U1). **ADAPTED-CODE PIN RULE applies** (moved helpers → prove in-place-first or git-stash rerun, NOT green-first-run). U2 owes an MV row. Alternatively **`P1`** (MdPipeline stage-spec, dep-free) to open the chaining track. U3 also unblocked (needs only U1).
