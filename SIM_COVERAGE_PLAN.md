...
- ▶ STATE — **M-METRIC-CORE DONE** (S1–S5); **M-CANDO-FIELD/COMPLETE DONE** (C1–C5); **M-ALL-ANCHORS-FIELD DONE** (CanDo C1/C2, NAMD N1/N2, mrDNA M1/M2). **`N4` NAMD card-source DONE 2026-07-08 → the comparison card's engine roster is COMPLETE: FOUR live engines (oxDNA O1, CanDo C5, mrDNA M5, NAMD N4), and NAMD is the GOLD-OVERRIDE reference.** N4: `backend/core/namd_shape_source.py` `build_namd_shape_source(shape_frame, core_reference, *, rmsf_positions=None, field=None)` — the O1/C5/M5 twin, same SOURCE-BUNDLE CONTRACT. It is BEHAVIORALLY the oxDNA builder with `engine="namd"`: `md_trajectory.md_rmsf` emits the SAME positions shape as `production_rmsf` (each entry carries BOTH `backbone_position` AND `rmsf`, string `direction`), so shape + RMSF come from ONE Kabsch-aligned `md_rmsf` pass (time-mean structure = low-noise shape; per-nt trajectory variance = RMSF). Route `GET /md/jobs/{id}/shape-source` (`get_md_shape_source`) reuses `_md_traj_inputs` (job's FROZEN prepared snapshot) + `_run_md_analysis(...'rmsf','md_rmsf'...)` (shares the flexibility-map cache), passing `result["positions"]` as BOTH `shape_frame` and `rmsf_positions` → `{ready, n_frames, ...bundle}`. **The gold override is N4's value-add, NOT the builder:** `shape_metrics.reference_for` (`_GOLD_ENGINE="namd"`, wired in S3) returns `"namd"` for every observable once a NAMD source is present, and `build_comparison_report` honors it — so the oracle's headline is `references.shape=='namd'` AND `references.rmsf=='namd'` on `[oxdna,cando,namd]` (+ a negative control proving the flip is NAMD-caused). Frontend `api.getMdShapeSource` + `getSources` → `[oxdna, cando, mrdna, namd]`; `main.js` captures `const mdPanel` + lazy `getMdJob` (+3 LOC, no TDZ — `mdPanel` init'd BEFORE `oxdnaPanel`). Oracle `tests/test_namd_shape_source.py` 7 fast + 1 slow (the slow real-2hb-DCD test RAN on-machine). `just test` 4362 passed / 72 skip / 1 xfail (no drop; the prior xdist active-design flake `test_namd_efield` PASSED this run); vitest 2294; smoke green. Field deferred (`field:None`, like O1/C5/M5). Live 4-engine eyeball owes **MV-21** (updated w/ N4 slice + reference-relabel check). **⚠ inherited uncommitted `md_vram` WIP in the tree (host-OOM handling) — only the N4 hunk of `routes_md.py` was staged; leave the rest for its own commit.**
- ▶ SOURCE-BUNDLE CONTRACT (proven by O1+C5+M5+**N4** — COMPLETE) — each engine builds `{engine:"<lower>", descriptors:compute_shape_descriptors(core_frame), rmsf:[{helix_id,bp_index,direction,copy,rmsf_nm}], shape_frame:core_frame, field:...|None}` and returns it from `getSources`; **core-filter first** (`_filter_to_reference_core` vs `core_reference_geometry`), emit the engine's own ABSOLUTE descriptors. NAMD (like oxDNA) reuses the `md_rmsf`/`production_rmsf` positions list for BOTH shape and RMSF (one entry carries both). **`main.js` capture-panel + lazy `get<Engine>Job` = +3–4 LOC pure wiring, no TDZ.**
- ▶ NEXT — The comparison-card roster is DONE (O1/C5/M5/N4). Remaining toward **M-FULL-COVERAGE** = the feature-coverage TAIL: **`M3`** (extra bases into the ARBD model — verify past the existing display bridge `extra_base_flank_keys` that inserts are present + ssDNA/flexible in the ARBD topology; deps met, FAST) → **`M4`** (mrDNA linkers, dep M3 — reuse C4's WLC-connector pattern) · **`N3`** (NAMD extra-base/linker validation coverage — already-atomistic path; assert model includes insert/linker atoms + descriptors computable from an MD frame; deps met, FAST) · **`O2`** (oxDNA maintenance, pull only on a logged gap). **Recommend `M3`** (lowest effort, unblocks M4, closes the mrDNA feature row) or `N3` (fast, independent). **For M3/N3: extra-base/linker softening → assert RMSF FLEXIBILITY or bead/atom PRESENCE, NEVER a twist/bend DIRECTION** (crossover-geometry reasoning forbidden — banked gotcha).
