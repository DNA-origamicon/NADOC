...
- ▶ STATE — **`U3` SLICE 2c-3a DONE 2026-07-08 (U3 still `in_progress`).** Converged the **oxDNA** panel's SECTION-COLLAPSE + POLL onto `initJobsPanelBase` (4th consumer). `arrowStyle:'class'`; hasActive = visible-active OR selected-running. Deleted module-level `_collapsed`/`_pollTimer`/`_scheduleNextPoll`; rewired `_revealMdPanel`/left-tab-change/workspace-path-change/init to `_base.isOpen()/applyCollapsed/clearPoll/schedulePoll/initCollapsed`. **ADVANCED drawer LEFT bespoke — deliberate:** oxDNA's `_advOpen` boolean opens on first click, but the base reads `display` and the real markup `style="display:none;display:grid"` computes to **visible** (last dup wins), so converging it would FLIP the first click. Oracle `oxdna_jobs_panel.test.js` **+3** (90/90): PARITY drives the REAL `initOxdnaJobsPanel` through collapse (is-collapsed class) + poll gate + stop-on-collapse — the stop-on-collapse assertion **FAILED pre-rewire (3 vs 2:** bespoke let one trailing poll fire) and PASSES post = adapted-code pin, not green-by-construction (base's `clearPoll()` on collapse = the one benign delta). Fresh-context review: **all 6 adapted sites CONFIRMED equivalent, no TDZ.** Gates: oxDNA 90/90, frontend **2362/2362** (+3 no drop), smoke 23/23, `main.js` Δ=0, no Python. **MV-26 owed** (in-app collapse gesture: one-off Playwright blocked by unrelated left-tab-switch plumbing — `#tab-content-dynamics` stayed display:none — did NOT rabbit-hole). Prior done: U1, U2, U3-s1/s2a/s2b/s2c-1/s2c-2, P1, P2. Feature tail (M4/N3/O2) parked.
- ▶ NEXT — **`U3` slice 2c-3b (converge md scaffold), then finish `U3`→`U4`.** **2c-3b — converge the 2803-line md panel onto `initJobsPanelBase`.** md uses a **module-level `_collapsed` var read in ~6 sites** → replace with `_base.isOpen()` (wider *adapted* change; prove in-place-first or git-stash rerun, NOT green-first-run). md's SECTION arrow is `arrowStyle:'class'` (verify against the source, per the oxDNA correction) — `'rotate'` is only the advanced-drawer arrow. **md adds a 2nd remote `setInterval` poll** (`_remotePollTimer`) **+ cross-panel md-collapse coordination** (it force-collapses oxDNA when md opens, mirroring oxDNA's `_revealMdPanel`) — these stay BESPOKE around the base (the base owns only the primary poll + section collapse). Check whether md's adv-drawer has the same `_advOpen`-boolean-vs-display-read hazard oxDNA had (if so, leave it bespoke too). Then **`U4`** (engine selector + one *Simulate* section, unsupported cards greyed-with-tooltip per U1; dep U2✓+U3). **FRONTEND DISCIPLINE (CLAUDE.md):** PARITY in **vitest** driving the real panel; byte-identical rewires proven OLD-then-NEW → equal; adapted rewires proven via git-stash rerun or in-place-first (an assertion that FAILS on bespoke + PASSES post is the strongest pin). `main.js` imports + factory init + thin wiring only. **PARKED:** `P3`/`P4` (job-planner) — resume Track P after U4.
