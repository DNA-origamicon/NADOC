...
- ▶ STATE — **`S1`+`S2`+`S3`+`S4` DONE** (M-METRIC-CORE: only S5 left). `shape_metrics.py` now holds the full math stack: `compute_shape_descriptors` (S1); `deviation_profile`/`rmsf_from_ensemble`/`normalize_rmsf_profile` (S2); `compare_descriptors`/`reference_for` (S3); **`field_response_profile`/`compare_field_response` (S4, 2026-07-06, 13/13 fast oracle `tests/test_shape_field_response.py`; `just test` 4155 passed).** **Only S5 remains for M-METRIC-CORE.**
- ▶ NEXT — **`S5`** (medium, deps=S3 met): the comparison CARD — `routes_shape_metrics.py` (daemon-thread registry, `POST /shape/compare/start` + `GET /shape/compare/{run_id}`) + `shape_compare_card.js` (mirror the oxDNA metrics-card pattern; overlay profiles + scalar/agreement table + a **field panel** driven by S4; PNG/CSV export via `metric_graph`/`metric_export_modal`). Fast pure-helper unit tests (chart spec, CSV rows, agreement/field table) + smoke + one-off display-vs-oracle Playwright + an `MV-N` row for live cross-engine data. S5 closes M-METRIC-CORE and unblocks C5/O1/M5/N4.
- ▶ ALSO PICKABLE NOW — `C1` (CanDo anchors, low), `N1` (NAMD eField, low), `N2` (NAMD anchors, low), `M3`, `O1` (oxDNA reference column, deps=S1 met, low). `C1`→`C2` is the fast path to **M-CANDO-FIELD** once S5 exists.
- ▶ REUSE — S5 card binds `compare_descriptors(candidate, reference, *, align_shape=True)` → `{candidate, reference, scalars{name:{candidate,reference,abs_delta,signed_pct_delta}}, rmsf{pearson,spearman,n,…}, shape_rmsd_nm}` and `reference_for(engines, observable)`. **Field panel binds S4:** `field_response_profile(field_pos, ref_pos, field_dir, anchor_keys)` → per-nt deflection map + `{anchored_max_drift_nm, free_proj_along_field_nm, deflection_vec_nm, passed, …}`, and `compare_field_response(cand, ref)` → `{cosine_similarity, magnitude_ratio, n_shared_free}`. **RMSF matching is per-BASE-PAIR** (`_rmsf_per_bp`); **field-response is NOT Kabsch-aligned** (anchored region is the common frame — don't align); both are copy-aware `(helix,bp,dir,copy)`.
- ▶ GOTCHAS banked — **cross-engine RMSF must collapse to per-bp before matching** (direction-keyed → silent empty intersection → `None`; LESSONS-banked-from-S3). **field-response: anchor membership is copy-agnostic but the deflection map is copy-keyed; never Kabsch-align field frames.** deviation cross-engine = **align=True**; degenerate correlation → `None` not NaN. Uniform field needs ≥1 anchor (COM-drift) in every engine; CanDo bend/twist exp36-calibrated (don't regress); every output Physical-layer only; repo has unrelated pre-existing ruff debt (19 errors in OTHER test files) — not a regression, don't sweep (`feedback_no_bulk_reformat`).
- ▶ REFERENCE — per-observable: oxDNA=shape+field, CanDo=RMSF, NAMD=gold-when-present (wired in `reference_for`). Metrics MUST be generate/view/export (mirror the oxDNA card). Auto-commit to master, no push.
