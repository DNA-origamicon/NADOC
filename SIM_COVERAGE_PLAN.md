...
- ▶ STATE — **`N3` DONE 2026-07-08 → NAMD feature column complete (N1–N4).** Was framed as pure validation coverage; turned up a REAL latent crash. `md_rmsf` keys a crossover insert `("__xb__",crossover_id,k)` = a STRING `bp_index` (`atomistic_to_nadoc.md_pkey`), and `namd_shape_source._rmsf_profile` did `int(p["bp_index"])` UNCONDITIONALLY → `ValueError` on ANY design with a linker; N4's slow test used a plain 2hb so never hit it. FIX: `_rmsf_profile` drops non-int `bp_index` (mirrors `_core_column_key`, which already drops inserts from the shape core) — shape + RMSF now symmetric. Added a defensive INVARIANT comment to `oxdna_shape_source._rmsf_profile` (same fragile line, safe only because its reader uses `include_extra_bases=False`). Oracle `tests/test_namd_extra_bases.py` **8 fast** — Part A (5, atomistic path already correct): `extra_bases="TT"`→ EXACTLY +2 DT residues, each tagged `(crossover_id,extra_base_k∈{0,1})`, full ribose+base+`O3'/P/O5'` linker, threaded INLINE; Part B (3, RED pre-fix): descriptors+rmsf emit from an insert-containing MD frame, inserts dropped from BOTH `shape_frame`+`rmsf`, descriptors BYTE-IDENTICAL to insert-free. Gates: 8/8 oracle (3 crashed pre-fix = RED proven), `just test` **4435/110/1** (no failures; baseline 4362), lint clean on touched (19 pre-existing debt untouched); backend-only → no vitest/smoke/new-card Playwright; main.js Δ=0. Review CONFIRMED correct, Three-Layer clean, O1 not reachable-buggy. **Live extra-base NAMD compare-card eyeball folds into MV-21.**
- ▶ NEXT — **Only M4 + O2 remain for M-FULL-COVERAGE (the last pending milestone).** Eligible pending = **M4** (mrDNA linkers into the ARBD model; deps M3✓; medium effort/leverage — closes mrDNA's feature column; reuse M2's per-type grid + regen-wrap + the mrdna_extra_bases pattern) and **O2** (oxDNA residual gaps; deps ∅; low/low — pull only when a concrete gap is logged, else it's a no-op). Rubric → **prefer `M4`** (real coverage gap, mirrors N3/M3). Oracle for M4 = FAST: the ARBD/mrDNA model materializes linker beads as ssDNA (nt-count grows by the linker total, inserts unpaired + threaded in-chain, base identity from the linker seq) — a property, not "it ran"; mirror `tests/test_mrdna_extra_bases.py`. Note: P2's follow-up (forces into the production RESEED conf end-to-end — `ProductionRunRequest.field/anchors`+reseed emission) stays open but is NOT a JSON task; the chain threads forces at the conf-snippet level today.
