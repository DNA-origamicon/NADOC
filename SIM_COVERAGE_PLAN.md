...
- ▶ STATE — **`U3` SLICE 2c-1 DONE 2026-07-08 (U3 still `in_progress`).** Factored the shared STATEFUL scaffold every jobs panel repeated — **section-collapse + advanced-parameters drawer + REST poll loop** — into one factory `initJobsPanelBase({section,els,pollMs,hasActive,tick,onOpen,onClose,arrowStyle})` in `jobs_panel_base.js` (pure `bodyDisplay`/`arrowChar`/`shouldPoll`/`applyArrow`). Converged **mrDNA + CanDo** (the two panels whose `_applyCollapsed`/heading/advToggle/`_clearPoll`/`_scheduleNextPoll` were **byte-identical**) onto it: each deleted ~21 LOC + drives the base with its own `onOpen`/`hasActive` (CanDo's separate display-card collapse stays bespoke). Oracle `jobs_panel_base.test.js` **+17** (jsdom CONFORMANCE: collapse toggles body+arrow+persists, adv-drawer, poll fires tick after pollMs ONLY open&&active + cleared on collapse — fake timers). Fresh-context review: **NO regressions** (all 5 checks confirmed — byte-equivalence, no lost call sites, no TDZ, per-panel behaviour kept, oracle non-superficial). Gates: affected 83/83, frontend **2356/2356** (+17 no drop), smoke 23/23, one-off Playwright drove mrDNA+CanDo collapse+adv in-app (0 console errors, deleted), lint N/A. `main.js` LOC-Δ=0; panels −42 LOC. **No MV row owed** (behaviour-preserving, no new pixels; MV-25 covers NAMD rows). Prior done: U1, U2, U3-s1/s2a/s2b, P1, P2. Feature tail (M4/N3/O2) parked.
- ▶ NEXT — **`U3` slice 2c-2 (converge the remaining 3 panels' scaffold), then finish `U3`→`U4`.** **2c-2 — converge LAMMPS/oxDNA/md onto `initJobsPanelBase`.** Each needs an accommodation the base already supports or notes: **LAMMPS** = `arrowStyle:'class'` (section arrow via `is-collapsed`) + an `onClose` doing `_viewsOff()`+`detachGizmo()` + `hasActive:()=>anyActive(_visibleJobs())` — NOTE its poll LACKS the open-guard mrDNA/CanDo have, so adding `schedulePoll`'s guard is an *adapted* change (prove via git-stash rerun or in-place-first, not green-first-run) + keep its separate viz-card collapse bespoke; **oxDNA/md** = `arrowStyle:'rotate'` + they use a `_collapsed` var + header-toggle (not `_applyCollapsed`), md adds a 2nd remote `setInterval` poll (`_remotePollTimer`) + cross-panel md-collapse coordination — base may need a small `applyArrow('rotate')` on the advanced drawer (`advArrowStyle:'rotate'`) too. Then **`U4`** (engine selector + one *Simulate* section, unsupported cards greyed-with-tooltip per U1; dep U2✓+U3). **FRONTEND DISCIPLINE (CLAUDE.md):** PARITY in **vitest pure helpers**; byte-identical rewires proven by driving OLD verbatim-copy + NEW → byte-equal (NOT green-first-run); any *adapted* rewire (e.g. LAMMPS poll-guard) proven via git-stash rerun or in-place-first. `main.js` imports + factory init + thin wiring only. **PARKED:** `P3`/`P4` (job-planner) — resume Track P after U4.
