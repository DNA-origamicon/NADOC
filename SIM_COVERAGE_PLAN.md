...
- ▶ STATE — **`U3` SLICE 2a DONE 2026-07-08 (U3 still `in_progress`).** Converged **cando + lammps** job LISTS onto the canonical `jobs_panel_model.js`/`jobs_panel_render.js` (flat like mrDNA) → the renderer now serves **4/5** panels. Renderer gained an **OPTIONAL per-row control** (`m.action`/`onAction`, gated on `ctx.rowAction`) so **lammps keeps its inline Stop button** (stopPropagation → no row-select); oxDNA/mrDNA/cando omit `rowAction` → `action=null` → no button → **oxDNA byte-parity UNCHANGED** (pin still green). cando's Coarse/Fine mode rides the canonical **leading-tag slot**; both gained `[N]` index + spinner + legend + poll-sig short-circuit; old bespoke rows + `statusBadge`/`statusKeyFor` imports DELETED. Oracle `jobs_panel_model.test.js` **14/14** (+5). Gates: affected panels 166/166, frontend **2330/2330** (+5 no drop), smoke 23/23, lint N/A (backend-ruff only). `main.js` LOC-Δ=0. Live cando/lammps pixels owe **MV-24**. Prior done: U1, U2, U3-s1, P1, P2. Feature tail (M4/N3/O2) parked.
- ▶ NEXT — **finish `U3`: slice 2b (converge md) then 2c (stateful base).** **2b — md is the 2882-line OUTLIER:** hierarchical via `flattenJobTree` **with `{collapsedIds}`** + expand/collapse chevron + ensemble-summary, `setInterval` + remote/Alpine timers, returns no api. Model already supports `collapsedIds`; the chevron is an extra per-row control — **reuse the slice-2a `m.action` mechanism** (add a `chevron` field to the row model + a click cb, gated on `childCount>0` and ctx opt-in) so oxDNA byte-parity is NOT broken. **2c — factor** the shared run-buttons + poll loop + section-collapse + advanced-drawer host into a stateful `initJobsPanelBase({engine,descriptor,deps})` (`runButtonEnabled` is built+tested but NO panel consumes it yet). Then **`U4`** (engine selector + one *Simulate* section, unsupported cards greyed-with-tooltip per U1; dep U2✓+U3). **FRONTEND DISCIPLINE (CLAUDE.md):** PARITY in **vitest pure helpers**; oxDNA rewires stay byte-identical (drive OLD verbatim-copy + NEW on fresh DOMs → byte-equal, NOT green-first-run, no `git stash`); mrDNA/cando/lammps/md convergence = intentional upgrade (conformance test + `just smoke` + an MV row). `main.js` imports + factory init + thin wiring only. **PARKED:** `P3`/`P4` (job-planner) — resume Track P after U4.
