...
- ▶ STATE — **`U3` DONE 2026-07-08 (slice 2c-3b closed it — all 5 panels converged).** Converged the **md (NAMD)** panel's SECTION-COLLAPSE **and ADVANCED DRAWER** onto `initJobsPanelBase` (5th/last consumer). `arrowStyle:'class'`, `advArrowStyle:'rotate'`; **NO tick/hasActive** — investigation found md uses NO primary poll (`_pollTimer` was vestigial/dead); its WebSocket + `_remotePollTimer`/`_prewarmTimer` stay bespoke, torn down by the `onClose` hook. `_collapsed` lived in only 3 sites (not "~6"). Deleted `_collapsed`/`_advOpen` + the section+adv blocks + the now-unused `section_collapse_state` import; terminal `if(!_collapsed) _onOpen()`→`_base.initCollapsed(true)`. **md's adv drawer CONVERGES** (its markup is a clean `display:none` — no oxDNA-style `display:none;display:grid` flip hazard). Oracle `md_jobs_panel.test.js` **+3** (75/75): PARITY drives the REAL `initMdJobsPanel` (auto-mocked api) through collapse + first-click-adv-open + onOpen-fetch + onClose-poll-stop — behaviour byte-identical, so proven by **git-stash rerun** (green on bespoke AND converged). Fresh-context review: all 6 points CONFIRMED, no TDZ. Gates: md 75/75, frontend **2365/2365** (+3 no drop), smoke 23/23, `main.js` Δ=0, no Python. **MV-27 owed** (in-app md collapse+adv gesture; behaviour-preserving → no Playwright leg). Run-button host left per-panel (engine-divergent; cohesion) — `runButtonEnabled` stays a shared helper for U4. Prior done: U1, U2, U3(all slices), P1, P2. Feature tail (M4/N3/O2) parked.
- ▶ NEXT — **`U4` — engine selector + one *Simulate* section (closes M-UNIFIED-PANEL).** Collapse the 6 sidebar panels into ONE collapsible *Simulate* section with an engine segmented-control/dropdown; the card stack is driven by the **U1 capability descriptor** (`engine_capabilities.js`), unsupported cards **greyed-with-a-why-tooltip** (CHARMM-GUI model), NOT absent. Oracle: selecting engine X shows EXACTLY X's supported cards (from U1) + greys the rest — a **pure selector-state helper** unit-tested + a one-off display-vs-oracle Playwright (rendered card set == descriptor) + an MV row. Reuse: U2's `initForcesCard`, U3's `initJobsPanelBase` (mount per selected engine) + the canonical job-list renderer, `md_engines_logic.js` `ENGINE_ORDER`, `runButtonEnabled`. **`main.js` LOC flat-or-lower** (this is CONSOLIDATION, not growth — cite Δ). **FRONTEND DISCIPLINE (CLAUDE.md):** PARITY/pure-helper oracle in **vitest**; adapted rewires proven via git-stash rerun or in-place-first. `main.js` imports + factory init + thin wiring only. **PARKED:** `P3`/`P4` (job-planner) — resume Track P after U4.
