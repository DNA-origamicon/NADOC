...
**mrDNA forces-UI (M6–M8, added 2026-07-10 — `M-MRDNA-FORCES-UI`; user-authorized, `meta.mrdna_forces_ui_scope`).** The
frontend overhaul left mrDNA's DONE field/anchor backend unreachable (panel sends only `coarse_steps`;
`engine_capabilities.js` greys both). `M6` wire the M1/M2 field+anchors backend into the unified panel (parity) →
`M7` NEW ARBD repulsion-plane hard-surface backend, headless (property oracle, mirrors M1/M2 grid-potential
mechanism + the `field_anchor` surface-opposes-field deposition rule; shared `{dir, offset_nm, stiff}` descriptor) →
`M8` surface UI card (parity). These are reachability/new-feature, **not** part of `M-FULL-COVERAGE` (surface is not
one of the four features; field/anchors are already covered by M1/M2).
...
**M-MRDNA-FORCES-UI** (M6+M7+M8, added 2026-07-10) = mrDNA's field/anchors reachable from the unified panel AND a
new mrDNA hard-surface (repulsion plane) with a UI card + comparable deflection descriptor. Separate from the four
locked features; see the mrDNA-forces-UI narrative above.
...
- ▶ STATE — **`M8` DONE 2026-07-10 → CLOSES `M-MRDNA-FORCES-UI` (M6+M7+M8 all done). mrDNA Hard-surface UI card, frontend-only, PARITY-pinned.** Mounted the SHARED oxDNA floor card on new `mrdna-surface-*` DOM by parameterizing `initOxdnaFloorSetup({ids})` with an ids bag (default = oxDNA ids → the pre-existing oxDNA card test stays green = backward-compat lift; no bespoke card built). `buildMrdnaLaunchBody` threads the card's `{dir,offsetNm,stiff}` into `POST /mrdna/jobs` as cross-engine snake_case `surface:{dir,offset_nm,stiff}` (byte-equal to oxDNA production; null when off/zero-stiff). Deposition exception wired client-side: `fieldNeedsAnchor(on,anchors,fieldSpec,surfaceSpec)` clears when the surface OPPOSES the field, reusing the newly-EXPORTED `surfaceOpposesField` mirror (from `chain_sim_model.js`) of `field_anchor.surface_opposes_field` (·≤−0.906) — no geometry re-derived. `engine_capabilities.mrdna.surface off→on` (`domAnchorId='mrdna-surface-toggle'`, presence enforced in live index.html). ORACLE: `mrdna_jobs_panel.test.js` surface card→body byte-equality + deposition-guard cases; `oxdna_floor_setup.test.js` custom-ids mount; `engine_capabilities.test.js` live-DOM census. Gates: frontend **2627/2627**, smoke **23/23**, `just test-smart`→**FAST** (frontend-only, no Python), ruff clean on touched (23 pre-existing errors in untouched Python = concurrent session). main.js Δ=0. Fresh-context review: no gaps (5/5 oracle reqs verified). display-vs-oracle deferred to **MV-35** (Run gated on real mrDNA/ARBD install, same as M6/MV-34). ⚠ concurrent session pre-dirtied index.html/main.js/spreadsheet/etc. — committed ONLY M8 files + the surface hunk of index.html.
- ▶ NEXT — **`M-MRDNA-FORCES-UI` is CLOSED.** Remaining eligible JSON tasks: **`O2`** (oxDNA residual gaps; deps ∅; low/low) — a NO-OP unless a NAMED gap is logged (extra-base CG render v2 / linker viz), so don't open it blind; and the parked feature tail **`M4`✓(done)** / **`N3`✓(done)** — check the JSON for any still-`pending` feature task before defaulting to a U/P track. If nothing feature-side is actionable, the highest-leverage open milestones are `M-FULL-COVERAGE` (needs a NAMED O2 gap) and the U/P tracks are DONE. Standing follow-up (NOT a JSON task): P2's forces into the production RESEED conf end-to-end (`ProductionRunRequest.field/anchors`+reseed emission).
...
- ▶ ✅ xdist ISOLATION FLAKE — **RESOLVED (confirmed 2026-07-10).** The old non-deterministic 1-test failure (`test_job_archive`/`test_cando_extra_bases`) is fixed by the `_routes_md` snapshot+attribute-repoint fix ([test_md_milestone1.py:487-502](tests/test_md_milestone1.py#L487)). Re-verified clean under the historically-failing configs: full slow-inclusive `-n auto` (4634/0-fail), fast `-n auto`×3, `-n 16`×2, `-n 4`×2, + targeted polluter→victim `-n0` (88 passed) — 0 failures. Don't chase; don't re-add this warning.
