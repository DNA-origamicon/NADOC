...
- ▶ STATE — **`M7` DONE 2026-07-10 → mrDNA gains a hard-surface (repulsion plane), backend-only, HEADLESS.** New `backend/core/mrdna_surface.py`: shared `{dir,offset_nm,stiff}` → a ONE-SIDED HARMONIC WALL `U=½·stiff·min(0,s)²` via ARBD `add_grid_potential`/`gridFile` (M2's mechanism, quadratic); `−∇U` repels along `+dir̂` below / `0` above; plane `offset` below the lowest bead (`plane_c=min_proj−offset`, generalizes oxDNA `wall_position_from_extent`), FIXED in the lab frame (grid written ONCE, regen RE-ATTACHES). **Composes with M2's field by APPENDING** (ARBD superposes gridFile entries) → deposition run carries both; runner installs surface AFTER field. `MrdnaJob.surface`+route+guards; **deposition rule** (`field_anchor.surface_opposes_field`) relaxes the field-needs-anchor guard in BOTH route + runner. ORACLE `tests/test_mrdna_surface.py` **11 fast + 1 slow**, all RED-able; SLOW ran+passed on GPU: free run streams the lowest bead to **s=−32 Å**, surface DEPOSITS it at **s=+3.9 Å** (36 Å hold, no anchor). Fresh-context review caught the SLOW oracle mis-calibration (field never reached the plane) + untested regen-ordering — BOTH fixed + re-validated on hardware (root cause: test installed surface-before-field, clobbering the wall). Gates: 11/11 fast, `just test-smart`→FULL **4463/280/1** (watermark bumped, no drop), ruff clean on touched (23 pre-existing errors in untouched files = concurrent session). main.js Δ=0; backend-only → display-vs-oracle N/A. ⚠ concurrent session's crossover/spreadsheet edits pre-dirty (index.html/main.js/etc.) — commit ONLY M7 files.
- ▶ NEXT — **`M-MRDNA-FORCES-UI` continues: M8 last** (deps=M7✓, low). M8 = surface UI card — generalize/mount the oxDNA floor card's `{dir,offset_nm,stiff}` descriptor for mrDNA, thread `surface` into the `POST /mrdna/jobs` launch body (the pure `buildMrdnaLaunchBody` in `mrdna_jobs_panel.js` already threads field+anchors — add surface the same way), flip `engine_capabilities.mrdna.surface off→on` with a real `domAnchorId` present in index.html. PARITY oracle mirroring M6 (`mrdna_jobs_panel.test.js`: card payload byte-equals `body.surface`; census stays green) + one-off display-vs-oracle + a new MV row. Closing M8 CLOSES `M-MRDNA-FORCES-UI`. Also still open: **`O2`** (oxDNA residual gaps; deps ∅; low/low) — no-op unless a NAMED gap is logged. Standing follow-up (NOT a JSON task): P2's forces into the production RESEED conf end-to-end (`ProductionRunRequest.field/anchors`+reseed emission).
