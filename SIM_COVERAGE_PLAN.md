...
- ▶ STATE — **M-METRIC-CORE DONE** (S1–S5). **`O1` DONE** (2026-07-06): first LIVE card column. `backend/core/oxdna_shape_source.py::build_oxdna_shape_source(shape_frame, core_reference, rmsf_positions?, field?)` (pure: core-filter relaxed frame → `compute_shape_descriptors` + `production_rmsf`→`rmsf_nm` profile) + route `GET /oxdna/jobs/{id}/shape-source` (routes_oxdna.py) + `getSources` in `oxdna_jobs_panel.js` now fetches the selected job's bundle (`main.js` Δ=0). 7/7 oracle `tests/test_oxdna_shape_source.py`; `just test` 4177; vitest 2200. Descriptors = oxDNA ABSOLUTE twist (cross-engine-comparable), NOT the differential twist the Graphs-&-Metrics card plots. Field=None (deferred). Live eyeball = **MV-21**.
- ▶ NEXT — **`C1`→`C2`** (CanDo anchors→E-field) is the **M-CANDO-FIELD headline**: does the cheap FEM predict oxDNA's field deflection? C1 (Dirichlet BC, low, deps met) then C2 (q·E nodal load, deps C1). Seams: `apply_boundary_conditions` (fem_solver.py:552) generalize for anchors; `assemble_prestress_force` (fem_solver.py:455) add q·E; `predict_shape` (fem_solver.py:1071) gains `anchors=`/`field=`. **`C5`** then adds CanDo as the 2nd card source (RMSF ref) → first real oxDNA-vs-CanDo agreement. `N1`/`N2` (NAMD eField/anchors, low) also pickable.
