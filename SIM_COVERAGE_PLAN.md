...
- ▶ STATE — **`P4` DONE 2026-07-08 → CLOSES M-JOB-PLANNER (P1+P2+P3+P4) + M-DEPOSITION-CHAIN (P1+P2+P4+U2).** The "Plan Run" overlay (`⛓ Plan Run…` in the NAMD MD panel) authors a linear stage list + queues it as one `MdPipeline` that runs unattended. Load-bearing logic = PURE `frontend/src/ui/stage_planner_model.js` (immutable add/⧉dup/↑↓reorder/✕remove/setStage/setRoot + `buildChainPayload`→`CreateChainRequest` JSON + `chainStatusSummary`→"stage N of M"/queued-behind/resumable). `md_plan_run.js` = thin `createModal` glue REUSING the shared U2 Forces card (`initForcesCard` namd, private `plan-efield-*` ids) + Anchors card (`initOxdnaAnchorsSetup`, `plan-anchors-*` ids) for the ACTIVE stage. Stage engine FIXED 'namd' (executor spawns only NAMD; cross-engine = the ROOT hop, model/payload support a CG root). `client.js` +4 chain helpers; `md_jobs_panel.js` +`return {getSelectedJob}` (also activates N4's `getMdJob` wiring); main.js Δ=+10 pure wiring. BRIGHT LINE (CHAIN) proven by a TWO-HALF parity oracle: `stage_planner_model.test.js` (12) builds a deposition→immobilize→sweep payload BYTE-EQUAL to `tests/test_stage_planner_payload.py` (4), which parses through `CreateChainRequest` + `build_pipeline_plan`→linear chain (stage N from N-1). Gates: 12 vitest + 4 pytest; `just test` **4427/110/1** (no failures); `just test-frontend` **2415**; lint clean on touched; smoke 23/23. One-off Playwright asserted `POST /md/chains` body == the authored payload (deleted). **MV-32 owed** (live anchor-chip-in-stage gesture + a real unattended NAMD chain run). Gotchas logged: raw `npx vitest` grabs a broken global 4.1.10 (use `just test-frontend`); real `/md/jobs` is a BARE array (stub shape). **U/P tracks now fully done (U1–U4, P1–P4).**
- ▶ NEXT — **The U/P priority is EXHAUSTED; the parked feature tail resumes toward M-FULL-COVERAGE.** Eligible pending = **N3** (namd extra-base/linker validation coverage + shared descriptors from MD frames; deps []; low effort, medium leverage — closes NAMD's feature column), **M4** (mrdna linkers into the ARBD model; deps M3✓; medium), **O2** (oxdna residual gaps; deps []; low/low — pull only when a concrete gap is logged). Rubric tie-break (lowest effort, real coverage gap) → **prefer `N3`**. All three feed M-FULL-COVERAGE (the only pending milestone). Oracle for N3 = FAST: atomistic model includes insert/linker atoms + shared descriptors computable from an MD frame (property, not "it ran"). Note: P2's documented follow-up (forces into the production RESEED conf end-to-end — `ProductionRunRequest.field/anchors`+reseed emission) is still open but is NOT a JSON task; the chain threads forces at the conf-snippet level today.
