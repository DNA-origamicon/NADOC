...
- ▶ STATE — **M-METRIC-CORE DONE** (S1–S5); **M-CANDO-FIELD DONE** (C1,C2,S4,S5,O1); **M-CANDO-COMPLETE DONE** (C1–C5). **`N2` NAMD anchors DONE (2026-07-07)** — anchors ride NAMD **`fixedAtoms`** (Dirichlet hold; NOT the ramped `conskfile` — NAMD allows only one, spent on the slow-release restraint), so they persist immobile across the ladder. `namd_topology.resolve_anchor_residue_indices` reuses the SHARED `resolve_anchor_particles` scope resolver → `(helix,bp,dir)` keys → built-PDB residue ORDINALS (via `Atom` provenance); `md_protocols.write_anchor_restraints_pdb` marks B=1 on exactly those residues; `_segment_conf`/`_min_conf` emit `fixedAtoms on/File/Col B`; `prepare_mgh_slow_release(anchors=)` writes `restraints_anchors.pdb` + wires every conf + manifest; `CreateJobRequest.anchors`; MD-panel Anchors picker (2nd `initOxdnaAnchorsSetup`, main.js +1). **REVIEW-CAUGHT HIGH (fixed): a NAMD-positional mark must mirror WHICH generator built the package PDB** — `export_pdb` (mgh_slow_release default) uses NATURAL chain order, psfgen (equilibrium-aware) SORTS; they diverge past 26 strands → wrong order silently pins offset residues. `built_pdb_residue_keys(sort_chains=)`/`resolve_anchor_residue_indices(full_topology=)` selects the match, proven on 176-strand `make_18hb_routed_design`. Oracle `tests/test_namd_anchors.py` 11 (9 fast + 2 slow). `just test` 4226 (+11, no drop); ruff clean (removed 1 dead import); vitest 2214; smoke render gate green (2 pre-existing env failures reproduce on master); display-vs-oracle N/A (input picker, not a display); live picker gesture → **MV-NAMD-ANCHORS**.
- ▶ NEXT — pick by rubric among eligible (all deps met): **recommend `N1`** (NAMD native `eField` conf block + Dynamics UI) — anchors-before-field is satisfied (N2 done), and N1 **reuses N2's `fixedAtoms` plumbing** for the anchored field run → completes NAMD's anchor+field pair (one engine at **M-ALL-ANCHORS-FIELD**; that milestone needs C1,C2✓,N1,N2✓,M1,M2). Then mrDNA `M1`(anchors, reuse the SAME `resolve_anchor_particles`→bead-index pattern N2/C1 used) → `M2`(field). Card-source tasks `M5`/`N4` (deps=S5 ✅) reuse the SOURCE-BUNDLE CONTRACT below. **For any linker work in another engine (M4/N3), reuse C4's pattern.** **For any ANCHOR work (M1), reuse N2's pattern:** shared scope resolver → per-nt keys → engine bead/atom indices; a positional mark must mirror the exact generator's residue order.
