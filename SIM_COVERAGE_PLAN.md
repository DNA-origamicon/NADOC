...
- ▶ STATE — **`U2` DONE 2026-07-08** (shared Forces/E-field card factory). `frontend/src/ui/forces_card.js` `initForcesCard({engine,ids?,gizmo?,getBaseCount?,getAnchorCount?,onChange?})`→`{getFieldSpec,isEnabled,refresh,applyConfig,detachGizmo}` collapses the 3 triplicated field cards: DELETED `efield_setup.js` (oxDNA gizmo) + `cando_efield_setup.js` (CanDo+NAMD numeric); the LAMMPS field third of `lammps_forces_setup.js` now DELEGATES to it (Anchors+Surface + public API unchanged). Rewired: `main.js` (oxdna+gizmo), `cando_jobs_panel.js` (cando), `md_jobs_panel.js` (namd). Per-engine divergences = DATA (`FORCES_FIELD_VARIANTS`/`FORCES_FIELD_IDS`): gizmo-vs-numeric, V/m via DOM presence, +y vs +x dir, ready-line style, gizmo-gate, job-arrow. PARITY oracle 14 FAST = ADAPTED-CODE PIN (drove LIVE old + new on fresh DOMs → byte-equal payloads, 13/13 while both existed) + LAMMPS's 9 PRE-EXISTING tests still pass. Review caught + FIXED one real regression (LAMMPS gizmo lingered cross-tab when open+enabled) w/ RED-verified test. Gates: 14/14, frontend 182/2316, smoke 23/23, ruff N/A. main.js LOC-Δ=+1. Live 4-panel gesture (esp. gizmo drag) owes **MV-22**. Also DONE: U1, P1, P2 (all 2026-07-08). Feature tail (M4/N3/O2) parked.
- ▶ NEXT — **stay on Track U (user directive: unified UI before Track P).** Order = **`U3` → `U4`** (both frontend, deps met: U3 dep U1✓; U4 dep U2✓+U3). **`U3` FIRST** = shared jobs-panel base: extract the common run-buttons + `job_tree` job-list + Advanced-card host out of the 5 bespoke `*_jobs_panel.js` into `initJobsPanelBase({engine,descriptor,deps})`, each engine supplying its U1 descriptor; reuse the already-shared `job_tree.js`/`metrics_card.js`/`shape_compare` — PARITY = the base renders each engine's job list + run controls identically to its bespoke panel (job-row model, run-button enablement). Then `U4` (engine selector + one *Simulate* section, unsupported cards greyed-with-tooltip per U1). **FRONTEND DISCIPLINE (CLAUDE.md):** per-engine PARITY in **vitest pure helpers**; **ADAPTED-CODE PIN** — prove each moved helper against the LIVE old code (drive old+new on fresh DOMs, or in-place-first), NOT green-first-run (do **NOT** use `git stash` for this — it reverts the whole working tree; compare against the still-present old module or `git show HEAD:` instead); stateful → **app exercise** + `just smoke`; a new/changed card → one-off **display-vs-oracle Playwright** (deleted after) + an **MV row**. `main.js` imports + factory init + thin wiring only (LOC flat-or-lower). **PARKED:** `P3`/`P4` (job-planner) + P2 follow-ups (forces into the production *reseed* conf + live 2-stage-chain MV) — resume Track P after U4 lands.
