...
- ▶ STATE — **M-METRIC-CORE DONE** (S1–S5); **M-CANDO-FIELD DONE** (C1,C2,S4,S5,O1). **`C3` DONE (2026-07-06) → M-CANDO-COMPLETE now only `C4` (linkers) left** (C1,C2,C3,C5 ✅). C3 finding: the extra-base **compliant-connector mechanism already existed** (`build_fem_mesh` meshes an extra-base crossover as a 2-node WLC ssDNA spring `k_trans=3·kT/(2·L_c·L_p)`, `k_rot=0`, NOT a rigid link — CONN3D2 analogue, shipped untested Phase-5 `9121b78`); C3 = the missing property **oracle** `tests/test_cando_extra_bases.py` (3 FAST mesh-census/monotone/2-node-compliance + 1 SLOW real-`predict_shape` local-RMSF ~1.87× up w/ self-vs-self RED-guard). **Asserts NO twist/bend direction** — softening a distributed load is non-monotone (crossover-geometry reasoning forbidden); RMSF/flexibility is the sign-safe channel + CanDo's observable. `just test` 4210 (+4, no drop); ruff clean; review no gaps; main.js Δ=0; no card→display-vs-oracle N/A.
- ▶ NEXT — pick by rubric. **Recommend `C4`** (linkers/overhang connections as FJC connector elements in `build_fem_mesh` from `overhang_connections`/extensions — deps=C3 ✅ now met; **CLOSES M-CANDO-COMPLETE**, the last CanDo task; reuse C3's WLC-spring connector pattern). Oracle (per JSON): two parts joined by a linker element show COUPLED response under a test load on one part (the other moves); no linker → decoupled — a clean compliance/coupling property, same style as C3's 2-node test, **no geometric reasoning**. Alternatives: NAMD `N1`/`N2` (anchors-before-field, low-effort, no deps, unblock **M-ALL-ANCHORS-FIELD**), or `M5`/`N4` (mrDNA/NAMD card sources, deps=S5 ✅, reuse the SOURCE-BUNDLE CONTRACT below).
