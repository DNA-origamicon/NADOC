...
- ▶ STATE — **`M4` DONE 2026-07-08 → mrDNA feature column complete; only O2 left for M-FULL-COVERAGE.** VERIFY task (like M3): `connect_overhangs` materializes REAL topology (linker complement strand(s) that hybridize each overhang into local duplex + a virtual `__lnk__` bridge helix), and the mrDNA bridge `_build_nt_arrays` ALREADY threads it (iterates `design.strands` + builds `helix_geom` from `design.helices` — the `__lnk__` helix + linker strands flow with NO code change). Gap = the missing property oracle. `tests/test_mrdna_linkers.py` **10 fast + 2 slow**: FAST arrays — ds bridge nts ALL paired (rigid duplex, `2·L`), ss bridge ALL unpaired (flexible tether, `L`) threaded in-chain prev(A)→bridge…→next(B), no-linker→no `__lnk__`; FAST built SegmentModel (THE bright line) — same two overhangs linked once ds once ss → `ds.ds_nt−ss.ds_nt == L` AND `ss.ss_nt−ds.ss_nt == L` for L∈{4,6,8} (ds bridge = L rigid bp, ss = L flexible nt — mrDNA agrees engine-level with C4's rigid-duplex-vs-WLC-tether). Isolates the bridge because `_make_complement_domain` is attach-INDEPENDENT (both types hybridize identical overhang nts; only ds `free_end/free_end` vs ss `root/free_end` WC-polarity differs); bridge beads scale with bridge length; SLOW real ARBD (ran+passed 15 s) — bridge `__lnk__` positions survive into the display frame, absent unlinked (presence-after-run, bridge-specific; NOT a Brownian co-motion correlation — FAST carries the mechanical prediction). No production change; Three-Layer clean (solver only READS the generated topology). Review CONFIRMED FAST pins sound + bridge-drop-sensitive; the two weaker "adds-particles"/SLOW-`n_beads` asserts it flagged as complement-satisfiable were tightened (bridge-length-scaling + `__lnk__`-presence). Gates: oracle 10 fast + 2 slow green, full `just test` **4447/110/1** (baseline 4435, +12 incl the 2 real-ARBD, no drop), ruff clean; backend-only → no vitest/smoke; main.js Δ=0. **Live mrDNA linker eyeball folds into MV-21.**
- ▶ NEXT — **Only `O2` remains for M-FULL-COVERAGE (the last pending feature milestone).** Eligible pending = **O2** (oxDNA residual gaps; deps ∅; low/low). O2 is a **no-op unless a concrete gap is logged** (extra-base CG render v2 / a residual oxDNA task surfaced in `project_oxdna_extra_bases`/issues) — before picking it, grep the oxDNA topic files + `issues_ledger.md` for a NAMED gap; if none is concrete, **ask the user what O2 should cover** rather than manufacture work, or close M-FULL-COVERAGE by marking O2 done-as-no-residual-gap with the user's OK. All U/P tracks are already DONE (M-UNIFIED-PANEL + M-JOB-PLANNER + M-DEPOSITION-CHAIN closed). Standing follow-up (NOT a JSON task): P2's forces into the production RESEED conf end-to-end (`ProductionRunRequest.field/anchors`+reseed emission) — the chain threads forces at the conf-snippet level today.
