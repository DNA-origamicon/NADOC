...
- ▶ STATE — **`P3` DONE 2026-07-08 (cross-engine output→input; M-JOB-PLANNER now needs only P4).** The oxDNA/mrDNA→NAMD coordinate+unit conversion ALREADY EXISTED + was validated (`oxdna_runner.build_namd_seed`/`mrdna_runner.build_namd_seed_from_mrdna` → `build_atomistic_model`, wired into `create_md_job` via `oxdna_job_id`/`mrdna_job_id`; sim-units/Å→nm inside). P3 was ROUTING the chain's `cross_engine` hop through it, not new conversion. THREE additive changes: (1) pure `cross_engine_seed(plan, resolved_parent)→CrossEngineSeed|None` in `md_pipeline.py` (table `{oxdna:'oxdna_job_id',mrdna:'mrdna_job_id'}`; same-engine→None=checkpoint restart; raises on unsupported sink / unknown source / unresolved parent); (2) `_chain_spawn` branches — cross-engine→`create_md_job(oxdna_job_id|mrdna_job_id=parent, field/anchors from stage forces)`, same-engine→unchanged `spawn_md_production`; (3) `create_md_chain` accepts a CG root, validated via `assert_namd_seed_available`/`assert_mrdna_namd_seed_available`. REVIEW-CAUGHT BUG fixed: stage protocol defaults to `"production"` but `create_md_job` rejects any protocol ∉ SUPPORTED_PROTOCOLS → mapped to `EQUILIBRIUM_AWARE_PROTOCOL` for the create path (RED-verified). Oracle 13 FAST (6 pure `test_md_job_pipeline.py` + 7 route `TestMdCrossEngineChain` incl. E2E oxDNA→NAMD chain + halt/resume). Gates: `just test` **4421/110/1** first run + rerun after the protocol fix; ruff clean; `main.js` Δ=0; backend-only → display-vs-oracle N/A. **MV-29 owed** (real oxDNA→NAMD handoff needs a live oxDNA job + NAMD prep — like P2's owed real chain). Realizable hop = CG ROOT → NAMD stage 0 (executor spawns only NAMD; mid-chain CG stage would need a CG spawn adapter — out of scope). Prior done: U1–U4, P1, P2. Feature tail (M4/N3/O2) parked.
- ▶ NEXT — **`P4` Planner UI (linear stage-builder overlay) → CLOSES M-JOB-PLANNER + M-DEPOSITION-CHAIN.** The "Plan Run" popup: an ordered stage list, per-stage engine+protocol+**Forces editor (reuse U2's `initForcesCard`)**, duplicate-stage for a field sweep, Queue; status vocabulary (stage N of M, queued-behind, partial-failure). Oracle (frontend): PURE stage-list model helpers (add/dup/reorder/remove, payload build) unit-tested in vitest + **the built payload == a valid `MdPipeline` (P1)** + one-off display-vs-oracle Playwright (queued chain == payload, then delete + MV row). Deps P2+P3+U2 all done. Module-first (`initX({deps})→{api}`); `main.js` gains only imports+factory init+thin wiring (cite LOC Δ). Adapted frontend rewires proven via git-stash rerun, not green-first-run. Backend already exposes `POST /md/chains` (build+persist+kick) + `/resume` + `GET /md/chains[/{id}]`; the UI just authors the `CreateChainRequest` (a CG-root cross-engine plan sets `root_engine:'oxdna'|'mrdna'`).
