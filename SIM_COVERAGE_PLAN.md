...
- ▶ STATE — **M-METRIC-CORE DONE** (S1–S5). **`O1` DONE** (oxDNA live card column). **`C1` DONE** (2026-07-06): CanDo FEM **anchors** (Dirichlet BC). `fem_solver.py`: `apply_boundary_conditions(K,f,mesh,fixed_nodes=None)` pins the given nodes' 6 DOF (`None`/`[]`→centroid, never singular); `solve_prestress_shape(...,fixed_nodes=)` clamps them each load step; new `resolve_anchor_nodes(design,mesh,anchors)` reuses `oxdna_interface.resolve_anchor_particles` (overhang/cluster/domain/strand/base)→duplex-core node indices; `predict_shape(design,*,anchors=None)` threads both paths + returns `anchor_keys`. Anchors are a job-request annotation, NOT a topology edit. 10/10 oracle `tests/test_cando_anchors.py`; `just test` 4186; `main.js` Δ=0. RMSF stays free-free.
- ▶ NEXT — **`C2`** (CanDo E-field, deps=C1 ✅ now met) is the **M-CANDO-FIELD headline** and the ONLY task left for that milestone: does the cheap FEM predict oxDNA's field deflection? Add uniform q·E per-node nodal load to `assemble_prestress_force` (fem_solver.py:455) + `predict_shape(design, field=..., anchors=...)`; nonlinear solve for large deflection. **Oracle = shared `measure_field_response`/S4 on the FEM deflected frame** (anchor holds, free deflects along field, monotone in |E|, zero-field→no deflection). Anchors already resolve via `resolve_anchor_nodes` (C1). Then **`C5`** = CanDo as the 2nd card source (RMSF ref)→first real oxDNA-vs-CanDo agreement. `N1`/`N2` (NAMD eField/anchors, low) also pickable.
