...
- ▶ STATE — **`U3` SLICE 2c-2 DONE 2026-07-08 (U3 still `in_progress`).** Converged the **LAMMPS** panel scaffold onto `initJobsPanelBase` — the FIRST live consumer of `arrowStyle:'class'` (section arrow via the `is-collapsed` class) and the first `onClose` cleanup (`_viewsOff()`+`forcesSetup.detachGizmo()`). Dropped the `section_collapse_state` import + `_pollTimer` + the `_applyCollapsed`/heading/`advToggle`/`_schedulePoll`/`_clearPoll` blocks (net −9 LOC, 334→325); rewired `_schedulePoll`→`_base.schedulePoll`, mount→`_base.initCollapsed(true)`, `body.style.display` reads→`_base.isOpen()`. The base adds the open-guard the bespoke LAMMPS poll lacked = the ONE *adapted* delta (incidental hardening; viz-card collapse stays bespoke). Oracle `lammps_jobs_panel.test.js` **+3** (19/19): a PARITY block drove the REAL `initLammpsJobsPanel` through the actual collapse / adv-drawer / open+active-poll gestures — run GREEN pre-rewire FIRST then post = adapted-code pin, not green-by-construction. Fresh-context review: **all 6 parity checks CONFIRMED**, open-guard is a fix not a regression. Gates: LAMMPS 38/38, frontend **2359/2359** (+3 no drop), smoke 23/23, one-off Playwright drove the real LAMMPS collapse+adv in-app (0 console errors, deleted), no Python, lint N/A. `main.js` LOC-Δ=0. **No MV row owed** (behaviour-preserving, byte-identical DOM). Prior done: U1, U2, U3-s1/s2a/s2b/s2c-1, P1, P2. Feature tail (M4/N3/O2) parked.
- ▶ NEXT — **`U3` slice 2c-3 (converge oxDNA + md scaffold), then finish `U3`→`U4`.** **2c-3 — converge oxDNA/md onto `initJobsPanelBase`.** Both use a **module-level `_collapsed` var + a header-toggle** (not a `_applyCollapsed` fn) read in ~6 sites → converging means replacing those reads with `_base.isOpen()` (wider *adapted* change; prove in-place-first or via git-stash rerun, NOT green-first-run). **CORRECTION to the old handoff:** oxDNA's SECTION arrow is actually `arrowStyle:'class'` (`arrow.classList.toggle('is-collapsed',…)`, like LAMMPS) — its `'rotate'` (`style.transform`) is only the ADVANCED-drawer arrow (`advArrowStyle:'rotate'`). **md** adds a 2nd remote `setInterval` poll (`_remotePollTimer`) + cross-panel md-collapse coordination (it force-collapses oxDNA when md opens) — these stay bespoke around the base (the base owns only the primary poll + section collapse). Then **`U4`** (engine selector + one *Simulate* section, unsupported cards greyed-with-tooltip per U1; dep U2✓+U3). **FRONTEND DISCIPLINE (CLAUDE.md):** PARITY in **vitest** driving the real panel; byte-identical rewires proven OLD-then-NEW → equal; any *adapted* rewire (the `_collapsed`→`isOpen()` swap) proven via git-stash rerun or in-place-first. `main.js` imports + factory init + thin wiring only. **PARKED:** `P3`/`P4` (job-planner) — resume Track P after U4.
