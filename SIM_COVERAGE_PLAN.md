...
- ▶ STATE — **`S1`+`S2` DONE**: `shape_metrics.py` now has `compute_shape_descriptors` (S1) + `deviation_profile`/`rmsf_from_ensemble`/`normalize_rmsf_profile` (S2, 2026-07-06, 10/10 fast oracle `tests/test_shape_deviation_rmsf.py`). `M-METRIC-CORE` pending (S3–S5 remain).
- ▶ NEXT — **`S3`** (`compare_descriptors(cand, ref)` — per-observable reference selection + agreement math: scalar signed %Δ, RMSF Pearson/Spearman, aligned-shape RMSD via `_rigid_superpose`; NAMD-override-when-present). **Deps S1+S2 now both met** → this is the critical path to S5/M-METRIC-CORE. Then `S4` (needs S1, low) → `S5` card (needs S3) = closes M-METRIC-CORE.
- ▶ ALSO PICKABLE NOW — `S4` (low, only needs S1; generalize `oxdna_health.measure_field_response` into shape_metrics), `C1` (CanDo anchors, low), `N1` (NAMD eField, low), `M3`, `O1` (oxDNA reference column, low). `C1`→`C2` is the fast path to **M-CANDO-FIELD** once S4/S5 exist.
- ▶ REUSE — S3 consumes S1's `compute_shape_descriptors(positions)` dict + S2's `deviation_profile`/`rmsf_from_ensemble` outputs. For RMSF agreement: CanDo gives NMA rmsf directly (`predict_shape["rmsf"]`), others via `rmsf_from_ensemble`; both feed `normalize_rmsf_profile`. Aligned-shape RMSD: reuse `fem_solver._rigid_superpose` or `oxdna_health._kabsch_superpose`. Caller core-filters real ssDNA-ended frames.
- ▶ GOTCHAS banked — deviation cross-engine = **align=True** (Kabsch strips pose); RMSF trajectory mode = align=True (strips box drift), align=False for a common-frame/tiny bundle. Uniform field needs ≥1 anchor (COM-drift) in every engine; CanDo bend/twist exp36-calibrated (don't regress); every output Physical-layer only; repo has unrelated pre-existing ruff debt (19 errors in other test files) — not a regression, don't sweep (`feedback_no_bulk_reformat`).
