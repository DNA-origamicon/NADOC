...
- ▶ STATE — **`U4` DONE 2026-07-08 → CLOSES `M-UNIFIED-PANEL` (U1+U2+U3+U4).** New `engine_selector.js`: pure `panelVisibility`/`selectedEngineCards` (derived from the U1 descriptor) + `initEngineSelector({selectorMount,stripMount,panelEls,onSelect})→{select,getSelected}`. A new `#simulate-panel` Dynamics-tab section = a 5-button segmented control fronting the 5 stacked engine panels: `select(x)` shows EXACTLY x's panel (`display:''`), hides the other 4, and renders a capability strip of all 8 cards — supported=blue chip, unsupported=**greyed chip with the U1 reason as a `title` tooltip (present, not absent)**. Module owns only whole-panel `display` (verified the 5 panels toggle only their own body-collapse, never whole-panel display → no fight). Oracle `engine_selector.test.js` **11/11**: pure-state (enabled subset === U1 `enabledCardKeys`, greyed === unsupported w/ `cardReason`) + jsdom factory (5 stub panels: show-one/hide-four, no-op bad select, chip census, onSelect, click) — NEW logic ⇒ green-first-run valid. Gates: frontend **2376/2376** (+11 no drop), smoke 23/23, `main.js` **Δ=+16 pure wiring** (import + factory init w/ 5 element lookups, NO cohesive logic; loop-routing the 5 panel inits to offset deferred as out-of-scope). `just lint` fails only on 19 pre-existing Python ruff errors (none in my files; no `.py` touched). One-off Playwright drove real-app DOM (5 buttons, per-engine show/hide, chip set==descriptor, greyed `title`s) PASSED but ran vs the welcome overlay (design-load didn't reach the view) → spec deleted, **MV-28 owed** (live-with-design gesture + hover-tooltip). Prior done: U1, U2, U3(all), P1, P2. Feature tail (M4/N3/O2) parked.
- ▶ NEXT — **Track P resumes: `P3` cross-engine output→input (toward M-JOB-PLANNER).** Wire an oxDNA relax's final coordinates into a NAMD production stage's seed conf with the RIGHT coordinate convention (the `cross_engine` hop P1 already flags in `MdPipeline`). Oracle = a **CHAIN** pin: a NAMD stage runs *seeded from the previous oxDNA stage's output* (coordinates land in the reseed conf, correct units/frame); on failure the chain *halts + resumes from the failed stage*. Reuse: P1's `md_pipeline.py` (`cross_engine` stage flag), P2's chain executor (submit-N+1-on-N + resume-from-failed), the existing `seed_oxdna_job_id` plumbing. Then `P4` = linear stage-builder overlay (reuses U2's `initForcesCard` per stage) closes **M-DEPOSITION-CHAIN**. Backend service in `backend/core` (imports nothing from `backend/api`); if it touches `main.js` keep it imports+factory init+thin wiring. **FRONTEND (if any UI): PARITY/pure-helper oracle in vitest; adapted rewires proven via git-stash rerun.**
