...
- ▶ STATE — **M-METRIC-CORE DONE** (S1–S5). **`O1` DONE** (oxDNA live card column). **`C1`+`C2` DONE → M-CANDO-FIELD CLOSED** (2026-07-06). `C2` = CanDo FEM **uniform E-field**. `fem_solver.py`: new `assemble_field_force(mesh, field)` builds the nodal body load from the shared oxDNA descriptor `{field_pN, dir}` (per-NUCLEOTIDE force; each duplex node = `FEM_FIELD_CHARGES_PER_NODE=2` backbones → `2·field_pN·dir_hat`, translational only); DEAD load (assembled once in global coords, NOT co-rotated) added to the eigenstrain each corotational step. Threaded through `solve_prestress_shape(...,field=)` + `predict_shape(design,*,anchors=,field=)`; reuses C1 `resolve_anchor_nodes` (a field needs ≥1 anchor: COM-drift). Field is a job-request annotation, NOT a topology edit. Oracle `tests/test_cando_field.py` 7 (3 fast unit + 4 SLOW nonlinear): anchors held + free deflects along field + monotone in |E| + zero-field→none, scored by shared **S4** on the RAW clamped-solve frame. `just test` 4194; `main.js` Δ=0.
- ▶ NEXT — **`C5`** (CanDo as the 2nd card source, deps=S5 ✅) is the highest-leverage pick: emit CanDo's shared descriptors + register it as a compare-source → the **first real oxDNA-vs-CanDo agreement number** (RMSF reference column + the field-deflection agreement C2 set up). **CRITICAL for C5's field emission: build the field-response from the RAW `solve_prestress_shape(field=)` frame, NOT `predict_shape`'s display positions** — the latter are per-frame Kabsch-reposed so a genuinely-held anchor drifts ~5nm (C2 lesson; the raw clamped frame is the common frame S4 needs). Follow the O1 SOURCE-BUNDLE CONTRACT (core-filter first, ABSOLUTE descriptors). Alternatives: `N1`/`N2` (NAMD eField/anchors, low, unblock M-ALL-ANCHORS-FIELD) or `C3` (extra-bases, no deps).
- ▶ SOURCE-BUNDLE CONTRACT (proven by O1; reuse for C5/M5/N4) — each engine builds `{engine:"<lower>", descriptors:compute_shape_descriptors(core_frame), rmsf:[{helix_id,bp_index,direction,copy,rmsf_nm}], shape_frame:core_frame, field:field_response_profile(...)|None}` and returns it from `getSources`. **Core-filter first** (`_filter_to_reference_core(frame, core_reference_geometry(design))`) so ssDNA ends don't skew twist/bend. Emit the engine's own ABSOLUTE descriptors (not a differential-vs-its-analytic). `build_comparison_report` + the card do the rest.
...
- ▶ GOTCHAS banked — **"match existing values" is ambiguous: pin the ESTIMATOR not the UI graph** (O1: absolute vs differential twist — same `measure_bundle_twist`, different reference+frame; LESSONS-from-O1). **cross-engine RMSF collapses to per-bp before matching** (direction-keyed→silent None; S3). field-response NOT Kabsch-aligned (anchored region = common frame). Uniform field needs ≥1 anchor (COM-drift) in every engine; CanDo bend/twist exp36-calibrated (don't regress); every output Physical-layer only; pre-existing ruff debt (19 errors in OTHER test files) — don't sweep (`feedback_no_bulk_reformat`).
