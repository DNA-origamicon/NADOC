...
- ▶ STATE — **`U3` SLICE 2b DONE 2026-07-08 (U3 still `in_progress`).** Converged the 2882-line **NAMD (md)** job LIST onto the canonical `jobs_panel_model.js`/`jobs_panel_render.js` → the renderer now serves **5/5** panels. Renderer gained **THREE generic gated slots**, each null/empty unless the ctx opts in (so **oxDNA byte-parity UNCHANGED**, pin still 14/14): `chevron` (leading expand/collapse → `onChevron`, stopPropagation), `postLabelMarkers(job,{childCount,collapsed})` (spans after label), `symbolOverride(job)` (replaces the status glyph, carries a `dataset`). md's tree/chevron/ensemble-summary/seed+Alpine badges/⧗-remote-queued+`[data-md-queued]`/Fix-button all ride these + the existing `action` slot; row ctx EXTRACTED to a pure module-scope `mdJobRowCtx` factory + shared `mdJobRowSig`; bespoke `_jobRow` DELETED (−104 LOC). Oracle `md_jobs_panel.test.js` **+9** (payload parity). Fresh-context review caught a **HIGH regression** (Fix button un-wired — `onAction` omitted); fixed + pinned. Gates: jobs-panel files 238/238, frontend **2339/2339** (+9 no drop), smoke 23/23, lint N/A. `main.js` LOC-Δ=0. Live NAMD-row pixels owe **MV-25**. Prior done: U1, U2, U3-s1, U3-s2a, P1, P2. Feature tail (M4/N3/O2) parked.
- ▶ NEXT — **finish `U3`: slice 2c (stateful base), then `U4`.** **2c — factor** the shared run-buttons + poll loop + section-collapse + advanced-drawer host into a stateful `initJobsPanelBase({engine,descriptor,deps})` each engine drives with its U1 descriptor (`runButtonEnabled` is built+tested but NO panel consumes it yet; the 5 panels now share the LIST renderer + the `_rowCtx`/`_listSig`/`_legend`/`_renderList` SHAPE — factor that repeated scaffold next). Then **`U4`** (engine selector + one *Simulate* section, unsupported cards greyed-with-tooltip per U1; dep U2✓+U3). **FRONTEND DISCIPLINE (CLAUDE.md):** PARITY in **vitest pure helpers**; oxDNA rewires stay byte-identical (drive OLD verbatim-copy + NEW on fresh DOMs → byte-equal, NOT green-first-run, no `git stash`); mrDNA/cando/lammps/md convergence = intentional upgrade (conformance test + `just smoke` + an MV row). `main.js` imports + factory init + thin wiring only. **PARKED:** `P3`/`P4` (job-planner) — resume Track P after U4.
