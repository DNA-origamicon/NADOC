...
- ▶ STATE — **M-METRIC-CORE DONE** (S1–S5); **M-CANDO-FIELD DONE**; **M-CANDO-COMPLETE DONE** (C1–C5); **NAMD anchor+field pair DONE** (N1✓+N2✓); mrDNA anchors (M1✓); **`M2` mrDNA E-field DONE 2026-07-08 → M-ALL-ANCHORS-FIELD COMPLETE** (all three job engines — CanDo C1/C2, NAMD N1/N2, mrDNA M1/M2 — run an anchored E-field job with a comparable along-field deflection descriptor). M2: `backend/core/mrdna_field.py` turns the SHARED `{field_pN (per NUCLEOTIDE), dir}` descriptor into a constant per-bead force = `field_pN·(nt in bead)·(pN→kcal/mol/Å)·dir̂`, `nt in bead = bead_mass/dpn`, `dpn = total_bead_mass/total_nt` (**DNA beads carry charge 0 → apply force directly, NOT q·E**), so TOTAL force = `field_pN·total_nt` exactly. **KEYSTONE: the constant force is a RAMP POTENTIAL `U=-(F·r)` via `add_grid_potential`/`gridFile` (`-∇U=F`); the `forceXGrid` tabulated-force path CRASHES ARBD (CUDA illegal memory access) on a constant grid — use `arbdmodel.grid.constant_force`.** `install_field_force` wraps `generate_bead_model` so grids re-attach to fresh `ParticleType`s after bead regen (idempotent). `MrdnaJob.e_field` (named `e_field` NOT `field` — shadows `dataclasses.field`) + `CreateMrdnaJobRequest.field` + guards (malformed/zero incl non-numeric→400; field-needs-anchor→400) + runner install after anchors (**RAISES if the field's anchors held 0 beads → the real COM-drift guard**). Oracle `tests/test_mrdna_field.py` 9 fast + 1 slow: FAST = per-bead force vs a **FIRST-PRINCIPLES** pN→kcal/mol/Å (independent of the code constant), 2×mass→2×force, `.dx` ramp `-∇U==F` via `loadGrid`, per-type grid, dry-run conf emits `gridFile field_*.dx`, regen-survival RED-checked, 2 REST guards; SLOW **real ARBD field-on vs off, one strand anchored** — anchored held ~0.5 Å while free bulk deflects ALONG +field ~8.5 Å (field-off ±2 Å wander), \|Δ\| within [0.45,2.0]× the overdamped Brownian pred `D·F·T/(k_B·T)` from the engine's OWN diffusivity/mass, **~12% agreement**. `just test` 4345 passed / 72 skip / 1 xfail (fresh, no drop); ruff clean on touched; main.js LOC Δ=0. Fresh review: product correct, 3 findings all fixed (runner 0-held-beads guard; slow-oracle prediction made emission-constant-independent; malformed-type→400). No mrDNA in-app field picker yet (API-only, like M1) → owes **MV-MRDNA-FIELD**.
- ▶ STATE-M1 (prior) — mrDNA anchors: `backend/core/mrdna_anchors.py` maps SHARED `resolve_anchor_particles`→per-nt keys→**nearest CG bead by 3D POSITION** (mrDNA groups helices by base-pairing not NADOC helix id + collapses each bp to 1 fwd bead → name/ordinal maps unreliable; r+beads share the Å frame). `install_anchor_restraints` wraps `generate_bead_model` so RESTRAINTs survive regen. **GOTCHA: mrdna writes the run PSF/PDB to the run dir, only the DCD under `output/`.** M2 reuses this to hold against field COM-drift.
- ▶ NEXT — **M-ALL-ANCHORS-FIELD is CLOSED.** Remaining toward `M-FULL-COVERAGE` (all engines × all 4 features + card): mrDNA `M3` (extra bases into the ARBD model — verify past the display bridge, deps met), `M4` (linkers, dep M3 — reuse C4's WLC-connector pattern), `M5` (mrDNA card-source + RMSF, dep S5 ✓ — reuse the SOURCE-BUNDLE CONTRACT); NAMD `N3` (extra-base/linker validation, deps met), `N4` (NAMD card-source gold-override, dep S5 ✓); oxDNA `O2`. **Recommend `M5` or `N4`** (card-source bundles, dep S5 met, high cross-val value — puts a 3rd/4th live column on the comparison card via the proven contract) OR `M3` (finish mrDNA's feature coverage; low effort, deps met). Rubric: card-source (M5/N4) leads on cross-validation value; else finish the in-progress mrDNA track (M3→M4). **For any mrDNA field-SOURCE work, emit deflection from the relaxed frame; for linker work reuse C4's WLC-connector; extra-base/linker softening → assert RMSF flexibility, NEVER a twist/bend direction.**
