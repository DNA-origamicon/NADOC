...
- ▶ STATE — **`P1` DONE 2026-07-08** (opens Track P). `backend/core/md_pipeline.py` = the generic `MdPipeline` stage-spec + PURE `build_pipeline_plan` chain builder: stage 0 seeds from `root_job_id`@`root_checkpoint`, stage N>0 from its IMMEDIATE predecessor (`parent_job_id=stage{N-1}`, `start_checkpoint="stage{N-1}::output"`); every hop `run_kind="production"`, seeds=`generate_seeds`; `cross_engine` flag = the seed_oxdna/mrdna generalization. 1-stage parity vs `spawn_md_production`. Generalizes the 3 special-cased provenance hops into ONE object. PARITY/CHAIN oracle 9 FAST (RED-verified offline). No jobs/no topology (Three-Layer clean). main.js LOC-Δ=0. Also DONE: U1 (2026-07-08). Feature tail (M4/N3/O2) parked.
- ▶ NEXT — **stay on U/P.** Two clean openers, either valid: **`P2`** (chain EXECUTOR — consume `StagePlan`, submit stage N on N-1's completion, halt + **resume-from-failed-stage** per LogRocket, carry each stage's forces into its child conf via `external_forces_block`; FAST mocked-3-stage advance+halt+resume oracle, SLOW real 2-stage local chain) to keep Track-P context and drive toward M-DEPOSITION-CHAIN; **or `U2`** (shared Forces card factory — collapse triplicated `efield_setup.js`/`cando_efield_setup.js`/`lammps_forces_setup.js` into `initForcesCard({engine,deps})` over `efield_math`/`oxdna_floor_math`, grey unsupported sub-cards per U1; **ADAPTED-CODE PIN RULE** applies, owes an MV row). U3 (jobs-panel base) also unblocked (needs only U1). P3 (cross-engine seed) unblocked too (dep P1).
