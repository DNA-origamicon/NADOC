...
- ▶ STATE — **M-METRIC-CORE DONE** (`S1`–`S5` all shipped). S5 (2026-07-06) added the comparison CARD: `backend/core/shape_compare.py::build_comparison_report(sources)` (pure assembly over S3/S4) + `routes_shape_metrics.py` (`POST /shape/compare/start` + `GET /shape/compare/{run_id}`) + `shape_compare_card.js` (scalar-delta table + RMSF overlay + agreement + field panel; PNG/CSV export; oxDNA Dynamics panel, `main.js` Δ=0). 15/15 fast oracle `tests/test_shape_compare_report.py`; vitest 9; display-vs-oracle Playwright passed (deleted); `just test` green.
- ▶ NEXT — **`O1`** (oxDNA reference column, low, deps=S1 met) is the fastest way to make the card show LIVE data — emit `{engine:"oxdna", descriptors, rmsf, shape_frame, field}` from a relaxed/production job so the card's `getSources` returns a real oxDNA bundle. Then **`C1`→`C2`** (CanDo anchors→E-field, the M-CANDO-FIELD headline: does the FEM predict oxDNA's deflection?), and **`C5`** to add CanDo as the second card source (RMSF reference). `N1`/`N2` (NAMD eField/anchors, low) also pickable.
- ▶ WIRING THE CARD (for O1/C5/M5/N4) — the card is already built; each engine task just needs to (a) build its source bundle `{engine, descriptors:compute_shape_descriptors(frame), rmsf, shape_frame:frame, field:field_response_profile(...)|None}` and (b) make `getSources` in `oxdna_jobs_panel.js` return it (currently `() => []`). `build_comparison_report` + the card do the rest. Live cross-engine data is standing check **MV-21**.
- ▶ REUSE — card binds `build_comparison_report(sources)` → `{ready, engines, references{shape,rmsf,field}, scalars[{name,reference,cells{engine:{value,signed_pct_delta}}}], rmsf_profiles[{engine,is_reference,points}], agreement[{engine,shape_rmsd_nm,rmsf,field}], field{reference,rows[]}}`. Source bundle = `{engine, descriptors?, rmsf?, shape_frame?, field?}`. Field/agreement reference resolves among **field-carrying engines** (not the data-agnostic policy `references.field`). Underlying math unchanged: `compare_descriptors`/`compare_field_response`/`reference_for` in `shape_metrics.py`.
- ▶ GOTCHAS banked — **cross-engine RMSF collapses to per-bp before matching** (direction-keyed → silent empty intersection → `None`; LESSONS-from-S3). **field-response NOT Kabsch-aligned** (anchored region is the common frame). deviation cross-engine = align=True; degenerate correlation → `None` not NaN. Uniform field needs ≥1 anchor (COM-drift) in every engine; CanDo bend/twist exp36-calibrated (don't regress); every output Physical-layer only; repo has unrelated pre-existing ruff debt (19 errors in OTHER test files) — don't sweep (`feedback_no_bulk_reformat`).
- ▶ REFERENCE — per-observable: oxDNA=shape+field, CanDo=RMSF, NAMD=gold-when-present (wired in `reference_for`). Metrics MUST be generate/view/export (mirror the oxDNA card — S5 did). Auto-commit to master, no push.
