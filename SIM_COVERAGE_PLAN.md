...
- ▶ STATE — **M-METRIC-CORE DONE** (S1–S5); **M-CANDO-FIELD DONE** (C1,C2,S4,S5,O1); **M-CANDO-COMPLETE DONE (2026-07-07)** — `C4` shipped, CanDo now covers all four features (anchors/E-field/extra-bases/linkers) + feeds the card. **C4 finding (user-corrected): a linked overhang is DUPLEX** — `connect_overhangs` materializes the overhang complement strand + a `__lnk__` bridge helix (ds=duplex bridge, ss=ssDNA tether). The gap was that `build_fem_mesh` couldn't SEE it (`_duplex_bp_per_helix` counted scaffold∧staple only). Fix (backend-only, `fem_solver.py`): (1) additive duplex `(scaf∧stap) ∪ (fwd∧rev∧linker)` — byte-identical on linker-free designs (0 exp36 regression, asserted); (2) `_add_linker_hops` couples LINKER-strand helix-hops (ds→rigid link, ss→WLC spring). Oracle `tests/test_cando_linkers.py` 5 FAST (synthetic two-part coupling bright line + real ds/ss census+connectivity). `just test` 4215 (+5, no drop); FEM+exp36 guards green; ruff clean; review both changes correct+additive; main.js Δ=0; no card→display-vs-oracle N/A.
- ▶ NEXT — **all CanDo done.** Pick by rubric among eligible (all deps met): **recommend `N1`** (NAMD native `eField` conf block + Dynamics UI, low-effort, no deps) or **`N2`** (NAMD anchors on the existing consref/conskfile restraints, low-effort) — anchors-before-field, both toward **M-ALL-ANCHORS-FIELD** (needs M1,M2,N1,N2; C1,C2 done). Or card-source tasks `M5`/`N4` (deps=S5 ✅) reusing the SOURCE-BUNDLE CONTRACT below; mrDNA `M1`(anchors)→`M2`(field) also open. **For any linker work in another engine (M4/N3), reuse C4's pattern:** mesh/bead the *materialized* linker topology (ds bridge=duplex, ss bridge=tether) and couple at the LINKER strand's helix-hop junctions (NOT `Design.crossovers`).
