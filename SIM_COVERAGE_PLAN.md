...
- ▶ STATE — **`S1`+`S2`+`S3` DONE** (M-METRIC-CORE half done). `shape_metrics.py` now has the full math stack: `compute_shape_descriptors` (S1), `deviation_profile`/`rmsf_from_ensemble`/`normalize_rmsf_profile` (S2), and `compare_descriptors`/`reference_for` (S3, 2026-07-06, 13/13 fast oracle `tests/test_shape_compare.py`; `just test` 4140 passed). **S4 + S5 remain for M-METRIC-CORE.**
- ▶ NEXT — **`S4`** (low, deps=S1 met): generalize `oxdna_health.measure_field_response` into `shape_metrics.py` — per-nt deflection map, free-region projection-along-field, anchored drift, magnitude ratio, cosine similarity. Then **`S5`** (deps=S3 met now): the comparison CARD (`routes_shape_metrics.py` + `shape_compare_card.js`, mirror the oxDNA metrics card; generate/view/export PNG/CSV). S4+S5 close M-METRIC-CORE and unblock C5/O1/M5/N4.
- ▶ ALSO PICKABLE NOW — `C1` (CanDo anchors, low), `N1` (NAMD eField, low), `N2` (NAMD anchors, low), `M3`, `O1` (oxDNA reference column, deps=S1 met, low). `C1`→`C2` is the fast path to **M-CANDO-FIELD** once S4/S5 exist.
- ▶ REUSE — S5 card binds `compare_descriptors(candidate, reference, *, align_shape=True)` → `{candidate, reference, scalars{name:{candidate,reference,abs_delta,signed_pct_delta}}, rmsf{pearson,spearman,n,…}, shape_rmsd_nm}`, and `reference_for(engines, observable)` for the per-observable reference column. **RMSF matching is per-BASE-PAIR** (`_rmsf_per_bp` collapses strand direction) so CanDo's dir-less NMA rmsf correlates with per-strand ensemble rmsf — don't re-introduce direction keying. S4 field-response feeds the card's field panel.
- ▶ GOTCHAS banked — **cross-engine RMSF must collapse to per-bp before matching** (direction conventions differ; direction-keyed → silent empty intersection → `None`; see LESSONS-banked-from-S3). deviation cross-engine = **align=True** (Kabsch strips pose); degenerate correlation → `None` not NaN. Uniform field needs ≥1 anchor (COM-drift) in every engine; CanDo bend/twist exp36-calibrated (don't regress); every output Physical-layer only; repo has unrelated pre-existing ruff debt (19 errors in OTHER test files) — not a regression, don't sweep (`feedback_no_bulk_reformat`).
