...
- ▶ STATE — **M-METRIC-CORE DONE** (S1–S5). **`O1`+`C5` DONE → S5 card now has TWO live sources → FIRST real oxDNA-vs-CanDo agreement** (2026-07-06). **`C1`+`C2` DONE → M-CANDO-FIELD CLOSED**. `C5` = CanDo the 2nd compare-source: new `backend/core/cando_shape_source.py` `build_cando_shape_source(shape_frame, core_reference, *, rmsf=, field=)` (twin of O1's `oxdna_shape_source`) — core-filters `predict_shape()['positions']`, emits ABSOLUTE `compute_shape_descriptors`, maps dir-less per-bp NMA `rmsf`→card shape with **`direction=None`** (pairs oxDNA per-strand via `_rmsf_per_bp` collapse — S3). Route `GET /cando/jobs/{id}/shape-source`; frontend `getSources` merges oxDNA+CanDo bundles (main.js captures `const candoPanel`, lazy `getCandoJob`, Δ=+4 wiring). `field:None` (deferred). Oracle `tests/test_cando_shape_source.py` 7 (6 fast + 1 SLOW real-`predict_shape`): descriptor self-consistency, core mask, rmsf remap, integration `[oxdna,cando]`→refs shape=oxdna/rmsf=cando + Pearson 1.0 n=24. `just test` 4206; review CONFIRMED-CORRECT; MV-21 updated (live eyeball).
- ▶ NEXT — pick by rubric among CanDo-track `C3` (extra-bases, no deps, medium) / `C4` (linkers, deps=C3) to finish **M-CANDO-COMPLETE** (only C3+C4 left), OR the anchors-before-field NAMD pair `N1`/`N2` (both low-effort, no deps, unblock **M-ALL-ANCHORS-FIELD**), OR `M5`/`N4` (other engines' card sources, deps=S5 ✅, reuse the C5/O1 SOURCE-BUNDLE CONTRACT). **Recommend `C3`** (finishes the CanDo feature-coverage track: extra crossover bases as compliant connector elements in `build_fem_mesh` from `Crossover.extra_bases` — soften local twist/bend, compare_descriptors delta beyond tol + correct sign; **CanDo extra-base ELEMENTS are mesh-build details derived from existing topology metadata, NOT topology mutations** — Three-Layer). Alternatively `N1` if prioritizing the every-engine-field milestone.
