...
- ▶ STATE — **`U3` SLICE 1 DONE 2026-07-08 (U3 still `in_progress`).** User steer: the **oxDNA panel is CANONICAL**; every engine converges to conform to it (indent, list index, status icons, legend, naming) — the shared base IS the oxDNA model, not a lowest-common-denominator. Extracted the oxDNA row/list SHAPE into PURE `frontend/src/ui/jobs_panel_model.js` (`buildJobRowModel`/`buildJobListModel`/`jobListSignature`/`runButtonEnabled`) + DOM `frontend/src/ui/jobs_panel_render.js` (`renderJobRow`/`renderJobList`). Rewired **oxDNA** onto them = **BYTE-IDENTICAL DOM** (parity PIN in `jobs_panel_model.test.js` drives a VERBATIM copy of the old `_jobRow`/`_renderList` vs the new → identical `outerHTML` every branch; +oxDNA panel 87/87 +smoke). Converged **mrDNA** (smallest) = intentional visual UPGRADE (rows gain `[N]` index + spinner-while-running + legend + poll short-circuit); its bespoke row rendering DELETED. `makeSpinner`→`job_status_symbol.js` (re-exported from oxDNA for importers/tests). Gates: oracle 9/9, frontend **183/2325** (+9 no drop), smoke 23/23, ruff N/A. `main.js` LOC-Δ=0; net −48 LOC across panels. Live mrDNA-row pixels owe **MV-23**. Also DONE prior: U1, U2, P1, P2. Feature tail (M4/N3/O2) parked.
- ▶ NEXT — **finish `U3` (slice 2), staying on Track U.** REMAINING for U3: (a) converge **cando + lammps** onto `renderJobList`/`buildJobListModel` (flat like mrDNA — cheap; supply each a `_rowCtx()`; lammps has an inline per-row Stop button to preserve); (b) converge **md** — the 2882-line OUTLIER: hierarchical via `flattenJobTree` **with `{collapsedIds}`** + the expand/collapse chevron + ensemble-summary, `setInterval` + remote/Alpine timers, returns no api. The model already supports `collapsedIds`; the chevron is an extra per-row control the renderer must gain (add a `chevron` field to the row model + a click cb) — do this WITHOUT breaking oxDNA's byte-parity (chevron only when `childCount>0` and ctx opts in); (c) factor the shared **run-buttons + poll loop + section-collapse + advanced-drawer host** into a stateful `initJobsPanelBase({engine,descriptor,deps})` (`runButtonEnabled` is built + tested but NO panel consumes it yet). Then **`U4`** (engine selector + one *Simulate* section, unsupported cards greyed-with-tooltip per U1; dep U2✓+U3). **FRONTEND DISCIPLINE (CLAUDE.md):** PARITY in **vitest pure helpers**; **ADAPTED-CODE PIN** = drive OLD (verbatim copy in the test) + NEW on fresh DOMs → byte-equal, NOT green-first-run (do NOT `git stash`); oxDNA rewires must stay byte-identical; mrDNA/cando/lammps/md convergence is an intentional upgrade (conformance test + `just smoke` + an MV row). `main.js` imports + factory init + thin wiring only. **PARKED:** `P3`/`P4` (job-planner) + P2 follow-ups — resume Track P after U4 lands.
