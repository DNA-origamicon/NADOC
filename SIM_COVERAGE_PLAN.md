...
- ▶ DETOUR 2026-07-08 — session diverted **off-plan at user direction** to fix **ISSUE-15** (remote MD job falsely `completed` when its checkpoint failed to download → never re-fetched → P2 chain-seed 400s). Fixed in `md_executor.reconcile_remote_job`: missing-checkpoint completion stays **re-pollable** (bounded retries → `failed`/`fetch_incomplete`); new `MdJob.fetch_attempts`. Commit `8b36c4a`, `just test` 4449/110/1 (+2). **`sim_coverage_plan.json` UNTOUCHED — O2 still the only pending coverage task.**
- ▶ STATE — **`M6` DONE 2026-07-10 → mrDNA field+anchors now REACHABLE from the unified Simulate panel.** Mounted the shared U2 Forces card (`initForcesCard` engine:`'mrdna'`, numeric/no-gizmo like CanDo/NAMD) + shared Anchors card (`initOxdnaAnchorsSetup` on `mrdna-anchors-*` ids); added the `mrdna` entry to `forces_card.js` `FORCES_FIELD_IDS`+`VARIANTS` + the `mrdna-efield-*`/`mrdna-anchors-*` DOM to index.html; flipped `engine_capabilities` mrdna efield+anchors `off→on` (real present ids). Extracted PURE `buildMrdnaLaunchBody`+`fieldNeedsAnchor` and rewired `_launch` to thread `field:{field_pN,dir}` + `anchors:[scope dicts]` into `POST /mrdna/jobs` (byte-identical contract to CanDo; backend already enforces field-needs-anchor 400 — mirrored client-side). ORACLE `mrdna_jobs_panel.test.js` **+7 fast**: PARITY — the real card output byte-equals `body.field`/`body.anchors` (pinned `field_pN=1.8`, dir `[0,0,3]→[0,0,1]`, anchors `['o1','o2']`); guard blocks field-without-anchor; `engine_capabilities.test.js` census flip enforced vs live index.html id-presence (surface/metrics probes still absent). Fresh-context review CONFIRMED, no gaps. Gates: frontend **198f/2621** vitest (+7), `just test-smart`→FULL **4622/110/1** (watermark bumped, no drop, backend untouched), smoke **23/23** (one flaky `assembly_exit_cleanup` self-cleared on retry). main.js Δ=0. **Live card-gesture→POST-body owes MV-34** (Run gated on real mrDNA install). ⚠ concurrent session's crossover/spreadsheet edits were in-tree (index.html/main.js pre-dirty) — committed ONLY M6 hunks.
- ▶ NEXT — **`M-MRDNA-FORCES-UI` continues: M7 next** (deps=M2✓, medium). M7 = NEW ARBD repulsion-plane hard-surface backend, HEADLESS (property oracle mirroring M1/M2's grid-potential mechanism + the `field_anchor` surface-opposes-field deposition rule; shared `{dir, offset_nm, stiff}` descriptor — the same dict LAMMPS + the oxDNA floor card use). Then **M8** (surface UI card, deps=M7, parity oracle mirroring M6 — mount a shared Surface card + flip `mrdna.surface` on). Also still open: **`O2`** (oxDNA residual gaps; deps ∅; low/low) — a no-op unless a NAMED gap is logged (grep oxDNA topic files + `issues_ledger.md`; if none, ask the user or close as no-residual-gap). Standing follow-up (NOT a JSON task): P2's forces into the production RESEED conf end-to-end (`ProductionRunRequest.field/anchors`+reseed emission).
