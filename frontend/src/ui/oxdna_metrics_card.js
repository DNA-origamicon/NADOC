/**
 * oxdna_metrics_card.js — the "Graphs and Metrics" card in the oxDNA Dynamics
 * panel.  For a job (or its whole parent/child lineage) it computes twist,
 * curvature and base-pairing over the production trajectory and lets the user
 * view them as annotated spatial + temporal graphs, watch an ETA loading bar
 * while they compute, and export PNG / CSV.
 *
 * Factory: `initOxdnaMetricsCard({ getSelectedJob, getJobs })`.  A child module of
 * the oxDNA jobs panel (wired from initOxdnaJobsPanel) — it owns its own DOM (by
 * id), the poll loop, and a per-scope results cache.  One background compute
 * yields all three metrics (single trajectory pass), so any metric's Generate
 * populates the whole card; each metric keeps its own buttons + progress bar.
 */
...
export function initOxdnaMetricsCard({ getSelectedJob = null, getJobs = null } = {}) {
  const card = document.getElementById('oxdna-metrics-card')
  ...
  const arrow = document.getElementById('oxdna-metrics-arrow')
  let _open = false
  ...
  const scopeLatest = document.getElementById('oxdna-metrics-scope-latest')
  const scopeChain = document.getElementById('oxdna-metrics-scope-chain')
  const rows = {}
  for (const { key, tok } of METRICS) {
    rows[key] = {
      gen: document.getElementById(`oxdna-metrics-${tok}-gen`),
      disp: document.getElementById(`oxdna-metrics-${tok}-display`),
      exp: document.getElementById(`oxdna-metrics-${tok}-export`),
      bar: document.getElementById(`oxdna-metrics-${tok}-bar`),
      fill: document.getElementById(`oxdna-metrics-${tok}-fill`),
      status: document.getElementById(`oxdna-metrics-${tok}-status`),
    }
  }
  ...
  async function _generate(metricKey) {
    ...
    if (!jobId) { _setStatus(row, 'Select or run an oxDNA job first.', '#d29922'); return }
    ...
    let start
    try {
      start = await startOxdnaMetrics(jobId, { scope: _scope() })
    } catch (e) {
  ...
  function _poll(runId, metricKey) {
    ...
    const tick = async () => {
      let st
      try { st = await getOxdnaMetricsRun(runId) } catch (e) {
