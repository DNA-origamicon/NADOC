...
export function setupStepState({ connected, balance, sshPresent, volumeId, preflight } = {}) {
  const credit =
...
export function initRunpodSetup({ mount, fetchImpl = fetch, onConnected = () => {} } = {}) {
  ...
  async function selectVolume(id) {
    ...
    await _post('/api/runpod/connect', { api_key: _apiKey.trim(), network_volume_id: id })
    _busy = ''
    _render()
  }
  ...
  function _checklist() {
    const s = setupStepState({
      ...
      volumeId: _volumeId, preflight: _preflight,
    })
    return `<div style="display:flex;flex-wrap:wrap;gap:10px 16px;padding:8px 10px;background:#0d1117;
      ...
      ${_stepRow(s.verify, 5, 'Pre-flight')}
    </div>`
  }
  ...
  function _preflightBlock() {
    const canRun = _connected && !!_volumeId
    ...
    return `Run the pre-flight gate — it proves the job can actually launch (key, volume, SSH,
      GPU arch, stock) before anything is rented:
  ...
  function _render() {
    ...
    body.innerHTML = `
      ...
      ${_section(2, 'API key', `Create a key at ${_link(LINKS.settings, 'RunPod → Settings → API Keys')},
        ...
        ${_connected ? `<div style="margin-top:5px;color:${_C.ok}">✓ Connected.</div>` : ''}
        ${_verifyError ? `<div style="margin-top:5px;color:${_C.bad}">${_esc(_verifyError)}</div>` : ''}`)}
      ${_section(3, 'Register your SSH key', _sshBlock())}
      ${_section(4, 'Network volume', _volumeBlock())}
      ${_section(5, 'Verify the pipeline', _preflightBlock())}
    `
