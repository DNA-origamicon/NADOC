  ...
  function _rebuild(design) {
    ...
    log.forEach((entry, i) => {
      ...
      if (entry.feature_type === 'deformation') {
        ...
        // Revert: drop this deformation AND every later log entry — same
        // contract as snapshot revert. Delta entries carry no baked snapshot,
        // so there's nothing to evict and the button is always available.
        const revertBtn = document.createElement('button')
        revertBtn.textContent = '↶'
        revertBtn.title = `Revert to before ${kind} (removes this and all later features)`
