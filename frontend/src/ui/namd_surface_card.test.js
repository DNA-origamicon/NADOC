...
it('previews setup while keeping selected job descriptors immutable',()=>{
 ...
 card.select(job,prep);expect(events.at(-1).enabled).toBe(true)
...
it('keeps automatically selected inherited walls visible and replays state for late renderers',()=>{
 const card=setup(),listen=vi.fn()
 window.addEventListener('nadoc:namd-surface-selection',listen)
 card.select({job_id:'production',parent_job_id:'parent'},{graphene_nanopore:true,graphene_pore_diameter_nm:0})
 expect(listen).toHaveBeenLastCalledWith(expect.objectContaining({detail:expect.objectContaining({enabled:true,jobId:'production'})}))
 card.select({job_id:'production'},{graphene_nanopore:true,graphene_pore_diameter_nm:0})
 expect(listen).toHaveBeenCalledTimes(1)
 window.dispatchEvent(new Event('nadoc:namd-surface-request'))
 expect(listen).toHaveBeenCalledTimes(2)
 window.removeEventListener('nadoc:namd-surface-selection',listen)
})
