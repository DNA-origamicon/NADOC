...
it('keeps the preview suppressed until all simulation owners release it', () => {
  const view = initGrapheneNanoporeOverlay(new THREE.Scene())
  view.update({ enabled: true, surface: { positionNm: 0 } })
  view.setSimulationActive(true)
  view.setSimulationActive(true, 'ion-paths')
  view.setSimulationActive(false)
  view.setDisplay({ representation: 'ball', visible: true })
  expect(view.mesh().visible).toBe(false)
  view.setSimulationActive(true)
  view.setSimulationActive(false, 'ion-paths')
  expect(view.mesh().visible).toBe(false)
  view.setSimulationActive(false)
  expect(view.mesh().visible).toBe(true)
  view.dispose()
