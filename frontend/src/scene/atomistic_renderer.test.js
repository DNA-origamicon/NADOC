...
describe('CPD rigid-group preview', () => {
  it('moves both crosslink ends together and restores their original bond', () => {
    const scene = new THREE.Scene()
    const ar = initAtomisticRenderer(scene)
    ar.setMode('ballstick')
    ar.update({ atoms: [0, 1].map(k => ({ serial: k, element: 'C', name: 'C5', helix_id: 'h0', crossover_id: 'xo', extra_base_k: k, x: k * 0.15, y: 0, z: 0 })), bonds: [[0, 1]] })
    const targets = [0, 1].map(k => ({ helix_id: '__xb__', crossover_id: 'xo', k }))
    const matrix = new THREE.Matrix4().makeTranslation(2, 3, 4)
      .multiply(new THREE.Matrix4().makeRotationZ(Math.PI / 2))
    expect(ar.applyResiduesMatrix(targets, matrix)).toBe(true)
    expect(bondCylinderScaleY(scene)).toBeCloseTo(0.15)
    const bond = new THREE.Matrix4()
    _bondMesh(scene).getMatrixAt(0, bond)
    const center = new THREE.Vector3().setFromMatrixPosition(bond)
    expect(center.x).toBeCloseTo(2)
    expect(center.y).toBeCloseTo(3.075)
    expect(center.z).toBeCloseTo(4)
    ar.applyResiduesMatrix(targets, new THREE.Matrix4())
    expect(bondCylinderScaleY(scene)).toBeCloseTo(0.15)
    ar.dispose()
  })
})
