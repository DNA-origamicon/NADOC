...
export async function prepareTrajectory({ prepared, source, store, from, to, step = 1, fps = 8, signal, onProgress = () => {} }) {
  ...
  if (state.assemblyActive || state.atomisticMode !== 'off' && state.atomisticMode != null || state.surfaceMode !== 'off' && state.surfaceMode != null) throw new Error('The first trajectory clip supports the Full part view. Turn off atomistic and surface views before preparing.')
  ...
  const live = () => !signal?.aborted && store.getState().currentDesign === document && controller.activeJobId() === job && controller.isActive() && controller.alignment?.() === alignment && JSON.stringify(controller.trajSpec()) === JSON.stringify(spec)
  const sameSource = () => store.getState().currentDesign === document && controller.activeJobId() === job && controller.isActive() && controller.alignment?.() === alignment && JSON.stringify(controller.trajSpec()) === JSON.stringify(spec)
  ...
  const sourceFrames = [], frames = []; let first, base, size = 0
  pause?.()
  try {
    for (let index = from; index <= to; index += step) {
      ...
      check(); controller.showFrame(index)
    ...
    first.trajectory = { version: 1, encoding: 'absolute-render-patch-gzip', fps, sourceFrames, frames,
      source: { engine: 'namd', job, spec, alignment, water: false }, fidelity: 'Exact exported Full display coordinates; temporal sampling only' }
    return { buffer: encodeContainer(first), title: first.title, trajectory: true }
  } finally {
    if (sameSource()) {
      ...
      if (sameSource()) {
        ...
        controller.showFrame(info.frame - 1); await companion?.settleFrame(info.frame - 1)
      }
    }
  }
}
