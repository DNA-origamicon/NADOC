...
export function mountMeetingLiveFrame({ viewer, base, revision, fetch: request = fetch, document: doc = document,
  ...
  async function tick() {
    ...
    try {
      ...
      apply.apply(raw); timeline.update(deliveredTimeline); shown = delivered.sequence; status.textContent = ''
    ...
    } finally { flight = null; if (!disposed) timeline.setBuffering(shown < (latest?.sequence ?? -1), latest?.timeline) }
  }
  ...
  }, dispose() { disposed = true; flight?.abort(); cancel(timer); status.remove(); timeline.dispose() } }
}
