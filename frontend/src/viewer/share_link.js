...
export function initShareLink({ exportView, broadcast, trajectory, document: doc = document, fetch: request = fetch, clipboard = navigator.clipboard }) {
  ...
  let busy = false, disposed = false, revision = 0, selectedId = null, shares = [], capabilities = [], jobs = null, statusTimer = null
  ...
  async function refresh() {
    ...
    try { const value = await api('status'); if (value.running === false) throw new Error('Host is offline'); if (!disposed && ticket === revision) { shares = value.shares; capabilities = value.capabilities ?? []; renderShares(); status.textContent = value.shares.length ? 'Update this presentation without changing the guest invitation.' : 'Host ready. Create a link for the current view.' } }
  ...
  async function create() {
    ...
    try {
      ...
      if (!disposed) { shares = [share, ...shares.filter(value => value.id !== share.id)]; selectedId = share.id; renderShares(); if (controls.perspective) await sharePerspective(true); status.textContent = target ? 'Shared view updated. Guests keep the same link and sign-in.' : 'Invitation ready. Send it to your guests.' }
  ...
  return { show, bindJobs(options) {
    ...
    jobs = initJobSharing({ ...options, ...broadcast, document: doc, fetch: request,
      ...
      beforeStart: async () => { await stopNative(); await jobs.setPerspective(controls.perspective) },
