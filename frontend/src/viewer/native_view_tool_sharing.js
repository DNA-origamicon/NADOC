...
/** Mirror tool and annotation changes only in the context explicitly published by this editor. */
export function initNativeViewToolSharing({ prepared, store, getContext, getRoom, isBusy, publish, onError,
  ...
  function capture() {
    ...
    return { context: JSON.stringify([broadcastDocument(store.getState()), source.scene.uuid, source.pane, getContext()]),
      tools: JSON.stringify([source.view?.viewTools ?? null, source.view?.annotations ?? [], source.view?.selection ?? null]), room: getRoom()?.id }
  }
  ...
  function tick() {
    if (!flight) flight = run().catch(error => onError(error.message)).finally(() => { flight = null })
    return flight
  }
