...
export function initScene(canvas) {
  ...
  function animateCameraTo({ position, target, up, fov, duration = 350 }) {
    ...
    finishAnimation?.(); finishAnimation = null
  ...
  const fastZoom = e => {
    ...
    const frame = requestAnimationFrame(() => {
      ...
      _inner.zoomSpeed = isTrackball ? 1.2 : 1.0
    })
    pendingFrames.add(frame)
  }
  canvas.addEventListener('wheel', fastZoom, { capture: true, passive: true })
  ...
  canvas.addEventListener('pointermove', fastPan, { capture: true, passive: true })
  ...
  function dispose() {
    ...
    canvas.removeEventListener('pointermove', fastPan, true)
    ...
    _inner.dispose()
    if (_savedInner && _savedInner !== _inner) _savedInner.dispose()
    _savedInner = null
    _onResize = null
    _navSegments = () => new Float64Array(0)
    renderer.dispose()
  }
