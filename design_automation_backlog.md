...
**▶ STATE (2026-07-16):** Tiers 0–7 + AF-36 + AF-27 P1/P2 + AF-38 + AF-39 + AF-42 done, and **AF-37's root-to-root CREATION half shipped this session** — `hb.create_overhang_binding` / `patch_overhang_binding` (the `bound=True` topology relocation) / `delete_overhang_binding` / `split_sub_domain` / `patch_sub_domain`, + oracle **`assert_bind_unbind_inverse`** (bind→unbind is a clean topological inverse; 4 clauses incl. an explicit non-vacuity guard). Coverage **56 → 61** — note the old handoff's "54" was **stale**; the function-identity pin is truth. AF-24 P2/P3 still **OTHER COMPUTER — don't pick up.**
...
**▶ NEXT cycle — COMPOSE the full hinge-with-linker end-to-end (the AF-27 keystone has all its pieces).** Drive a `scratchpad/build_linked_hinge.py` generator (lift the AF-36 pattern): `build_hinge(k,n)` → `overhang_extrude` a staple overhang on EACH leaf at the gap face → `hb.connect_overhangs` (P1) to tie them with a ds/ss linker whose contour CONFINES the angle → place a revolute joint on the gap edge → `hb.relax_overhang_connection` to settle the rest pose → set absolute hinge angle → save. **BLOCKED, ASK-FIRST (still un-answered):** WHICH overhang nucleotide is the linker attach endpoint, ss-vs-ds, and the bridge length for a given gap — directionality/topology, do NOT infer. Augment = compose `assert_linker_connects` + `assert_linker_relaxed_pose` on the generated design.
**▶ ALSO PICKABLE NOW:** **AF-40/41** (headless constrained-drag solver + movable-link chain; mirror AF-29's parity-port pattern) are the best self-contained pickups. **AF-37's remaining half = *sub-domain* (partial) bindings — ⚠ ASK THE USER FIRST:** `project_overhang_duplex_foundation.md` **Phase 6 explicitly demotes sub-domains + retires `OverhangBinding`** once the Duplex migration is proven, so more sub-domain-binding wrappers may be building on a path being replaced. (Wrapping the *root-to-root* half was still right: the route is live + UI-wired, `overhang_bindings` DATA drives duplex derivation + `prior_driven_topology`, and `assert_bind_unbind_inverse` is exactly the regression net `synthesize_duplexes_from_bindings` needs to prove Phase 6 preserves behaviour.) Older open: AF-24 P2/P3 (OTHER COMPUTER), Tier F AF-ATOM P2/P3.
**▶ WATCH-OUTS:** (1) **Don't build a handoff's named oracle without checking it isn't already covered** — AF-37's specified `assert_binding_locks_joint` would have been redundant (bind's auto-relax was reverted 2026-05-14 → `locked_angle_deg` stays None; already route-pinned) = a passthrough. Banked as a log lesson. (2) **`just test-smart` may print `HEAVY TEST IN THE FAST SUITE` on tests that are 0.7–1.5 s standalone** (guard times under `-n auto`/nice-10) — **do NOT triage on sight**; check standalone first (`pytest <nodeid> -p no:xdist -p no:randomly --durations=5`). Root cause = **ISSUE-20**; rule = **LESSONS H11**. Budget/guard stay untouchable.
