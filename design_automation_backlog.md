...
**▶ STATE (2026-06-29):** Tiers 0–7 + AF-36 + **AF-27 P1+P2 (overhang-LINKER create + relax)** + **AF-37 end-to-root DIRECT binding** done (coverage **52**). AF-37 end-to-root shipped via the ConnectionVersion-apply path (NOT the reverted sub-domain/bind-lock surface): `hb.create_connection_version(connection_type="end-to-root")` → `hb.apply_connection_version` → `apply_end_to_root_binder` (regenerate B as A's RC binder spliced into B's root staple + forced-ligation the junction + drop stale crossover + delete orphan helix; B consumed). Oracle `assert_end_to_root_binder` (8 clauses, round-trip-pinned) + autonomous-build composition gate (`assert_roundtrip_stable`) + caDNAno-export validation. AF-37 root-to-root / sub-domain-binding / joint-lock STILL OPEN (blockers 1–4 in the entry). AF-27 P2 oracles `assert_linker_relaxed_pose` / `assert_bond_relaxed_pose` (STRAIN-REDUCTION). AF-24 **P2/P3** still **OTHER COMPUTER — don't pick up.**
