...
**AF-33 P2 — 2x4/2x6 hinge primitives (`build_hinge_primitive` extension) + golden-equality pins** · _shape:_ 2 NEW
`_HingeSpec` entries (`2x4_double_hinge_link` / `2x6_triple_hinge_link`) in `backend/api/headless_hinge_build.py` — the
SAME `build_hinge_primitive` builder, more bridges; NO new wrapper, NO new oracle (reuses AF-33 P1's
`assert_matches_primitive`); `crud.py`/`assembly.py`/`main.js`/`headless_build.py` LOC Δ = **0** · _headless-coverage Δ:_
**48 → 48 / 239** (FLAT — drives only already-covered routes) · _oracle:_ `assert_matches_primitive` REUSED — each
code-built primitive is byte-for-byte its hand-built golden (`canonical_topology` == golden AND `_fl_endpoint_set` ==
golden [load-bearing: topology fingerprint blind to `forced_ligations`] AND `roundtrip_nadoc`-stable AND `validate_design`)
· _recipe (transcribed VERBATIM from each golden's own feature log, NOT geometrically re-derived — the ASK-FIRST gap
geometry):_ `bundle-create` params → `cells` (serpentine); the *Fine Routing* cluster's per-column `strand-end-resize` +
`forced-ligation-create` children → each bridge's `trim` + `(three, five)`. **2x4 carries ASYMMETRIC hand-authored trims**
by column parity (even cols `scaf_1_c 3p −16`, odd cols `scaf_1_c 5p −2`) — differ from 2x2's `3p −3`/`5p −16`, confirming
they're per-primitive constants, not a rule. **2x6 was generated by `build_hinge(2,6)`** (uniform LO-end FL geometry, NO
trims) so its spec bridges carry no `trim` · _empirical de-risk before coding:_ `build_hinge(2,6)` reproduces the 2x6
golden (canon+FL match) but `build_hinge(2,4)` does NOT (the golden's hand trims), and a per-bridge (trim→FL) replay of
both candidate specs matched their goldens byte-for-byte (per-column strand independence makes per-bridge ordering ==
the golden's all-trims-then-all-FLs) · _tests:_ 2 shape (`test_build_2x4_has_the_hinge_shape` /
`test_build_2x6_has_the_hinge_shape`) + 2 golden-match (`test_build_2x4_matches_golden` /
`test_build_2x6_matches_golden`, skip-if-missing) in `test_headless_hinge_build.py`; the now-stale unknown-name test
repointed to `9x9_nonexistent_hinge` (2x4/2x6 are supported now); full suite **3318 passed / 61 skipped** (no drops),
lint clean · **"Validation gained, not just a passthrough:** before P2 only the 2x2 hinge had a code builder; nothing
proved the code path reproduces the 2x4/2x6 standard hinges — INCLUDING their asymmetric hand-authored gap trims and
off-strand-graph FL links. The golden-match pins prove `build_hinge_primitive("2x4_double_hinge_link")` and
`"2x6_triple_hinge_link")` are byte-for-byte their validated goldens, so a drifted trim, dropped/mis-wired link, or wrong
leaf layout fails. NB **builder-only** — multi-link routing (G6) is still an algorithm blocker (`test_hinge_router`
xfail), so their AF-34-style autoscaffold validation is pending.**"

**AF-35 — headless multi-op primitive PLACEMENT (preserve-verbatim graft) + `assert_primitive_placed`** · _shape:_ NEW
pure `backend/core/primitive_placement.py` (`place_primitive_into` + helpers `detect_plane`/`primitive_anchor_cell`/
`translate_design`) — the **service+oracle** shape (backlog rule 3, HTTP-free) — + a thin commit wrapper
`hb.place_primitive` in `headless_build.py` (+66 LOC) + NEW oracle `assert_primitive_placed` (+helpers) in
`tests/automation_harness.py`; `crud.py`/`assembly.py`/`main.js` LOC Δ = **0** · _headless-coverage Δ:_ **48 → 48 / 239**
(FLAT — the graft imports a `backend/core` service, no new route handler) · _user decision (ASK-FIRST resolved
2026-06-27):_ **preserve the primitive's scaffold/FL routing VERBATIM** (not re-route after compose) → built as a rigid
GRAFT, NOT a feature-log replay (a replay would route through `bundle-segment`, a different builder → AF-30 ISSUE-13 axis
drift; copying the primitive's already-correct geometry + one rigid lattice translation is what makes "verbatim" literally
true) · _what's copied+remapped:_ `helices`/`strands`/`forced_ligations`/`cluster_transforms` (the hinge's 2 identity
rigid-leaf clusters included); the standalone's `feature_log` is NOT grafted (it's its construction history, not the
host's); a `_UNSUPPORTED_LIST_FIELDS` guard RAISES on any other populated field so "verbatim" stays honest · _oracle:_
`assert_primitive_placed(before, after, primitive, *, anchor_cell)` — 6 clauses (non-vacuity + additive [host
`canonical_topology` unchanged] + anchored [placed min-cell == anchor] + verbatim [offset-corrected INDEPENDENTLY via
`_lattice_position` → canon == primitive] + FL-links-survived [grid-keyed, the canon blind-spot] + clusters-survived);
the offset-correct uses the lattice CONSTANT not the graft's own `_world_delta`, so a graft bug can't self-mask · _tests:_
4 in `test_headless_build.py` (empty/additive/revertable/collision) + 7 red in `test_automation_harness.py`
(vacuous/wrong-anchor/dropped-FL/lost-cluster/distorted/mutated-host) · full suite **3329 passed / 61 skipped** (no
drops, +11 from AF-33 P2's 3318), lint clean · **"Validation gained, not just a passthrough:** before AF-35 a saved
multi-op primitive (a hinge) could only be built standalone — there was no headless way to COMPOSE it into a bigger
design, and nothing proved a composition preserved its hinge FL links + leaf clusters or left the host intact. The oracle
proves the placed sub-structure is an EXACT rigid copy of the primitive at the requested cell, with its off-strand-graph
FL/cluster records intact and the host unmutated — none of which `canonical_topology` alone can see. The prerequisite for
'an autoscaffold design WITH a hinge in it' (vs a design that IS a bare hinge).**"

**AF-36 — end-to-end HINGE DESIGN generator (phase-paired fine routing + relax + joint + hinge-angle) + 3 seek-fidelity
backend fixes · MANUALLY VALIDATED (user, 2026-06-27)** · _shape:_ upgraded `build_hinge` in
`backend/api/headless_hinge_build.py` (phase-paired short/long ssDNA via new `_rail_faces_toward` + `_scaffold_on_helix`
helpers; `short_ssdna_bp`/`long_ssdna_bp` params default **2/16**) + 3-field fix to `_topology_substitute` in `crud.py`
(restore `cluster_joints` + `flexible_segment_marks` + `flexible_connections` on feature-log seek) + the user's design
driver `scratchpad/build_3x6_hinge.py` (NOT a repo module); `assembly.py`/`main.js` LOC Δ = **0** · _headless-coverage Δ:_
**48 → 48** (no new route — `build_hinge` composes shipped wrappers; the seek fix is a bug fix not a wrapper) · _oracles:_
NEW `_rail_faces_toward` **phase oracle** (gap-face scaffold backbone radial · rail-pair chord: `>0`→short, `<0`→long —
validated byte-for-byte against the 2x2/2x4 goldens, which it now reproduces) · REUSED `assert_flexible_segments_relaxed`
(each connection's chord ≤ **its own** contour — short 2nt=1.18nm pulled exactly taut [the binding constraint], long
16nt=9.44nm slack) · 2 NEW seek regression pins `test_seek_before_joint_placement_drops_the_joint` (test_joints.py) +
`test_seek_before_mark_drops_flexible_marks` (test_flexible_segments.py), **both proven red-without-fix** · _the design
(`workspace/3x6_autogen_hinge.nadoc`):_ two 18-helix 3×6 arms · 6 FL rungs = **3 short + 3 long** ssDNA pairs (alternating
by column, all on the leaf-A rail) · relaxed · revolute joint on the gap edge · **absolute 60° fold** (composed onto the
relaxed pose by solving the fold θ so the signed dihedral about the hinge axis == 60°) · validator-clean · feature-log
seek-faithful (joints + marks toggle, empty-state resets) · _build_hinge fine-routing fix:_ replaced UNIFORM tethers with
phase-paired short/long; the 2x2 golden (user-corrected mid-session from an inverted authoring bug) + 2x4 now reproduce
byte-for-byte (`build_hinge(2,2)` == corrected 2x2; the hand-trim 2x2 spec re-transcribed `3p −16`/`5p −2`) · _tests:_ 27
in `test_headless_hinge_build.py` + 2 seek pins; full suite **3331 passed / 61 skipped**, lint clean · **"Validation
gained, not just a passthrough:** the first full **text-to-DNA-origami** composite — "a three-link hinge between two 3×6
200bp arms, relaxed, with a 60° revolute joint" built entirely headlessly and validated at every layer (topological via
the goldens + validator, geometric via the phase oracle + per-tether relax chords, feature-log via seek fidelity). The
fine-routing fix is the REAL hinge mechanic (phase-paired short/long, not uniform — the away-facing rail extends ~1 turn
to re-phase), and the seek-fidelity fixes make every autogenerated pose/joint/mark op rollback-faithful — the
prerequisite for a trustworthy generated construction history.**"

---

**AF-27 P2 — overhang-linker + generic-bond RELAX wrappers (the hinge-confinement rest-pose, pose layer)** · _shape:_
2 wrappers `hb.relax_overhang_connection` + `hb.relax_bond` in `backend/api/headless_build.py` (import the route handlers
`relax_overhang_connection`/`relax_bond_endpoint` + `RelaxLinkerRequest`/`RelaxBondRequest`/`RelaxBondEndpoint`); `crud.py`/
`assembly.py`/`main.js` LOC Δ = **0** · _headless-coverage Δ:_ **48 → 50** (two routes flipped by function identity) ·
_three-layer:_ VERIFIED display-pose — both relax routes mutate only `cluster_transforms` (+ a `ClusterOpLogEntry`), never
the strand graph, so `canonical_topology` is invariant (the load-bearing pin) · _oracles:_ 2 NEW reusable —
`assert_linker_relaxed_pose` (linker: `|anchor_chord − _ds_target_length_nm|` falls) + `assert_bond_relaxed_pose` (bond:
`|endpoint_chord − target_nm|` falls), both **STRAIN-REDUCTION** (user choice 2026-06-27 over raw chord-≤-contour) + pose-moved
+ topology-unchanged, sharing private `_assert_relax_pose`/`_relax_pose_moved`; chord re-measured on the POSED geometry
(`_anchor_pos_and_normal` / by-(helix,bp,dir) lookup), independent of the relax's connector-arc optimiser · _key finding:_
the natural relax fixture is DEGENERATE — joint origin ON the moving overhang puts its anchor on the hinge axis so rotation
can't change the chord (the load-bearing can-go-red); origin OFF the overhang drives the chord exactly to the natural span ·
_tests:_ 5 in `test_headless_build.py` (linker pass/degenerate/pose-only + bond 0-DOF/1-DOF) + 5 in `test_automation_harness.py`
(linker pass + degenerate-red + topology-mutation-red + bond pass + bond no-op-red) + coverage-count meta bumped 48→50 in
3 files; full suite **3341 passed / 61 skipped** (no drops), lint clean · **"Validation gained, not just a passthrough:**
before AF-27 P2 a script could TIE two leaves with a linker (P1) but could not COMPUTE its relaxed rest pose, and nothing
proved a relax actually pulls the linker/bond toward its natural span WITHOUT editing the strand graph; the two strain-reduction
oracles pin exactly that (the chord moves to the natural span, a rigid pose moves, topology is untouched) — the geometric
rest pose that confines the hinge angle, now headless and validated, with the degenerate-hinge can-go-red proving the pin
isn't vacuous.**"
