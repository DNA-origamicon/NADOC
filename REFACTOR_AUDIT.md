...
15. **CWD-safety preamble (Step 0).** (Pass 6-B added after worker initially wrote to main repo instead of worktree, self-recovered via `just test-file` failure but lost time on the recovery dance. Pass 11-C hardened from advisory `STOP and report` to literal shell `exit 1` after Pass 10-I worker self-reported a CWD mismatch yet proceeded with the work anyway — substance was clean but pathview.js + pathview/palette.js landed in master directly with no worktree branch to merge from. See Finding #37 + Followup 10-I.) Every worker prompt's first executable step MUST be:
    ```
    pwd
    ...
    ```
    The Agent tool's `isolation: "worktree"` sets the worktree path automatically, but the worker's tool-use environment can drift if early Read/Edit calls use absolute paths back to the main repo. The pre-flight assert catches the drift before any write. The literal `exit 1` (not "STOP and report") prevents the Pass 10-I failure mode where a worker self-reports the breach in the final message but does the work anyway.

    **Corollary (Pass 11-C added):** if pwd does not match `$EXPECTED_WORKTREE`, the worker MUST exit non-zero before any code change. Manager re-dispatches with a fresh worktree. This covers the auto-cleaned-worktree edge case (Agent tool's `isolation: "worktree"` is supposed to create a worktree but in a race / edge case may not) — the worker MUST stop, never fall back to running in master.
...
21. **Coverage targets calibrated against Skip list.** (Pass 9-C added after the prompt's `≥50%` target was unreachable additively because every non-named function was Skip-listed.) For coverage-backfill prompts, compute `(stmts in named helpers + currently-covered stmts) / total stmts` BEFORE writing the target into the prompt. Set the floor at that number, not at a round figure. Or drop numeric targets entirely: require "fully cover the named helpers + opportunistic gains" as the binary pass criterion. Add a "natural ceiling" escape clause: if the worker's analysis shows the target is unreachable without violating the Skip list, allow REFACTORED status on the helper-coverage criterion alone, with an audit note. **(Pass 11-B refinement)**: for I/O modules with documentably env-bound fallback branches (e.g. ImportError-guards for optional deps, optional binary-trajectory readers), set target to natural ceiling rather than capping at 70%. Pass 11-B delivered 96% coverage where the conservative target was 70% — natural ceiling was 96% because the only uncovered code was an MDAnalysis-import fallback (hard dep, branch unreachable) and an XTC-binary loader (env-bound, intentionally skip-marked).
...
25. **Model-divergence apparent-bug consolidation** (escalation criterion: 3+ corroborations triggers escalation). When an apparent-bug flag is corroborated in ≥2 unrelated test backfills, the manager MUST escalate to a top-level investigation Finding rather than letting it accumulate as per-file flags. **Original instance OBSOLETE (2026-05-10)**: the `Design.crossover_bases` / `xover.strand_a_id` / `xover.domain_a_index` divergence (3 corroborations at Pass 10 close) was resolved 2-of-3 by remote MD work (`fem_solver.py` rewritten to use `xo.half_a.helix_id`), and the last remaining 1-of-3 (`xpbd_fast.py:364-370`) was archived to `archive/physics_xpbd_fem/` when xpbd + FEM were pruned as derelict features per user 2026-05-10. The escalation criterion itself remains valid as a framework rule for future instances; the original concrete instance is no longer live.
...
- **5 lowest-coverage modules** (all 0%; tied; ranked by line count):
  ...
  **Next-tier (>0%, still concerning):** `backend/api/ws.py` 4.2% (websocket; raised to 81% by Pass 10-D, then partially obsolete after dereliction prune); `backend/core/mrdna_bridge.py` 6.5%; `backend/core/atomistic_to_nadoc.py` ~~18.7%~~ **55%** pre-Pass-11 (raised to 96% by Pass 11-B; original 18.7% figure was stale, reproduced 55% pre-Pass-11); `backend/core/sequences.py` 20.8% (raised to 97% by Pass 7-B); `backend/physics/fem_solver.py` 21.6% (testable physics math; ARCHIVED 2026-05-10 as derelict feature); `backend/core/seamed_router.py` 29.0%.
...
- **Framework debt surfaced**: precondition #15 must be hardened from warn → fail (worker self-reported but proceeded; queued as precondition update — closed by Finding #40 in Pass 11-C)

### 38. `atomistic.py` pure-helper extraction + tests — `low` ✓ REFACTORED + MERGED 2026-05-10
- **Category**: (c)+(test) god-file pure-helper extraction (third application of the proven template)
- **Move type**: extracted-with-edits (18 functions + 23 constants moved verbatim; atomistic.py re-imports the 23 names actively used by remaining call sites)
- **Where**: `backend/core/atomistic_helpers.py` (new, 483 LOC, 99% coverage); `backend/core/atomistic.py` 2602 → 2204 LOC (Δ=−398, ~15% reduction)
- **Diff hygiene**: worktree-used: yes (`agent-aea8de38905ba144e`); files-this-refactor-touched: 3 (atomistic.py, atomistic_helpers.py, tests/test_atomistic_helpers.py); other-files: none
- **Transparency check**: PASS — re-import block at atomistic.py L51-75 covers the 23 names actively referenced by atomistic.py's remaining surface; precondition #20 dead-import sweep correctly applied (18 of 41 moved symbols dropped from re-import block — they were pure-leaf consumed only by gradient siblings within atomistic_helpers.py)
- **API surface added**: 18 functions + 23 constants exported from `atomistic_helpers.py` (all `_`-prefixed; module-private convention preserved)
- **Visibility changes**: none (visibility prefix preserved post-move)
- **Pre-metric → Post-metric**:
  - `atomistic.py` LOC: 2602 → 2204 (Δ=−398)
  - `atomistic_helpers.py`: 483 LOC, 99% coverage (1 missed line: defensive `if g is None: continue` guard in `_rb_grad_propagate`)
  - Tests: +61 across 22 classes (`TestNormalise`, `TestLerp`, `TestCosAngle3pt`, `TestMakeSpinRotation`, `TestSpinRotationDeriv`, `TestBackboneBridgeCost`, `TestCosAngleGrad`, `TestBackboneBridgeCostGrad`, `TestGlycosidicCost`, `TestGlycosidicCostGrad`, `TestRepulsionCost`, `TestRepulsionCostGrad`, `TestRBPairRepulsion`, `TestRBPairRepulsionGrad`, `TestRBGradPropagate`, `TestFwdBridgeX0`, `TestBwdBridgeX0`, `TestBezierPt`, `TestBezierTan`, `TestArcBowDir`, `TestArcCtrlPt`, `TestConstants`)
  - Lint Δ: 0
- **Pure-helper claim verified (precondition #19 substantive rule)**: imports ONLY `__future__`, `math`, `numpy`. No `backend.core.*`, no subprocess, no I/O. Tighter than the prompt's allowlist (which permitted `backend.core.{models,constants}`).
- **Test honesty**: zero mocks/patches. All 5 spot-checked gradient tests use real central-difference comparisons (`(f(x+ε) − f(x−ε)) / (2ε)`) against analytic gradients with `np.allclose` atol 1e-5 to 1e-7.
- **Apparent-bug flags**: none
- **Out-of-scope preservation**: orchestrators (`build_atomistic_model`, `_apply_backbone_torsions`, `_build_extra_base_atoms`, `_minimize_*_extra_base`, `merge_models`, `_atom_frame`, `_apply_phosphate`, `_atom_pos`/`_set_atom_pos`/`_translate_atom`, `_rb_extract`/`_rb_world`/`_rb_apply`, `_extra_base_frame`) all remain in atomistic.py. `_PHASE_*`, `_SUGAR`, `_FRAME_ROT_RAD`, `_ATOMISTIC_*` constants untouched.
- **Linked Findings**: #18 (apparent-bug history; `_SUGAR` not modified), #27 (Pass 9-B gromacs_helpers precedent), #29 (Pass 10-A namd_helpers precedent), policy row 2026-05-10 (atomistic family unlock)
- **Template progression**: 09-B gromacs_package −114 LOC (5%); 10-A namd_package −458 LOC (52%); 11-A atomistic.py −398 LOC (15%). Template now triple-validated; suitable for further atomistic-family follow-on extracts.

### 39. `atomistic_to_nadoc.py` test backfill — `low` ✓ REFACTORED + MERGED 2026-05-10
- **Category**: (test) coverage backfill — additive (new test file only)
- **Move type**: additive
- **Where**: `tests/test_atomistic_to_nadoc.py` (new, 810 LOC, **33 tests across 10 classes**: `TestBuildChainMap` 3, `TestExtractFromPdb` 6, `TestBuildPGroOrder` 3, `TestParseGroPositionsHelper` 3, `TestMapPositionsHelper` 2, `TestExtractFromGro` 3, `TestUnwrapMinImage` 3, `TestCompareToDesign` 4, `TestCentroidOffset` 2, `TestBuildReferenceMapAndCompute` 3 + 1 module-level skip-marked test for `extract_from_xtc` env-bound binary XTC)
- **Diff hygiene**: worktree-used: yes (`agent-a9ba8c4f60d2f6367`); production untouched (`git diff HEAD -- backend/` empty)
- **Transparency check**: not applicable (additive)
- **API surface added**: none
- **Pre-metric → Post-metric**:
  - `atomistic_to_nadoc.py` coverage: **55%** (re-measured — Finding #16's stale 18.7% figure updated below) → **96%** (suite-wide; 91% when running test file in isolation; +40pp; calibrated target was 70% per #21, exceeded by 26pp)
  - Tests: +32 (33 with skip-mark)
  - Lint Δ: 0
- **Test honesty (followup-validated)**: zero mocks/patches; 5/5 spot-checked tests use real `np.allclose` / `pytest.approx` / direct equality assertions on synthesized fixtures (no repo-state coupling)
- **Natural ceiling reasoning**: 96% is the natural ceiling. Missing lines: 153 (unreachable due to AB-11B-1, see below), 184-188 (ImportError fallback for MDAnalysis when present — MDAnalysis is a hard dep, branch unreachable), 203-205 (XTC binary path — env-bound, intentionally skip-marked)
- **Apparent-bug flags** (do NOT fix; production untouched):
  1. **AB-11B-1**: `atomistic_to_nadoc.py:142` calls `int(line[22:26])` inside the first `build_p_gro_order` loop without a length guard, while the second loop at L152 has the guard `if len(line) < 26: continue`. For lines shorter than 26 chars matching the `ATOM `/`HETATM` prefix, the first loop will `ValueError`/`IndexError` before the second loop's guard can fire — making line 153 effectively unreachable. Either both loops should guard or first should be folded into second. Cosmetic (real PDB writers always pad columns 1-26). Queued for future cleanup.
- **Linked Findings**: #16 (parent backend coverage audit — metric update: 18.7% → 55% pre-Pass-11), #20 (PDB orchestrator precedent), policy row 2026-05-10 (atomistic family unlock)
- **Note**: Finding #16's `atomistic_to_nadoc.py` baseline coverage figure of **18.7%** was stale; reproduced 55% pre-Pass-11. Likely closed naturally as `tests/test_atomistic_round_trip.py` evolved between the audit (2026-05-09) and this Pass (2026-05-10). Finding #16 metric updated.

### 40. Precondition #15 hardening + xpbd-orphan janitor cleanup — `low` ✓ REFACTORED + MERGED 2026-05-10
- **Category**: (b)+(framework) hygiene + framework reinforcement
- **Move type**: framework-edit + targeted deletions
- **Where**: `REFACTOR_AUDIT.md` precondition #15 body + `frontend/src/scene/loop_skip_highlight.js` + `frontend/src/scene/domain_ends.js`
- **Diff hygiene**: worktree-used: yes (`agent-acaa4736ee12dc948`); files-this-refactor-touched: 3 (REFACTOR_AUDIT.md scope-bounded to precondition #15 body, loop_skip_highlight.js, domain_ends.js); other-files: none
- **Transparency check**: PASS — `applyPhysicsPositions` had zero live callers project-wide post-deletion (the only callers were in main.js's deleted xpbd bootstrap from commit `d262c20`)
- **Part A — Precondition #15 update**: warn → fail. Step 0 shell snippet updated from advisory `# Assert both equal $WORKTREE_PATH; if not, STOP and report.` to literal `if [ "$(pwd)" != "$EXPECTED_WORKTREE" ]; then echo "FATAL: not in worktree $EXPECTED_WORKTREE (got $(pwd))"; exit 1; fi`. Pass 6-B (original) + Pass 11-C (this update) + Pass 10-I (breach reason) all cited inline. Auto-cleaned-worktree edge case explicitly addressed in corollary. Original rationale paragraph preserved verbatim and extended (not replaced).
- **Part B — Janitor deletions**: `applyPhysicsPositions` removed from `loop_skip_highlight.js` (−59 LOC net) and `domain_ends.js` (−25 LOC net). Companion `revertPhysics` methods left intact (out of scope; belong to vulture@60 backlog).
- **Pre-metric → Post-metric**:
  - REFACTOR_AUDIT.md: precondition #15 body updated (no other tracker writes)
  - `loop_skip_highlight.js`: 434 → 375 LOC
  - `domain_ends.js`: 844 → 819 LOC
  - vite build: PASS (3.37s)
  - vitest: 26/26 passes
  - Backend: 998/6/57/9 — bytewise-identical baseline match
  - Lint Δ: 0 (283 → 283)
- **Apparent-bug flags**: none
- **Linked Findings**: #37 (10-I breach), policy row 2026-05-10 (dereliction), audit-log row `2026-05-10 | dereliction` (commit d262c20 that orphaned the deleted methods)

### 41. `cg_to_atomistic.py` test backfill — `low` ✓ REFACTORED + MERGED 2026-05-10
- **Category**: (test) coverage backfill — additive
- **Move type**: additive (new test file only)
- **Where**: `tests/test_cg_to_atomistic.py` (new, ~650 LOC, **24 tests across 7 classes**: `TestFitHelixAxis`, `TestProjectOntoAxis`, `TestSmoothCgPositionsPerDomain`, `TestRefitHelixAxes`, `TestBuildAtomisticModelFromCgSpline`, `TestBuildAtomisticModelFromCg`, `TestSplineOverrideDrivesBackbonePositions` + 1 module-level test)
- **Diff hygiene**: worktree-used: yes (`agent-a742374901efe7367`); production untouched; manager hand-removed 1 unused `Vec3` import post-merge (1 LOC, kept lint Δ=0)
- **Transparency check**: not applicable (additive)
- **API surface added**: none
- **Pre-metric → Post-metric**:
  - `cg_to_atomistic.py` coverage: 0% (Finding #16) → **100%** (73/73 stmts) — natural ceiling reached; pure-math module with no env-bound branches
  - Tests: +24
  - Lint Δ: 0
- **Test honesty (manager-verified)**: zero `Mock`/`MagicMock`/`@patch`/`monkeypatch` (grep returned 0). All assertions use real numpy/string/dict comparisons against expected concrete values.
- **Apparent-bug flags**: none
...
- **Diff hygiene**: worktree-used: yes (`agent-a0fccbcbc9bd19688`); files-this-refactor-touched: 2; other-files: none
- **Transparency check**: PASS — vite build chunk hashes IDENTICAL pre/post (`main-3bcecZdP.js`, `cadnano-editor-DgK0uZy0.js`) — proves rollup tree-shake produced equivalent output
- **API surface added**: 6 named exports from `atom_palette.js` (`ELEMENTS`, `C_HIGHLIGHT`, `C_DIM_FACTOR`, `_dimColor`, `BALL_RADIUS`, `BOND_RADIUS`); previously module-private in atomistic_renderer.js, now package-private (importable by sibling modules). Minor expansion of internal surface only.
- **Visibility changes**: 4 constants + 1 pure colour-math fn changed module-private → package-private. Acceptable — pure data + pure function, no side effects, no closure capture.
- **Dead code removed (precondition #20)**: `C_STRAND_BODY = 0xAAAAAA` was defined but never referenced anywhere in the codebase (manager-verified: grep returned 0 hits). Dropped during extraction.
- **Pre-metric → Post-metric**:
  - atomistic_renderer.js LOC: 516 → 498
  - atom_palette.js: 33 LOC, ZERO imports (cleaner than 09-A which imports `constants.js`)
  - Tests: Δ=0 (no test target for renderer; visual smoke-test deferred to USER TODO)
  - Lint Δ: 0
- **Closure-capture surface map (Pass 13+ planning)**: worker provided substantial bonus value — full audit of remaining extraction surface in atomistic_renderer.js. Key findings:
  - **Module-mutable state**: `_colorMode`, `_vdwScale`, `_strandColors`, `_baseColors` (written by `setColorMode`/`setVdwScale`, read in `_resolveAtomColor`/`_colorForAtom`)
  - **Factory-scoped mutable state**: `_elementMeshes`, `_elementAtoms`, `_elementRadius`, `_bondMesh`, `_bondAtomPairs`, `_lastSel`, `_lastMulti`, `scene`, `_lastData`
  - **Shared THREE scratch**: `_tmpMat`, `_tmpQ`, `_tmpS`, `_tColor`, `_Y_AXIS`, `_ZERO3`, `_SPHERE_GEO`, `_CYLINDER_GEO`
  - **Recommended Pass 13 approach**: introduce a `_state` object inside the factory + extract pure helpers taking `(state, ...args)`. Would unblock 5+ leaf extractions in subsequent passes.
- **Apparent-bug flags**: none
- **USER TODO** (deferred): load saved `.nadoc` with atomistic data; toggle off → vdw → ballstick; verify CPK colours (P=orange, C=dark-grey, N=blue, O=red); click strand → confirm white highlight + dim others; click nucleotide → confirm cascading dim levels. Mark Finding #42 USER VERIFIED if clean.
- **Followup status**: rate-limit-truncated mid-audit. Manager performed direct verification: confirmed zero imports in palette.js, verified `C_STRAND_BODY` is dead via grep, ran vite build (PASS), ran vitest (26/26 PASS).
- **Linked Findings**: #23 (parent frontend audit), #26 (09-A leaf precedent), #36 (10-H leaf precedent), #19 (rule split), #20 (dead-import sweep)

### 43. AB-11B-1 fix: `build_p_gro_order` length-guard — `low` ✓ REFACTORED + MERGED 2026-05-10
- **Category**: (b) bug-fix — single-issue scope
- **Move type**: 2-LOC production change + 2 new tests
- **Where**: `backend/core/atomistic_to_nadoc.py:141-142` (length guard added to first loop); `tests/test_atomistic_to_nadoc.py::TestBuildPGroOrder` (+2 tests)
- **Diff hygiene**: worktree-used: yes (`agent-a4767fb6f6e91bc25`); files-this-refactor-touched: 2; other-files: none
- **Transparency check**: PASS — semantic invariance for well-formed PDB (any line ≥26 chars hits identical code path as before); short-line input previously crashed in first loop before second loop's existing guard could fire
- **API surface added**: none
- **Fix details (Option A — recommended)**:
  ...
  ```
  Chose Option A over Option B: the two loops have different downstream filters (loop 1 builds `block_starts`; loop 2 filters on `P` atom name + consults `block_starts`). Folding would change semantics or require a two-pass over the same data. Option A keeps structure intact.
- **New tests (2)**:
  1. `test_short_atom_line_does_not_raise` — 14-char `"ATOM  "` line interleaved between valid records; verifies no `ValueError` (pre-fix: `int("")` from `line[22:26]` slice in first loop)
  2. `test_short_hetatm_line_skipped_no_spurious_entry` — 20-char `HETATM` line; verifies second-loop guard at L153 is now reachable AND produces no spurious entry in `order`
- **Pre-metric → Post-metric**:
  - `atomistic_to_nadoc.py` coverage: 96% (Pass 11-B) → **97%** suite-wide (line 153 + 155 now covered; missing lines reduced)
  - Tests: 33 → 35 (32 + 1 skip → 34 + 1 skip)
  - Lint Δ: 0
- **Apparent-bug flags**: none new (this fix CLOSES AB-11B-1)
...
| 10-I | [`refactor_prompts/10-I-pathview-palette-and-debug-gating.md`](refactor_prompts/10-I-pathview-palette-and-debug-gating.md) | (b)+(c)+(d) cadnano-editor | (template) | ✓ closed (REFACTORED + MERGED 2026-05-10 [precondition #15 BREACHED — closed by Pass 11-C]); 30 hex-color constants + DBG flag; 23/24 logs gated; pathview.js 4076 → 4067 — Finding #37 |
| 11-A | [`refactor_prompts/11-A-atomistic-helpers-extract.md`](refactor_prompts/11-A-atomistic-helpers-extract.md) | (c)+(test) — atomistic pure-helper extraction (first under unlock policy) | (template) | ✓ closed (REFACTORED + MERGED 2026-05-10); 18 functions + 23 constants → atomistic_helpers.py (483 LOC, 99% cov, 61 tests); atomistic.py 2602 → 2204 (−398 LOC, 15%); leaf imports `__future__`+`math`+`numpy` ONLY — Finding #38 |
| 11-B | [`refactor_prompts/11-B-atomistic-to-nadoc-test-backfill.md`](refactor_prompts/11-B-atomistic-to-nadoc-test-backfill.md) | (test) coverage backfill | (template) | ✓ closed (REFACTORED + MERGED 2026-05-10); 33 tests (32 pass + 1 env-skip); atomistic_to_nadoc.py **55% → 96%** (Finding #16 metric updated; original 18.7% figure was stale); surfaced AB-11B-1 (build_p_gro_order length-guard mismatch) — Finding #39 |
| 11-C | [`refactor_prompts/11-C-precondition-15-harden-and-janitor.md`](refactor_prompts/11-C-precondition-15-harden-and-janitor.md) | (b)+(framework) hygiene | (template) | ✓ closed (REFACTORED + MERGED 2026-05-10); precondition #15 hardened warn → literal `exit 1` (closed Pass 10-I framework debt) + 2 xpbd-orphan `applyPhysicsPositions` methods deleted (−84 LOC) — Finding #40 |
...
### Followup 11-A — atomistic helpers extract (eval 2026-05-10)

**Worker outcome confirmation**: REFACTORED — APPROVED.
**Worktree audit context**: `/home/joshua/NADOC/.claude/worktrees/agent-aea8de38905ba144e` (intact).
**Diff vs claimed-touched files**: 3 claimed | 3 observed. `git diff HEAD -- backend/` shows only `atomistic.py` modified + `atomistic_helpers.py` new.

**Metric audit**
- lint: claimed Δ=0; observed 283 errors post (matches 283 baseline). PASS.
- test: 61/61 passes in `test_atomistic_helpers.py` (0 failed/skipped). PASS.
- full suite: 6 fails + 9 errors matches baseline (5 animation + 1 seamless flake + 9 atomistic_round_trip env errors).
- LOC: atomistic.py 2204 (claimed), atomistic_helpers.py 483, tests 621. All confirmed.
- Coverage on `atomistic_helpers.py`: 99% (217 stmts, 1 miss at line 336 — defensive `if g is None: continue` guard in `_rb_grad_propagate`; defensible).

**Leaf-purity verification (precondition #19)**: PASS — atomistic_helpers.py imports EXACTLY `from __future__ import annotations`, `import math as _math`, `import numpy as _np`. Full grep confirms no other imports anywhere in file. **Tighter than allowed** — the prompt permitted `backend.core.{models,constants}`; worker's pure-math helpers needed neither.

**Verbatim move (3 funcs sampled)**: `_normalise`, `_backbone_bridge_cost`, `_arc_ctrl_pt` bodies byte-identical vs HEAD.

**Re-import shim (precondition #20)**: 23 names re-imported at atomistic.py L51-75; all 23 have ≥2 references in atomistic.py. **Dead-import sweep correctly applied** — 18 of 41 moved symbols are NOT re-imported because their only call sites are within atomistic_helpers.py itself (gradient siblings consuming each other). No `# noqa: F401` needed.

**Test honesty**: zero `Mock|MagicMock|@patch|patch(`. Spot-checked 5 finite-difference tests (`TestSpinRotationDeriv.test_finite_difference`, `TestBackboneBridgeCostGrad.test_finite_difference_match`, `TestGlycosidicCostGrad.test_finite_difference`, `TestRepulsionCostGrad.test_finite_difference`, `TestRbPairRepulsionGrad.test_finite_difference`) — all compute analytic via grad function, numerical via central-difference, assert `np.allclose` / `pytest.approx` with atol 1e-5 to 1e-7.

**Out-of-scope preservation**: `build_atomistic_model` (L666), `_apply_backbone_torsions` (L545), `_build_extra_base_atoms` (L1812), `merge_models` (L376), `_atom_frame` (L442), `_apply_phosphate` (L1291), `_minimize_*` (L1152, 1331, 1441, 1580) all still in atomistic.py. `_SUGAR` (L115), `_SUGAR_BONDS` (L141), `_ATOMISTIC_PHASE_OFFSET_RAD` untouched.

**Apparent-bug flags validated**: none.

**Prompt evaluation**: scope right. The "≥3 functions or ≥150 LOC" floor was easily exceeded (18 funcs, 398 LOC). The pure-helper definition with explicit import allowlist was load-bearing. The dead-import sweep instruction (#20) caused worker to drop ~18 names from the re-import block where only intra-helpers usage exists — correct application but worth flagging for future audits.

**Proposed framework edits**
1. Followup template addition: explicit "moved-but-not-reimported subset" check. Workers correctly applying #20 will move N symbols and re-import M < N. Auditor should verify N−M dead-imports have zero atomistic.py call sites (not just documentation comments). I did this manually; could be a checklist item.
2. The "61 tests across 22 classes for 18 functions moved (3.4 tests/function)" density is a healthy precedent for atomistic-family follow-on extracts. Future atomistic Pass 12+ prompts can cite this as the floor.

### Followup 11-B — atomistic_to_nadoc tests (eval 2026-05-10)

**Worker outcome confirmation**: REFACTORED — APPROVED.
**Worktree audit context**: `/home/joshua/NADOC/.claude/worktrees/agent-a9ba8c4f60d2f6367` (locked, intact).
**Diff vs claimed-touched files**: 1 claimed (untracked `tests/test_atomistic_to_nadoc.py`) | 1 observed; `git diff HEAD -- backend/` empty.

**Metric audit**
- lint: claimed Δ=0; observed Δ=0 (283 errors before/after).
- test (worktree): 1030 passed / 6 failed / 9 errors / 58 skipped — failure set matches master baseline. Delta = +32 passes, +1 skip (worker's new tests). All baseline failures are 10hb-fixture FileNotFound (env-bound, baseline-stable).
- coverage on `atomistic_to_nadoc.py`: pre = **55%** (worker's measurement reproduced — Finding #16's 18.7% is stale); post = **96%** suite-wide (91% in isolation).
- Missing lines post: 153 (unreachable per AB-11B-1), 184-188 (ImportError fallback for MDAnalysis hard-dep), 203-205 (XTC binary path env-bound, intentionally skip-marked) — all match worker diagnosis.

**Pre-coverage discrepancy (55% vs Finding #16's 18.7%)**: REAL. Reproduced 55% on master with `atomistic_to_nadoc.py` removed from suite-wide coverage. The 36-pt gap likely closed naturally as `tests/test_atomistic_round_trip.py` evolved between Finding #16's audit (2026-05-09) and Pass 11-B (2026-05-10). **Manager updates Finding #16: `atomistic_to_nadoc.py` baseline = 55% pre, not 18.7%.**

**Test honesty (precondition #21)**: PASS. Grep for `Mock|MagicMock|@patch|patch(` returns zero hits. Spot-checked 5 random tests (260, 391, 471, 666, 781): all use synthesized fixtures (`_build_synthetic_model`, `_format_pdb_atom`, `tmp_path`-written .gro), all assert concrete expected values via `np.allclose` / `pytest.approx` / direct `==` on tuples. None read repo files for inputs.

**Production-untouched**: confirmed empty diff against `backend/`.

**Apparent-bug AB-11B-1 (build_p_gro_order length-guard mismatch)**: VALIDATED. `atomistic_to_nadoc.py:142` calls `int(line[22:26])` inside the first loop with no `len(line) < 26` guard, while the second loop at L152 has the guard. For lines shorter than 26 chars matching the `ATOM `/`HETATM` prefix, the first loop will `ValueError`/`IndexError` before the second loop's guard fires — making L153 effectively unreachable from any short-line input. Cosmetic. Document in audit log; do NOT fix (out of scope, future cleanup pass).

**`_SUGAR` / Finding #18**: untouched — zero `_SUGAR` references in test file.

**Skip-marked test**: `test_extract_from_xtc_requires_binary_fixture` uses `@pytest.mark.skip(reason="extract_from_xtc requires a real binary XTC trajectory; covered by tests/test_atomistic_round_trip.py when 10hb fixtures are present (env-bound, Pass 11-B prompt §Stop conditions).")` — clear, not a silent skip.

**Calibrated-target audit**: Prompt's calibrated target was `min(70%, natural_ceiling)`. Pre = 55%, post = 96%. Worker delivered 26pp above the 70% default. Calibration was conservative — natural ceiling for this module is ~96% (capped only by binary-XTC and MDAnalysis-import-failure fallbacks, both legitimately env-bound). Future calibrations on similar I/O-with-fallback modules should target 90%+ when fallback branches are documentably env-bound.

**Apparent-bug flags validated**: AB-11B-1 (build_p_gro_order missing length guard at L142, makes L153's guard partially unreachable).

**Prompt evaluation**: scope right. Worker delivered 33 tests / 32 pass / 1 documented skip / 96% coverage in one pass. No unanticipated stops.

**Proposed framework edits**
1. Update Finding #16: `atomistic_to_nadoc.py` baseline coverage = **55%**, not 18.7%. (Applied in Pass 11 closeout.)
2. Calibrated-target heuristic refinement (precondition #21): for I/O modules with documentably env-bound fallback branches (ImportError-guards, optional binary deps), set target to natural ceiling rather than capping at 70%. Cite this 96%-vs-70% gap as the example.
3. Document AB-11B-1 in REFACTOR_AUDIT.md apparent-bug log for a future cleanup pass; do NOT bundle into a coverage backfill.

### Followup 11-C — Precondition #15 harden + xpbd-orphan janitor (eval 2026-05-10)

**Worker outcome confirmation**: REFACTORED — APPROVED.
**Worktree audit context**: `/home/joshua/NADOC/.claude/worktrees/agent-acaa4736ee12dc948` (intact, HEAD at `d262c20`, 3 dirty files).
**Diff vs claimed-touched files**: 3 claimed | 3 observed (REFACTOR_AUDIT.md +9/-3, domain_ends.js -25, loop_skip_highlight.js -59 net).

**Metric audit**
- lint: claimed Δ=0 (283→283), observed PASS.
- vitest: 26/26 PASS in 3 files.
- vite build: `built in 3.37s` PASS.
- backend test: 998/6/57/9 — exact bytewise match to baseline (`diff /tmp/11C_pre_failset.txt /tmp/11C_post_failset.txt` empty).

**Scope audit**
- Diff stat shows ONLY: REFACTOR_AUDIT.md, frontend/src/scene/domain_ends.js, frontend/src/scene/loop_skip_highlight.js. No tracker writes outside precondition #15 body.
- Locked areas (`_PHASE_*`, atomistic, `make_bundle_continuation`): not touched.
- `applyFemPositions`, `clearFemOverlay`, `revertPhysics` all preserved (verified via grep).

**Part A — precondition #15 deep checks**
- Literal `exit 1` present (worktree REFACTOR_AUDIT.md L401-404). ✓
- Corollary present, explicitly cites auto-cleaned-worktree edge case. ✓
- Pass 6-B + Pass 11-C + Pass 10-I all cited inline in parenthetical. ✓
- Original rationale paragraph preserved verbatim and extended (not replaced). ✓
- `git diff HEAD -- REFACTOR_AUDIT.md` confirms only precondition #15 block was edited. ✓

**Part B — janitor deep checks**
- Live-tree grep for `applyPhysicsPositions` across `frontend/`, `backend/`, `tests/`: 0 matches post-deletion. ✓
- domain_ends.js diff: deletes lines 686-712 (the JSDoc-prefixed `applyPhysicsPositions(updates)` method). ✓
- loop_skip_highlight.js diff: deletes "Physics positions" section header + JSDoc + `function applyPhysicsPositions(updates)` body; also drops `applyPhysicsPositions` from `getEntries` JSDoc + module exports. ✓
- Both deletions are XPBD-bootstrap orphans from commit `d262c20`; no unrelated code touched.
