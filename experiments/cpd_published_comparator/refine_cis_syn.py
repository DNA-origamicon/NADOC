...
PARENT = ART / "cpd-sugar-boundary-refinement-v4"
    ...
    for e in (1, 2):
        ...
        cases.append(
            dict(
                ...
                target=checked(report["sources"][0]),
                psf_path=folder / "fragment.psf",
                system_path=folder / "system.xml",
                minimum=folder / "minimum_A.txt",
            )
        )
    manifest = json.loads(
        (
    ...
    cases.append(
        dict(
            ...
            names=[
                a["stable_atom_key"]
                for a in json.loads(
                    checked(manifest["sources"]["stable_atom_map"]).read_text()
                )
            ],
            target=checked(manifest["sources"]["target_geometry"]),
            psf_path=ART / "cpd-published-local-benchmarks-no-c5-planarity-v1/core.psf",
            system_path=ART / "cpd-angle-refinement-v2/candidate.xml",
            minimum=ART / "cpd-angle-refinement-v2/minimum_A.txt",
        )
    )
    for c in cases:
        c["qm"] = np.array(
        ...
        c["initial"] = np.loadtxt(c["minimum"])
...
def main(root, all_angles, typed_input=None, warm_start=None, all_bonds=False):
    ...
    for c in cases:
        ...
        if c["endpoint"]:
            shutil.copy2(PARENT / c["id"] / "fragment.rtf", out / "fragment.rtf")
        ...
        orders = {}
        for e in (1, 2):
            orders[f"{e}:C5"] = [f"{e}:C4", f"{e}:C6", f"{e}:C7", f"{3 - e}:C5"]
            orders[f"{e}:C6"] = [f"{e}:N1", f"{e}:C5", f"{e}:H6", f"{3 - e}:C6"]
        if c["endpoint"]:
            e = c["endpoint"]
            for center, seq in [
                ("C1'", ["O4'", "C2'", "N1", "H1'"]),
