...
def load_cases():
    ...
    curves = json.loads((ART / "cpd-anti-remote-water-curves-v1/assessment.json").read_text())[
        "records"
    ]
    cases = []
    for e in (2,):
        ep = ART / f"cpd-anti-remote-esp-v1/endpoint-{e}"
        ...
        for curve in curves:
            if curve["identity"]["model_id"] != job["model_id"]:
                continue
            if not curve["passed"]:
                continue  # Explicitly retain unbracketed curves outside minimum metrics.
            points = []
...
if __name__ == "__main__":
    root = ART / "cpd-anti-remote-water-transfer-v1"
    ...
    write(
        ...
        dict(
            ...
            sources=[source(ART / "cpd-anti-remote-water-curves-v1/assessment.json")]
