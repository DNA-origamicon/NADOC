...
write(root/'additional_targets.json',dict(esp=source(remote_folder/'esp_audit.json'),weight='Each ESP conformer has equal block weight; original water blocks unchanged',scope='Development fit; new conformer is now training, not independent validation; Five remote water sites added separately; 1-O4 mixed contact excluded'))
from experiments.cpd_anti_additive.remote_water_transfer import load_cases as load_remote_cases
remote_water = load_remote_cases()[0]
assert remote_water['names']==cases[1]['names'] and len(remote_water['curves'])==5
assert all(c['site']!='1-O4' for c in remote_water['curves'])
W=[];wy=[]
for curve in remote_water['curves']:
    points=curve['points'];i=int(np.argmin([v['target'] for v in points]));assert 0<i<len(points)-1
    for point in points[i-1:i+2]:W.append(point['offset_row']);wy.append(point['target']-point['offset_lj'])
W=np.array(W);wy=np.array(wy)
blocks.append(W@cases[1]['map']/np.sqrt(len(wy)))
targets.append((wy-W@cases[1]['charges'])/np.sqrt(len(wy)))
write(root/'remote_water_targets.json',dict(source=source(ART/'cpd-anti-remote-water-curves-v1/assessment.json'),independent_review=source(ART/'cpd-anti-remote-water-curves-v1/independent_review.json'),excluded_site='1-O4: extended minimum is mixed sugar contact, not clean target contact',contact_review=source(ART/'cpd-anti-remote-water-extension-v2/contact_review.json'),scope='Five remote water curves now training; preceding frozen transfer assessment retained'))
A = np.vstack(blocks)
...
for strength in [1, 10, 100]:
    ...
    records.append(
        dict(
            remote_water_metrics=metrics(remote_water,qr),
...
write(
    ...
    dict(
        records=records,
        simulation_ready=False,
        scope="Exploratory shared base charges across three conformers of two fragments; fixed sugar/caps/LJ; remote water now training, geometry validation pending; no export or acceptance",
    ),
)
for r in records:
    print(
