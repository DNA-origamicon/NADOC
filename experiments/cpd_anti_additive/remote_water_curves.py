...
def prepare(root):
    ...
    for e in (2,):
        cp = (
            ART
            / f"cpd-anti-remote-water-calibration-service-v1/endpoint-{e}-independent-calibration-review.json"
        )
        cal = json.loads(cp.read_text())
        assert cal["passed"]
        checked(cal["protocol"])
        pp = ART / f"cpd-anti-remote-water-curve-plans-v1/endpoint-{e}-plan.json"
        ...
        for site in p["sites"]:
            for i, distance in enumerate(site["distances_angstrom"]):
                ...
                existing = (
                    ART / f"cpd-anti-remote-water-calibration-v1/endpoint-{e}/{site['id']}/df"
                )
                reuse = False
...
def run(root):
    ...
    if not all(a["passed"] for a in audits):
        raise RuntimeError("Native calculations complete but one or more curve audits failed; preserve results")
    print(
        "Curve audits",
