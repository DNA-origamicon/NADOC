"""Anti core transfer diagnostic using the published additive hypothesis, not syn validation."""
...
def main(candidate, root, omit_c5_planarity=False):
    ...
    policy_path = Path(
        "experiments/cpd_published_comparator/local_benchmark_policy_v1.json"
    ).resolve()
    policy = json.loads(policy_path.read_text())
    shutil.copyfile(policy_path, root / "policy.json")
    shutil.copyfile(__file__, root / "executed_source.py")
    from experiments.cpd_drude_recovery.campaign import OLD, audit_graph
    ...
    manifest_path = checked(campaign["training_datasets"][0]["response_manifest"])
    ...
    graph_system = mm.XmlSerializer.deserialize(
        checked(manifest["sources"]["linear_fit_system"]).read_text()
    )
    # Reconstruct the graph from the independent anti bonded plan, never the old
    # Drude evaluator whose syn connectivity was invalidated.
    old_audit = OLD / "bundle/results/anti_promoted_bonded_fit_plan.json"
    plan = json.loads(old_audit.read_text())
    terms = [t for t in plan["transfer_candidates"] if t["category"] == "bonds"]
    terms += [
        t
    ...
    named_pairs = sorted({tuple(sorted(t["atoms"])) for t in terms})
    pairs = [tuple(idx[n] for n in pair) for pair in named_pairs]
    assert len(pairs) == 38 and graph_system.getNumParticles() == 36
    registry = json.loads(
        Path("backend/data/forcefield/photoproduct_registry.json").read_text()
    )
    write(root / "graph_audit.json", audit_graph(names, pairs, registry))
    cap = {
        "CM": ("CG331", -0.27),
        "HCM1": ("HGA3", 0.09),
        "HCM2": ("HGA3", 0.09),
    ...
    assignments = [
        cap[n.split(":")[1]]
        if n.split(":")[1] in cap
        else ATOMS[{"C7": "C5M"}.get(n.split(":")[1], n.split(":")[1])]
        for n in names
    ]
    ...
    masses = [
        l
        for l in (FF / "par_all36_cgenff.prm").read_text().splitlines()
        if l.startswith("MASS") and l.split()[2] in wanted
    ]
    rtf = [
        "* Explicit capped published-model test hypothesis\n*\n36 1",
        *masses,
        "AUTO ANGLES DIHE",
        "RESI CPDM 0.0",
        "GROUP",
    ...
    for i, (n, pos) in enumerate(zip(pdbnames, xyz), 1):
        pdb.append(
            f"ATOM  {i:5d} {n:4s} CPDM    1    {pos[0]:8.3f}{pos[1]:8.3f}{pos[2]:8.3f}  1.00  0.00      CORE"
        )
...
""")
    p = subprocess.run(
        ["psfgen", str(root / "build.tcl")], capture_output=True, text=True
    )
    ...
    for interpretation in ("first", "last"):
        ...
        (root / f"{interpretation}_system.xml").write_text(
        ...
        rigid = np.column_stack(
            [np.tile(v, (36, 1)).ravel() for v in np.eye(3)]
        ...
        bonds = [
            {
                ...
                "error_A": float(
                    np.linalg.norm(final[a] - final[b])
                    - np.linalg.norm(xyz[a] - xyz[b])
                ),
                "heavy": all(
                    not names[k].split(":")[1].startswith("H") for k in (a, b)
                ),
            }
            for a, b in pairs
        ]
        ...
        for a in psf.angle_list:
            ...
            angles.append(
                {
                    "atoms": [names[k] for k in indices],
                    "error_deg": angle(final, indices) - angle(xyz, indices),
                    "heavy": all(
                        not names[k].split(":")[1].startswith("H") for k in indices
                    ),
                }
            )
        centers = []
        for res in (1, 2):
            for center, neighbors in [
                ("C5", [f"{res}:C4", f"{res}:C6", f"{res}:C7", f"{3 - res}:C6"]),
                ("C6", [f"{res}:N1", f"{res}:C5", f"{res}:H6", f"{3 - res}:C5"]),
            ]:
        ...
        checks = {
            "bond_geometry": bond_max <= 0.03,
            "angle_geometry": angle_max <= 3,
            "stereochemistry": all(c["preserved"] for c in centers),
            "stationary": bool(np.max(np.abs(g)) < 0.001),
            "positive_internal_curvature": bool(eigenvalues.min() > -0.001),
        }
        report = {
            "interpretation": interpretation,
            "checks": checks,
            "geometry_minimum_benchmarks_pass": all(checks.values()),
            "local_acceptance": "incomplete",
            "max_bond_error_A": bond_max,
            "max_angle_error_deg": angle_max,
            "max_heavy_bond_error_A": max(
                abs(b["error_A"]) for b in bonds if b["heavy"]
            ),
            "max_heavy_angle_error_deg": max(
                abs(a["error_deg"]) for a in angles if a["heavy"]
            ),
            "initial_energy": initial_e,
            "minimum_energy": e,
            "max_force": float(np.max(np.abs(g))),
            "minimum_internal_curvature": float(eigenvalues.min()),
        ...
        results.append(
            {
                ...
                if k not in ("bonds", "angles", "centers", "parameter_overlay_warnings")
            }
        )
    ...
    write(
        root / "assessment.json",
        {
            ...
            "transfer_hypothesis": "Published cis-syn additive assignments on independently audited anti graph; not anti electrostatic validation",
            "selection_disclosure": "C5 planarity removal motivated by first-round geometry diagnostic; this geometry is now development evidence for the corrected candidate, not fresh validation."
            if omit_c5_planarity
            else "Frozen original reconstruction benchmark",
            "sources": [
                source(p)
                for p in (
                    manifest_path,
