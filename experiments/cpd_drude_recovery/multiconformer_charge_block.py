"""One predeclared fixed-induced charge step across two development geometries."""
import importlib
import json
...
root = Path('.development-artifacts/cpd-multiconformer-charge-block-v1').resolve()
...
write(root/'plan.json', {'simulation_ready': False, 'gate_effect': 'none',
    'scope': 'One fixed-induced charge block using two exposed development geometries, then full polarization relaxation. No weight search, response refit, or release acceptance.',
    'cycles': 1, 'equal_geometry_weights': True, 'esp_sigma_au': .001, 'dipole_sigma_au': .1,
...
def objective(q):
    ...
    gradient = .2*(q-prior)/(len(q)*sigma**2)
    for c in cases:
        er = c['A']@q-c['adjusted_esp']
        dr = c['D']@q-c['adjusted_dip']
        value += (np.mean(er**2)/.001**2+np.mean(dr**2)/.1**2)/len(cases)
        gradient += (2*c['A'].T@er/(len(er)*.001**2)+2*c['D'].T@dr/(3*.1**2))/len(cases)
    return value, gradient
opt = minimize(objective,q0,jac=True,method='SLSQP',bounds=[(-1.2,1.2)]*36+[(-.5,0)]*8,
    constraints={'type':'eq','fun':lambda q:C@q-d,'jac':lambda q:C},options={'maxiter':3000,'ftol':1e-12})
assert opt.success and abs(C@opt.x-d).max() < 1e-8
...
write(root/'assessment.json', {'simulation_ready':False,'gate_effect':'none','records':records,
    'iterations':int(opt.nit),'constraint_error':float(abs(C@opt.x-d).max()),
