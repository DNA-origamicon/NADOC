# design-automation log — archive (oracle detail, lessons, per-loop history)

Split out of `design_automation_log.md` on 2026-07-09 for context economy.
...
| `assert_extension_present` (Phase 5 — terminal fluorophore/modification extension) | a terminal `StrandExtension` (added ACGTN sequence and/or a fluorophore/quencher/biotin `modification` key — the Fluorescence/FRET view glows the modification ones) is wired to the named strand end and SURVIVES a `.nadoc` round-trip. Extensions live on their own `__ext_` helix, OUTSIDE the strand graph, so `canonical_topology` doesn't fingerprint them (the connections/duplex/attachment blind-spot again) — only re-reading after a real export→import proves the label persisted + still resolves. Asserts on BOTH in-memory + `roundtrip_nadoc`: (1) the extension exists under `ext_id`, on `strand_id` at `end`; (2) its `modification`/`sequence` match (when given); (3) `validate_design` raises no "Strand extension" issue (strand resolves, modification is a known key, sequence is ACGTN). Can-go-red: dropped extension (1) / wrong end-or-content (2) / unknown modification or dangling strand (3) / a label the import lost (round-trip). | `tests/automation_harness.py` (`assert_extension_present`); `backend/api/headless_build.py` (`add_strand_extension`, wraps `POST /design/extensions`, coverage 54→55); `backend/core/validator.py` (new strand-extension check) | any terminal-annotation-on-a-strand pin (5'/3' mods, handles, labels); the "outside-strand-graph label survives round-trip + validates" pattern reusable for any `__ext_`-style overlay |
| `assert_duplex_relocated` (Phase 4b — different-length duplex relocation) | a DIFFERENT-length `Duplex` (no equal-length binding) relocated the DRIVEN overhang's WHOLE domain onto the DRIVER's helix at the paired-window range, WITHOUT stretching the short driven to the long driver's length. The duplex-graph analog of `assert_direct_binding_applied` for the binding-less path (`hb.connect_duplex` on different-length overhangs). Asserts on BOTH the in-memory design and its `roundtrip_nadoc` re-import: (1) both overhangs survive (neither consumed); (2) exactly one duplex joins the pair, `bound` True with a populated `prior_driven_topology` (revertible), driver side names `driver_oh_id`; (3) the driven relocated onto the driver's helix keeping its OWN `driven_length_bp` (the load-bearing pin — a paired-window target, NOT the driver's full domain, so a 10 bp driven stays 10 on a 24 bp driver) + driven `OverhangSpec.helix_id` moved; (4) no orphaned helices + no improper crossover (`validate_design` agrees — the relocated bond is a ForcedLigation). Can-go-red: a connect that didn't relocate (driven still on its own helix) / a driven stretched to the driver's length (clause 3 length pin) / a missing/unbound duplex. | `tests/automation_harness.py` (`assert_duplex_relocated`); `backend/api/headless_build.py` (`connect_duplex`, wraps `POST /design/duplexes/connect`); drives `backend/core/duplex.relocate_duplex` + `binding_relax.compute_bind_topology(target_*_override=…)` | any binding-LESS relocation whose driven must keep its own length onto a longer partner's window; the "relocated-but-not-stretched" length pin reusable for partial-pairing / toehold geometry |
...
| `assert_corner_folded` (CORNER — mitred-corner primitive + phase-aware optimizer, 2026-07-08) | a headless mitred 90° corner is correct across ALL THREE layers — the FIRST oracle to validate a build with the design-layer CLASH detector as a metric. On the `build_corner` output (two SQUARE sheets, mitre-trimmed, sheet B folded 180° about the miter diagonal, N cross-seam forced ligations): (1) **corner angle** — the angle between the two flush faces (posed mean helix axes via `deformed_helix_axes`) is within tol of 90° (fold happened + landed square; can-go-red on a flat/unfolded build → ~0°); (2) **seam count** == N forced ligations; (3) **every seam bond short** — each posed FL backbone stretch (`forced_ligation_stretches`, measured on `deformed_nucleotide_positions`) < `max_stretch_nm` (1.0); (4) **the optimizer helped** — total posed stretch ≤ the unoptimised uniform baseline (proves the phase-aware search is not a no-op); (5) **no worse steric clashes** — `steric_clash_count` (= `clash.clash_report` MINUS the FL-partner pairs) ≤ the unoptimised baseline; (6) **the fold is logged** — a `cluster_op` feature-log entry records sheet-B's pose (load-bearing: the pose lives off the strand graph, `canonical_topology` blind); (7) **round-trip stable** + all N FL records persist. **KEY calibration insight:** a GOOD forced ligation (~0.3 nm) is itself a sub-0.65 nm "clash" to the detector (FL partners are ~20 nm apart STRAIGHT so the straight-vs-posed exclusion does NOT drop them) — so raw `clash_report.count` is ANTI-correlated with ligation quality; the corner metric must EXCLUDE the seam FL bonds (`steric_clash_count`), which is what "ideally 0 after seam design" means. Can-go-red: unfolded (1) / wrong FL count (2) / overstretched bond (3) / no-op optimizer (4) / new folding collisions (5) / unlogged fold (6) / dropped FL on reload (7) | `tests/automation_harness.py` (`assert_corner_folded`); `backend/api/headless_corner_build.py` (`build_corner`/`forced_ligation_stretches`/`corner_face_angle_deg`/`steric_clash_count`/`resolve_corner_spec`); composes `backend/core/clash.clash_report` + `deformation.deformed_nucleotide_positions`/`deformed_helix_axes` + `roundtrip_nadoc` (AF-1) | any folded-primitive build whose validation needs the clash detector as a metric (multi-corner boxes, folded barrels); the "exclude designed bonds from the clash count" rule reusable for ANY clash check on a design with forced ligations / placed crossovers spanning a fold |
