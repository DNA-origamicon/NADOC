...
def test_caps_and_spaces_edits():
    ...
    for bp in core_candidates(base, h):  # make the WHOLE helix a hotspot
        field[(h.id, bp)] = 100.0
    edits = identify_finetune_edits(
        base, field, OVER, max_edits=3, sigma=1.0, min_spacing=8
    )
...
def test_apply_add_and_remove():
    ...
    removed = apply_finetune_edit(
        skips, {"helix_id": "A", "bp_index": 28, "op": "remove"}
    )
    assert removed["A"] == [10]  # nearest (30) removed
    # remove the last skip on a helix drops the helix entry
    cleared = apply_finetune_edit(
        {"B": [20]}, {"helix_id": "B", "bp_index": 20, "op": "remove"}
    )
...
def test_greedy_does_no_harm_when_no_hotspots(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda design, ws, **k: {
            ...
            "dev_max": 2.0,
            "dev_mean": 1.0,
            "deviation_by_bp": _field(conv),
            "shape_profile": OVER,
        },
    )
    r = st.greedy_finetune_skips(conv, "/tmp", max_edits=3, tol_twist_deg=8.0)
    assert (
        r["status"] == "done" and r["edits_kept"] == [] and r["converged_skips"] == orig
    )
...
def test_greedy_accepts_improving_edit(monkeypatch):
    ...
    placed = any(
        any(abs(b - spot) <= 1 for b in bps) for bps in r["converged_skips"].values()
    )
...
def test_greedy_reverts_when_twist_breaks_tolerance(monkeypatch):
    ...
    def fake(design, ws, **k):
        ...
        return {
            "twist": twist,
            "dev_max": 5.0 if calls["n"] == 1 else 1.0,
            "dev_mean": 1.0,
            "deviation_by_bp": field,
            "shape_profile": OVER,
        }
