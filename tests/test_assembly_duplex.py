...
def test_assembly_duplex_roundtrip_on_assembly():
    dx = AssemblyDuplex(
        name="AD1",
        left=AssemblyDuplexEnd(instance_id="inst-A", overhang_id="oh-A_5p",
                               start_bp=0, end_bp=7),
        right=AssemblyDuplexEnd(instance_id="inst-B", overhang_id="oh-B_3p",
                                start_bp=0, end_bp=7),
    )
...
def test_assembly_duplex_rejects_unequal_length():
    with pytest.raises(ValueError):
        AssemblyDuplex(
            left=AssemblyDuplexEnd(instance_id="i", overhang_id="oA",
                                   start_bp=0, end_bp=7),
            right=AssemblyDuplexEnd(instance_id="j", overhang_id="oB",
                                    start_bp=0, end_bp=3),
        )
...
def test_assembly_duplex_rejects_self_pair_overlap():
    with pytest.raises(ValueError):
        AssemblyDuplex(
            left=AssemblyDuplexEnd(instance_id="i", overhang_id="oA",
                                   start_bp=0, end_bp=3),
            right=AssemblyDuplexEnd(instance_id="i", overhang_id="oA",
                                    start_bp=3, end_bp=0),
        )
...
def test_migration_skips_unresolved_binding():
    ...
    bad = AssemblyOverhangBinding(
        name="ABx",
        instance_a_id="inst-A", sub_domain_a_id="nope", overhang_a_id="oh-A_5p",
        instance_b_id="inst-B", sub_domain_b_id="nope", overhang_b_id="oh-B_3p",
    )
...
def test_classify_full_watson_crick():
    ...
    a = a.model_copy(update={
        "duplexes": synthesize_assembly_duplexes_from_bindings(a)})
