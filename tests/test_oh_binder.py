...
from backend.core.models import (
    Design, Helix, Strand, Domain, Direction, StrandType, Vec3,
    OverhangSpec, SubDomain, OverhangConnection, NADOC_SUBDOMAIN_NS,
)
from backend.core.sequences import (
    assign_staple_sequences, is_watson_crick_complement,
)

client = TestClient(app)


def _design_with_overhang_and_complement(*, binder_type=StrandType.SCAFFOLD,
                                         binder_binds=None, overhang_seq=None):
    ...
    h = Helix(id="h0", axis_start=Vec3(x=0, y=0, z=0), axis_end=Vec3(x=0, y=0, z=20),
              length_bp=40, bp_start=0)
    oh_dom = Domain(helix_id="h0", start_bp=20, end_bp=27, direction=Direction.FORWARD,
                    overhang_id="ovhg_test")
    staple = Strand(id="stap", strand_type=StrandType.STAPLE, domains=[oh_dom])
    spec = OverhangSpec(
        id="ovhg_test", helix_id="h0", strand_id="stap", sequence=overhang_seq,
        sub_domains=[SubDomain(
            id=str(uuid.uuid5(NADOC_SUBDOMAIN_NS, "ovhg_test:whole")),
            name="a", start_bp_offset=0, length_bp=8)],
    )
    binder = Strand(id="bind", strand_type=binder_type,
                    domains=[Domain(helix_id="h0", start_bp=27, end_bp=20,
...
def test_convert_tags_untagged_partner_as_overhang():
    ...
    h = Helix(id="h0", axis_start=Vec3(x=0, y=0, z=0), axis_end=Vec3(x=0, y=0, z=20),
              length_bp=40, bp_start=0)
    staple = Strand(id="stap", strand_type=StrandType.STAPLE,
                    domains=[Domain(helix_id="h0", start_bp=20, end_bp=27,
                                    direction=Direction.FORWARD)])
    binder = Strand(id="bind", strand_type=StrandType.SCAFFOLD,
                    domains=[Domain(helix_id="h0", start_bp=27, end_bp=20,
                                    direction=Direction.REVERSE)])
    d = Design(helices=[h], strands=[staple, binder])
    d2 = convert_strand_to_binder(d, "bind")
    new_ovhg_id = next(s for s in d2.strands if s.id == "bind").domains[0].binds_overhang_id
...
def test_convert_no_partner_raises():
    h = Helix(id="h0", axis_start=Vec3(x=0, y=0, z=0), axis_end=Vec3(x=0, y=0, z=20),
              length_bp=40, bp_start=0)
    lonely = Strand(id="lonely", strand_type=StrandType.SCAFFOLD,
                    domains=[Domain(helix_id="h0", start_bp=0, end_bp=7,
                                    direction=Direction.FORWARD)])
...
def test_convert_back_keeps_preexisting_overhang():
    ...
    d2 = convert_strand_to_binder(d, "bind")
    assert next(s for s in d2.strands if s.id == "bind").domains[0].binds_overhang_id == "ovhg_test"
    d3 = convert_binder_to_scaffold(d2, "bind")
    assert next(s for s in d3.strands if s.id == "bind").strand_type == StrandType.SCAFFOLD
    assert [o.id for o in d3.overhangs] == ["ovhg_test"]   # pre-existing kept
    assert next(s for s in d3.strands if s.id == "stap").domains[0].overhang_id == "ovhg_test"
...
def test_convert_to_scaffold_endpoint_404():
    design_state.set_design(_design_with_overhang_and_complement())
    assert client.post("/api/design/strands/nope/convert-to-scaffold").status_code == 404
...
def test_tag_painted_binder_over_existing_overhang():
    ...
    fresh = Strand(id="painted", strand_type=StrandType.STAPLE,
                   domains=[Domain(helix_id="h0", start_bp=27, end_bp=20,
                                   direction=Direction.REVERSE)])
...
def test_generate_binder_endpoint():
    design_state.set_design(_design_with_overhang_and_complement(overhang_seq="AAACCCGG"))
...
def test_generate_binder_endpoint_404():
    design_state.set_design(_design_with_overhang_and_complement())
    assert client.post("/api/design/overhang/missing/generate-binder").status_code == 404
...
def test_linker_complement_domains_carry_binds_overhang_id():
    h = Helix(id="oh_a", axis_start=Vec3(x=0, y=0, z=0), axis_end=Vec3(x=0, y=0, z=8),
              length_bp=8, bp_start=0, grid_pos=(0, 0))
    h2 = Helix(id="oh_b", axis_start=Vec3(x=3, y=0, z=0), axis_end=Vec3(x=3, y=0, z=8),
               length_bp=8, bp_start=0, grid_pos=(0, 3))
    sa = Strand(id="sa", strand_type=StrandType.STAPLE,
                domains=[Domain(helix_id="oh_a", start_bp=0, end_bp=7,
                                direction=Direction.FORWARD, overhang_id="oh_a_5p")])
    sb = Strand(id="sb", strand_type=StrandType.STAPLE,
                domains=[Domain(helix_id="oh_b", start_bp=0, end_bp=7,
    ...
    bound = {d.binds_overhang_id for s in linker_strands for d in s.domains
