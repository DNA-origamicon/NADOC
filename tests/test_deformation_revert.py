...
def _add_bend(plane_a: int = 20, plane_b: int = 60, kappa: float = 1.0):
    return client.post("/api/design/deformation", json={
        "type": "bend",
        "plane_a_bp": plane_a,
        "plane_b_bp": plane_b,
...
def test_delete_bend_keeps_later_flat_append():
    ...
    r = client.post("/api/design/bundle", json={"cells": [[0, 0]], "length_bp": 42, "name": "B"})
    assert r.status_code == 201, r.text

    assert _add_bend(plane_a=8, plane_b=30).status_code == 200

    seg = client.post("/api/design/bundle-segment", json={
