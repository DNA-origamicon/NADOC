...
def test_surface_and_atomistic_reps_round_trip():
    d = _demo_design()
    d.representation_overrides = [
        RepresentationOverride(representation=rep,
                               segments=[RepresentationSegment(helix_id=HELIX, bp_start=0, bp_end=5)])
        for rep in ("surface", "vdw", "ballstick")
    ]
    restored = Design.from_json(d.to_json())
    assert [o.representation for o in restored.representation_overrides] == ["surface", "vdw", "ballstick"]


def test_put_overrides_bogus_rep_422(client):
    body = {"overrides": [{"representation": "bogus",
                           "segments": [{"helix_id": HELIX, "bp_start": 0, "bp_end": 5}]}]}
    assert client.put("/api/design/representation-overrides", json=body).status_code == 422


def test_put_overrides_atomistic_rep_persists(client):
    body = {"overrides": [{"representation": "vdw",
                           "segments": [{"helix_id": HELIX, "bp_start": 0, "bp_end": 8}]}]}
    assert client.put("/api/design/representation-overrides", json=body).status_code == 200
...
def test_put_overrides_unknown_helix_404(client):
    body = {
        "overrides": [
            {"representation": "full", "segments": [{"helix_id": "ghost", "bp_start": 0, "bp_end": 5}]}
        ]
    }
    assert client.put("/api/design/representation-overrides", json=body).status_code == 404
...
def test_put_overrides_leaves_geometry_untouched(client):
    ...
    body = {"overrides": [{"representation": "full",
                           "segments": [{"helix_id": HELIX, "bp_start": 0, "bp_end": 5}]}]}
    assert client.put("/api/design/representation-overrides", json=body).status_code == 200
...
def test_overrides_survive_import(client):
    ...
    d.representation_overrides = [
        RepresentationOverride(
            name="focus", representation="cylinders",
