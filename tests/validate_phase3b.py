...
def _run_em_only(
    ...
    # ── 1. Build input PDB ────────────────────────────────────────────────
    pdb_text = _build_gromacs_input_pdb(
        design, ff=ff, nuc_pos_override=nuc_pos_override
    )
    ...
    if em_log.exists():
        ...
        for li, line in enumerate(lines[:-1]):
            if "LJ (SR)" in line and li + 1 < len(lines):
                ...
                lj_col = next((i for i, f in enumerate(hdr_fields) if "LJ" in f), None)
        ...
        converged = "converged to Fmax" in log_txt or "Norm of force" in log_txt
...
def main() -> int:
    ...
    design = _load_design()
    print(
        f"Design loaded: {len(design.helices)} helices, {len(design.strands)} strands"
    )
    print()
    ...
    with tempfile.TemporaryDirectory(prefix="nadoc_p3b_baseline_") as d:
        ...
        print(
            f"  Steps: {r['steps']}  LJ_init: {lj_str}  "
    ...
    print("=== Results ===")
    print(
        f"{'Label':<12} {'Steps':>8} {'LJ_initial':>14} {'Converged':>10} {'Time(s)':>8}"
    )
    for r in results:
        lj = f"{r['lj_initial']:.3e}" if r["lj_initial"] is not None else "N/A"
        print(
            f"{r['label']:<12} {r['steps']:>8} {lj:>14} {str(r['converged']):>10} "
            f"{r['elapsed_s']:>8.1f}"
        )
