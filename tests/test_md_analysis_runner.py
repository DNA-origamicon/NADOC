...
def test_subprocess_self_timeout_via_alarm():
    ...
    os.close(fd)
    p = R._CTX.Process(target=R._target, args=(rp, "time", "sleep", (30,), 2.0), daemon=True)
    t0 = time.monotonic()
    p.start()
    p.join(timeout=12)
...
def test_cancel_for_whole_job_kills_every_view():
    async def scenario():
        a = asyncio.create_task(R.run_analysis("J", "rmsf", "time", "sleep", (30,), timeout_s=60))
        b = asyncio.create_task(R.run_analysis("J", "traj", "time", "sleep", (30,), timeout_s=60))
        await asyncio.sleep(1.0)
