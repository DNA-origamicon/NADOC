...
def test_twist_stretch_physical_response(D_reg):
    ...
    f = np.zeros(6)
    f[2] = 1.0  # unit axial force on w_j
...
def test_electrostatics_stiffens_rmsf_and_cando_unchanged(routed_6hb):
    ...
    from backend.physics.fem_solver import _snupi_electro_sparse, _snupi_es_params
    ...
    K_es, f_es = _snupi_electro_sparse(
        mesh, defpos, _snupi_es_params(), scale=1.0, axial_only=True
    )
...
def test_g1_sequence_specific_K_differs_from_family_mean(routed_6hb_seq, routed_6hb):
    ...
    Kseq, _ = assemble_global_stiffness(
        build_fem_mesh(routed_6hb_seq), material="snupi"
    )
    Kmean, _ = assemble_global_stiffness(build_fem_mesh(routed_6hb), material="snupi")
    assert Kseq.shape == Kmean.shape
    assert not np.allclose(
        Kseq.toarray(), Kmean.toarray()
    )  # sequence moved the material
    # cando ignores sequence: same K with or without sequences assigned
    Kc_seq, _ = assemble_global_stiffness(
        build_fem_mesh(routed_6hb_seq), material="cando"
    )
...
def test_g12_salt_changes_debye_length_and_rmsf(routed_6hb):
    ...
    r_lo = predict_shape(
        routed_6hb, nonlinear=False, with_rmsf=True, material="snupi", mgcl2_M=0.02
    )["rmsf"]
    r_hi = predict_shape(
        routed_6hb, nonlinear=False, with_rmsf=True, material="snupi", mgcl2_M=0.20
    )["rmsf"]
...
def test_g12_salt_ignored_by_cando(routed_6hb):
    ...
    def _rmsf(mat, salt):
        out = predict_shape(
            routed_6hb, nonlinear=False, with_rmsf=True, material=mat, mgcl2_M=salt
        )["rmsf"]
...
def test_g3_lone_crossovers_classify_single_paired_double(routed_6hb):
    ...
    mesh = build_fem_mesh(routed_6hb)
    assert mesh.rigid_links and all(
        lk.co_type in ("double_co", "single_co") for lk in mesh.rigid_links
    )
...
def test_g3_single_co_softens_K_vs_all_double(routed_6hb, monkeypatch):
    ...
    assert not np.allclose(
        K_g3.toarray(), K_all.toarray()
    )  # single_co moved the crossover stiffness
...
def test_g2_registered_frame_orients_soft_bending_along_cross_strand(D_reg):
    ...
    cross = np.array([1.0, 0.0, 0.0])  # cross-strand = world-x
...
def test_g2_registered_frame_changes_snupi_rmsf_only(routed_6hb):
    ...
    Ks_reg, _ = assemble_global_stiffness(
        mesh, material="snupi", bp_registered_frame=True
    )
    Ks_arb, _ = assemble_global_stiffness(
        mesh, material="snupi", bp_registered_frame=False
    )
    assert not np.allclose(
        Ks_reg.toarray(), Ks_arb.toarray()
    )  # registration reoriented anisotropy
    Kc_reg, _ = assemble_global_stiffness(
        mesh, material="cando", bp_registered_frame=True
    )
    Kc_arb, _ = assemble_global_stiffness(
        mesh, material="cando", bp_registered_frame=False
    )
