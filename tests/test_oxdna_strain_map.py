...
def _measured(res):
    ...
    return {(p["helix_id"], p["bp_index"]) for p in res["positions"] if p["strain"] is not None}
...
def _nt(pos, a1=(1.0, 0.0, 0.0)):
    return {"backbone_position": np.asarray(pos, dtype=float),
...
def identity_site(monkeypatch):
    ...
    monkeypatch.setattr(health, "oxdna_backbone_site", lambda pos, a1, a3: np.asarray(pos, float))
    monkeypatch.setattr(health, "oxdna_backbone_sites", lambda cm, a1, a3: np.asarray(cm, float))
...
def test_backbone_strain_is_signed_and_worst_bond_wins(identity_site, monkeypatch):
    ...
    assert by[("H", 2)] == pytest.approx(0.6 / R0 - 1.0, abs=1e-9)   # negative == compressed
    assert by[("H", 1)] == pytest.approx(0.6 / R0 - 1.0, abs=1e-9)   # worst of its two bonds
...
def test_backbone_strain_tension_is_positive(identity_site, monkeypatch):
    ...
    res = health.strain_map(object(), full_map, metric="backbone")
    assert all(p["strain"] == pytest.approx(0.95 / R0 - 1.0, abs=1e-9) for p in res["positions"])


def test_backbone_strain_skips_nucleotides_absent_from_the_frame(identity_site, monkeypatch):
    ...
    monkeypatch.setattr(health, "backbone_bond_pairs", lambda design: [(a, b), (b, ghost)])
    full_map = {a: _nt([0.0, 0, 0]), b: _nt([R0 * U, 0, 0]), ("H", 5, "FORWARD"): _nt([50.0, 0, 0])}
    res = health.strain_map(object(), full_map, metric="backbone")
...
def test_wc_strain_measures_base_site_separation(identity_site):
    ...
    assert all(p["strain"] == pytest.approx(1.0 / HR0 - 1.0, abs=1e-9) for p in res["positions"])
...
def test_wc_strain_leaves_unpaired_nucleotides_unmeasured_but_emitted(identity_site):
    ...
    res = health.strain_map(object(), full_map, metric="wc")
    assert {(p["helix_id"], p["bp_index"], p["direction"])
            for p in res["positions"] if p["strain"] is not None} == {
...
def test_field_override_supplies_values_while_full_map_supplies_geometry(identity_site, monkeypatch):
    ...
    res = health.strain_map(object(), full_map, metric="backbone", field={a: 0.25, b: -0.5})
    by = {(p["helix_id"], p["bp_index"]): p for p in res["positions"]}
    assert by[("H", 0)]["strain"] == pytest.approx(0.25)          # from `field`, not the geometry
    assert by[("H", 1)]["strain"] == pytest.approx(-0.5)
    assert by[("H", 1)]["backbone_position"] == pytest.approx([R0 * U, 0, 0])   # from full_map
...
def test_field_override_omits_nucleotides_it_does_not_cover(identity_site, monkeypatch):
    a, b = ("H", 0, "FORWARD"), ("H", 1, "FORWARD")
    monkeypatch.setattr(health, "backbone_bond_pairs", lambda design: [(a, b)])
    res = health.strain_map(object(), {a: _nt([0.0, 0, 0]), b: _nt([R0 * U, 0, 0])},
                            metric="backbone", field={a: 0.1})
    assert _measured(res) == {("H", 0)}
    assert _emitted(res) == {("H", 0), ("H", 1)}


def test_mean_strain_over_frames_is_not_the_strain_of_the_mean_structure(identity_site, monkeypatch):
    ...
    mean_structure = {a: _nt([0.0, 0, 0]), b: _nt([R0 * U, 0, 0])}   # the average position
    per_frame = [health.strain_field(object(), f, metric="backbone")[a] for f in (short, long_)]
    ...
    assert health.strain_field(object(), mean_structure, metric="backbone")[a] == pytest.approx(0.0)
...
def test_backbone_strain_measures_extension_tail_bonds(identity_site, monkeypatch):
    ...
    monkeypatch.setattr(health, "backbone_bond_pairs", lambda design: [(anchor, t0), (t0, t1)])
    full_map = {
        anchor: _nt([0.0, 0, 0]),
    ...
    assert by[("__ext_9", 0)] == pytest.approx(0.55 / R0 - 1.0, abs=1e-9)   # worst of its two
...
def test_backbone_rejects_bonds_outside_the_fene_window(identity_site, monkeypatch):
    ...
    assert f[b] == pytest.approx(0.9 - 1.0, abs=1e-9), "b keeps its GOOD bond, not the torn one"
...
def test_designed_ssdna_flags_are_topological():
    ...
    full_map = {
        ("H", 0, "FORWARD"): {}, ("H", 0, "REVERSE"): {},   # duplex
...
def test_dsdna_stats_absent_when_nothing_is_duplex(identity_site, monkeypatch):
    monkeypatch.setattr(health, "backbone_bond_pairs", lambda design: [])
    k = ("__ext_1", 0, "FORWARD")
    res = health.strain_map(object(), {k: _nt([0.0, 0, 0])}, metric="backbone", field={k: 0.1})
...
def test_fene_violation_fraction_flags_a_torn_frame(identity_site, monkeypatch):
    ...
    keys = list(clean)
    ia, ib = health._strain_index(object(), keys, "backbone")
    assert health._fene_violation_fraction(*health._gather_frame(clean, keys), ia, ib) == 0.0
    assert health._fene_violation_fraction(*health._gather_frame(torn, keys), ia, ib) == pytest.approx(0.5)
...
def test_wc_display_bound_ignores_the_melted_mode():
    ...
    assert bound < health.WC_UNPAIRED_STRAIN, "must not be dragged past the H-bond cutoff"
...
def test_display_bound_is_robust_and_metric_aware():
    ...
    assert health._display_strain_bound(tight, "backbone") == pytest.approx(0.08, abs=0.01)


def test_strain_map_reports_a_display_bound_below_the_max_when_outliers_exist(identity_site, monkeypatch):
