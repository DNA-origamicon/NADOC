...
def test_near_end_crossover_ligates():
    ...
    design, xover, ligated = _place(
        _design_01(), "h0", "h1", 15, "FORWARD", "REVERSE", 15, 16
    )
    assert ligated is True
    spanning = [
        s for s in design.strands if {d.helix_id for d in s.domains} == {"h0", "h1"}
    ]
...
def test_near_end_crossover_makes_single_nt_stubs():
    ...
    stubs = [
        ...
        if len(s.domains) == 1
        and s.domains[0].start_bp == 16
        and s.domains[0].end_bp == 16
    ]
    stub_helices = {s.domains[0].helix_id for s in stubs}
    assert stub_helices == {"h0", "h1"}, (
        f"expected 1-nt stubs on both helices, got {stub_helices}"
    )
...
def test_crossover_through_overhang_boundary_connects():
    ...
    design, _x, ligated = _place(
        _design_overhang_boundary(), "hB", "hA", 0, "FORWARD", "REVERSE", -1, 0
    )
    assert ligated is True
    spanning = [
        s for s in design.strands if {d.helix_id for d in s.domains} == {"hA", "hB"}
    ]
...
def test_crossover_severs_overhang_into_plain_stub():
    ...
    stubs = [
        ...
        for s in design.strands
        if len(s.domains) == 1
        and s.domains[0].start_bp == -1
        and s.domains[0].end_bp == -1
    ]
    assert len(stubs) == 2
    assert all(s.domains[0].overhang_id is None for s in stubs), (
        "severed stubs must lose overhang tag"
    )
