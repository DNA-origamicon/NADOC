...
def _seed(*, scaffold_sequenced: bool = True, seq_a="ACGTACGT", seq_b="TTTTGGGG"):
    ...
    unrelated = Strand(
        ...
        strand_type=StrandType.STAPLE,
        domains=[
            Domain(helix_id="scaf_h", start_bp=7, end_bp=0, direction=Direction.REVERSE)
        ],
        sequence=MANUAL_SEQ,
    )
...
def _linker_complement() -> str | None:
    d = design_state.get_or_404()
    return next(
        (s.sequence for s in d.strands if s.id.endswith("__a") and "__lnk__" in s.id),
        None,
    )
...
def test_apply_connection_version_does_not_touch_unrelated_strand():
    ...
    r = client.post(
        ...
        json={
            ...
            "overhang_a_seq": "GGGGGGGG",
            "overhang_b_seq": "CCCCCCCC",
            "bridge_length": 6,
            "bridge_seq": "TTTTTT",
        },
    )
