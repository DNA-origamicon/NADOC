...
def _ca_asset(n_res=4, spacing_nm=0.38, x0=2.0):
    ...
    atoms = [ProteinAtom(serial=i, name="CA", element="C", res_name="ALA",
                         chain_id="A", res_seq=i + 1, x=x0 + spacing_nm * i, y=2.0, z=2.0)
             for i in range(n_res)]
    return ProteinAsset(name="p", atoms=atoms, center_of_mass=[x0 + spacing_nm * (n_res - 1) / 2, 2.0, 2.0])
...
def test_protein_atoms_world_placed_by_pose():
    ...
    pose = gizmo_move_to_pose(np.eye(4), pivot=[0, 0, 0],
                              translation=[1.0, 0.0, 0.0], rotation=[0, 0, 0, 1])
...
def test_two_attachments_get_distinct_chains():
    ...
    from backend.core.atomistic import build_atomistic_model
    prot = [a for a in build_atomistic_model(d, include_proteins=True).atoms
...
def test_ca_enm_pairs_within_cutoff_symmetric_and_reflengths():
    ...
    by_gap = {(j - i): r0 for (si, sj, r0), (i, j) in zip(pairs, [(0, 1), (0, 2), (0, 3), (1, 2), (1, 3), (2, 3)])}
...
def test_click_linker_connects_conjugation_atom_to_dna_handle(monkeypatch):
    ...
    dna = next(a for a in model.atoms
...
def test_psfgen_builds_protein_plus_dna_topology():
    ...
    for i, rn in enumerate(["ALA", "GLY", "SER"]):
        for nm, el, dx in [("N", "N", 0.0), ("CA", "C", 0.13), ("C", "C", 0.25), ("O", "O", 0.30)]:
            atoms.append(ProteinAtom(serial=serial, name=nm, element=el, res_name=rn,
                                     chain_id="A", res_seq=i + 1,
                                     x=2.0 + 0.38 * i + dx, y=2.0 + (0.05 if nm == "O" else 0.0), z=2.0))
            serial += 1
