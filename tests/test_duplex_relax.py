...
from backend.core.models import (
    ClusterRigidTransform, Design, Direction, Domain, Helix, OverhangSpec,
    Strand, StrandType, SubDomain, Vec3,
)
...
_CLOSE_FIXTURE = Path(__file__).resolve().parent / "fixtures" / "relax_2x2_closebond.nadoc"
...
def test_relax_closes_a_stretched_bond_via_cluster_motion_only():
    ...
    design = design.model_copy(update={
        "cluster_transforms": [design.cluster_transforms[0], c2], "overhangs": ohs})

    out, info = relax_direct_binding(design, b.driver_oh_id, b.driven_oh_id)
    assert info["final_chord_nm"] == pytest.approx(info["target_nm"], abs=0.2)
    assert any(abs(t) > 1.0 for t in info["thetas_deg"])   # the joint carried the closure
...
def _helix(hid, length, *, x=0.0):
    return Helix(id=hid, axis_start=Vec3(x=x, y=0.0, z=0.0),
...
def _simple_design() -> Design:
    ...
    return Design(helices=[_helix("h_drv", 24), _helix("h_drvn", 10, x=8.0)],
