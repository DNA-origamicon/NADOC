...
def test_create_binding_200():
    ...
    r = client.post(
        "/api/assembly/overhang-bindings",
        json={
            "instance_a_id": "inst-A",
            "sub_domain_a_id": sd["A"],
            "overhang_a_id": "oh-A",
            "instance_b_id": "inst-B",
            "sub_domain_b_id": sd["B"],
            "overhang_b_id": "oh-B",
        },
    )
...
def test_create_binding_self_rejected_400():
    ...
    r = client.post(
        ...
        json={
            ...
            "sub_domain_a_id": sd["A"],
            "overhang_a_id": "oh-A",
            "instance_b_id": "inst-A",
            "sub_domain_b_id": sd["A"],
            "overhang_b_id": "oh-A",
        },
    )
...
def test_create_binding_duplicate_409():
    ...
    swapped = {
        "instance_a_id": "inst-B",
        "sub_domain_a_id": sd["B"],
        "overhang_a_id": "oh-B",
        "instance_b_id": "inst-A",
        "sub_domain_b_id": sd["A"],
        "overhang_b_id": "oh-A",
...
def test_patch_binding_unknown_404():
    ...
    r = client.patch(
        "/api/assembly/overhang-bindings/missing", json={"binding_mode": "toehold"}
    )
    assert r.status_code == 404
...
def test_create_connection_200():
    ...
    assert any(
        e["op_kind"] == "assembly-overhang-connection-add" for e in asm["feature_log"]
    )
...
def test_create_connection_rejects_negative_length_400():
    ...
    r = client.post(
        "/api/assembly/overhang-connections", json=_connection_payload(length_value=-1)
    )
    assert r.status_code == 400
...
def test_patch_connection_changes_length():
    ...
    r2 = client.patch(
        f"/api/assembly/overhang-connections/{cid}", json={"length_value": 24}
    )
    assert r2.status_code == 200, r2.text
...
def test_delete_connection_removes_row():
    ...
    assert any(
        e["op_kind"] == "assembly-overhang-connection-delete"
        for e in asm["feature_log"]
    )
...
def test_post_overhang_connection_generates_ds_linker_topology():
    ...
    r = client.post(
        "/api/assembly/overhang-connections",
        json=_conn_payload_real(linker_type="ds", attach_a="free_end", attach_b="root"),
    )
    assert r.status_code == 200, r.text
    asm = r.json()["assembly"]
...
def test_post_overhang_connection_generates_ss_linker_topology():
    ...
    r = client.post(
        ...
        json=_conn_payload_real(
            linker_type="ss", attach_a="free_end", attach_b="free_end"
        ),
    )
    assert r.status_code == 200, r.text
    asm = r.json()["assembly"]
...
def test_post_indirect_zero_length_generates_binding_strand_no_bridge():
    ...
    s_nucs = [
        n for n in geo.json()["nucleotides"] if n.get("strand_id") == f"__lnk__{cid}__s"
    ]
...
def test_delete_overhang_connection_removes_linker_topology():
    _seed_real_two_part_assembly()
    r = client.post(
        "/api/assembly/overhang-connections", json=_conn_payload_real(linker_type="ds")
    )
    cid = r.json()["assembly"]["overhang_connections"][0]["id"]
...
def test_patch_length_regenerates_linker_topology():
    ...
    r2 = client.patch(
        f"/api/assembly/overhang-connections/{cid}", json={"length_value": 12}
    )
    assert r2.status_code == 200, r2.text
    asm = r2.json()["assembly"]
    strand_a = next(s for s in asm["assembly_strands"] if s["id"] == f"__lnk__{cid}__a")
    bridge_dom = next(
        d for d in strand_a["domains"] if d["helix_id"] == f"__lnk__{cid}"
    )
...
def test_patch_bridge_sequence_updates_strand_sequence():
    ...
    s_before = next(
        s
        for s in r.json()["assembly"]["assembly_strands"]
        if s["id"] == f"__lnk__{cid}__s"
    )
    assert "AAAA" in (s_before["sequence"] or "")

    r2 = client.patch(
        f"/api/assembly/overhang-connections/{cid}", json={"bridge_sequence": "GGGG"}
    )
    assert r2.status_code == 200, r2.text
    s_after = next(
        s
        for s in r2.json()["assembly"]["assembly_strands"]
        if s["id"] == f"__lnk__{cid}__s"
    )
