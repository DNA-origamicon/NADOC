...
from backend.core.models import (
    Design, Direction, Domain, Duplex, DuplexEnd, Helix, OverhangSpec, Strand,
...
def test_validate_design_flags_non_pairing_duplex():
    ...
    dx = Duplex(left=DuplexEnd(overhang_id="ohA", start_bp=0, end_bp=3),
                right=DuplexEnd(overhang_id="ohB", start_bp=3, end_bp=0))
    design = Design(strands=[sa, sb], overhangs=[ohA, ohB], duplexes=[dx])
    msgs = [r.message for r in validate_design(design).results
...
def test_validate_design_does_not_flag_all_N_or_complementary():
    ...
    ohB = OverhangSpec(id="ohB", helix_id="hB", strand_id="sb", sequence="GTTT")  # RC(AAAC)
    dx = Duplex(left=DuplexEnd(overhang_id="ohA", start_bp=0, end_bp=3),
                right=DuplexEnd(overhang_id="ohB", start_bp=3, end_bp=0))
    design = Design(strands=[sa, sb], overhangs=[ohA, ohB], duplexes=[dx])
    msgs = [r.message for r in validate_design(design).results
...
def _helix(hid, length):
    return Helix(id=hid, axis_start=Vec3(x=0.0, y=0.0, z=0.0),
                 axis_end=Vec3(x=0.0, y=0.0, z=length * BDNA_RISE_PER_BP),
                 phase_offset=0.0, length_bp=length, grid_pos=(0, 0))


def _diff_length_design() -> Design:
    st_drv = Strand(id="st_drv", strand_type=StrandType.STAPLE,
                    domains=[Domain(helix_id="h_drv", start_bp=0, end_bp=23,
                                    direction=Direction.FORWARD, overhang_id="OHd")])
    st_drvn = Strand(id="st_drvn", strand_type=StrandType.STAPLE,
                     domains=[Domain(helix_id="h_drvn", start_bp=0, end_bp=9,
                                     direction=Direction.FORWARD, overhang_id="OHn")])
    ohd = OverhangSpec(id="OHd", helix_id="h_drv", strand_id="st_drv", sequence="A" * 24,
                       sub_domains=[SubDomain(id="sdd", start_bp_offset=0, length_bp=24)])
    ohn = OverhangSpec(id="OHn", helix_id="h_drvn", strand_id="st_drvn", sequence="T" * 10,
                       sub_domains=[SubDomain(id="sdn", start_bp_offset=0, length_bp=10)])
    return Design(helices=[_helix("h_drv", 24), _helix("h_drvn", 10)],
                  strands=[st_drv, st_drvn], overhangs=[ohd, ohn])
...
def test_headless_connect_duplex_relocates_driven():
    ...
    assert_duplex_relocated(design_state.get_design(),
                            driver_oh_id="OHd", driven_oh_id="OHn", driven_length_bp=10)
...
@pytest.mark.skipif(not _FIXTURE.exists(), reason="relax_2x2_binding.nadoc fixture missing")
def test_headless_relax_duplex_closes_bond_and_oracle():
    ...
    ohs = [o.model_copy(update={"rotation": [0, 0, 0, 1.0]})
           if o.id == before.duplexes[0].left.overhang_id else o for o in before.overhangs]
    before = before.model_copy(update={
        "cluster_transforms": [before.cluster_transforms[0], c2], "overhangs": ohs})
