...
def test_available_reports_shape():
    ...
    assert (
        isinstance(body["free_ranks"], int)
        and 1 <= body["free_ranks"] <= body["max_ranks"]
    )
...
def test_create_field_without_anchor_allowed(monkeypatch, tmp_path):
    ...
    r = client.post(
        "/api/lammps/jobs",
        json={"steps": 500, "field": {"field_pN": 20.0, "dir": [1, 0, 0]}},
    )
    assert r.status_code == 200, r.text
    job = r.json()
...
def test_trajectory_guard_on_design_mismatch(monkeypatch, tmp_path):
    ...
    design_state.set_design(
        make_6hb_design(length_bp=42)
    )  # far fewer than 999 nucleotides
