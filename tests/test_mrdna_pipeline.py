...
_PRIMITIVE_DESIGN = (
    EXAMPLES / "6hb_test.nadoc"
)  # 6-helix bundle, scaffold + staples, crossovers
...
skip_no_mrdna = pytest.mark.skipif(
    not _has_mrdna,
    reason="mrdna not installed (set $MRDNA_TOOL_PATH or use ~/mrdna-tool)",
)


def _generate_primitive_fixture(
    out_dir: Path, design_path: Path = _PRIMITIVE_DESIGN, steps: int = _PRIMITIVE_STEPS
):
...
class TestWslCudaLibs:
    ...
    def test_prepends_wsl_lib_dir_under_wsl(self, monkeypatch):
        ...
        mb.ensure_wsl_cuda_libs()
        assert (
            mb.os.environ["LD_LIBRARY_PATH"].split(mb.os.pathsep).count(mb._WSL_LIB_DIR)
            == 1
        )
...
class TestSyntheticRoundTrip:
    ...
    def test_sequential_backbone_distance(self, roundtrip_fixture):
        ...
        assert len(violations) == 0, (
            f"{len(violations)} backbone distance violations > 1.0 nm: {violations[:3]}"
        )
...
class TestRoutedPrimitiveIntegration:
    ...
    def test_crossover_keys_present(self, primitive_override):
        ...
        design, override = primitive_override
        helix_ranges = {
            h.id: range(h.bp_start, h.bp_start + h.length_bp) for h in design.helices
        }
        xover = _crossover_junction_keys(design)
        in_range_keys = [k for k in xover if k[1] in helix_ranges.get(k[0], range(0))]
