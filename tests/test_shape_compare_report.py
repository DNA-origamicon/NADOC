    ...
    for h in range(n_helix):
        ...
        for i in range(n_axial):
            out.append(_pos(h, i, "forward",
                            (x + shift[0], y + shift[1], rise * i + shift[2])))
    return out
...
def _field_profile(disp_by_key, field_dir=(1.0, 0.0, 0.0), anchor_keys=()):
    ...
    return field_response_profile(fld, ref, field_dir, anchor_keys,
                                  anchor_tol_nm=1.0, min_free_proj_nm=0.1)


def _src(engine, **kw):
    return {"engine": engine, "descriptors": kw.get("descriptors"),
            "rmsf": kw.get("rmsf"), "shape_frame": kw.get("shape_frame"),
            "field": kw.get("field")}
...
def test_rmsf_agreement_and_profiles():
    ...
    assert engs["oxdna"]["points"] == [[0.0, 1.0], [1.0, 2.0], [2.0, 3.0],
                                       [3.0, 4.0], [4.0, 5.0]]
