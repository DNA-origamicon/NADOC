...
def test_thermal_nma_is_deterministic_and_matches_reported_rmsf():
    ...
    rmsf, frames = compute_thermal_nma(K, 2, n_modes=4, n_rigid=6, n_frames=12000, seed=7)
    rmsf2, frames2 = compute_thermal_nma(K, 2, n_modes=4, n_rigid=6, n_frames=12000, seed=7)

    assert frames.shape == (12000, 12)
    assert np.array_equal(frames, frames2)
    assert np.array_equal(rmsf, rmsf2)
    sampled = np.sqrt(np.sum(np.var(frames.reshape(-1, 2, 6)[:, :, :3], axis=0), axis=1))
...
def test_runner_caches_compact_thermal_payload(tmp_path):
    ...
    payload = {
        ...
        "positions": [],
        "thermal_trajectory": {
            "kind": "normal-mode-ensemble", "temperature_k": 298.0,
            "keys": [["h0", 0, "FORWARD", 0]],
            "frames": [[1.0, 2.0, 3.0]], "n_frames": 1, "representative_frame": 0,
        },
    }
