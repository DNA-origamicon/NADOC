...
# ── XB (crossover-base) cone direction validation ─────────────────────────────
#
# CrossoverBases beads (synthetic helix_id = __xb_<cb.id>) sit on a Bézier arc
# between the two anchor nucleotides at a crossover junction.  After rotation the
# arc is recomputed from the rotated anchor positions, so XB bead coordinates are
# not simply rotated copies of the original positions — BUT the individual cone
# vectors between consecutive beads on that arc must still equal R @ orig_dir
# (because both endpoints are rotated by the same R about J).
#
# The design used here is the smallest possible construct that has XB beads:
# two parallel helices, a scaffold strand that crosses between them at bp 20,
# and a "TT" CrossoverBases inserted at that crossover.

from backend.core.models import (
    Design, Direction, Domain, Helix, Strand, StrandType, Crossover, CrossoverType, Vec3,
)


def _setup_2hb_with_xb_joint():
    """
    Build a minimal 2-helix design (42 bp) with:
      - Scaffold strand crossing from helix A (FORWARD) to helix B (REVERSE) at bp 20.
      - A CrossoverBases("TT") bead pair at that crossover.
      - A cluster covering both helices.
      - A joint at the axis_start of helix A (corner of the cross-section, Z=0).

    Returns (cluster_id, J_np_array).
    """
    # Two antiparallel helices 2 nm apart in X, both 42 bp.
    helix_a = Helix(
        axis_start=Vec3(x=0.0, y=0.0, z=0.0),
        axis_end=Vec3(x=0.0, y=0.0, z=42 * 0.334),
        length_bp=42,
    )
    helix_b = Helix(
        axis_start=Vec3(x=2.0, y=0.0, z=0.0),
        axis_end=Vec3(x=2.0, y=0.0, z=42 * 0.334),
        length_bp=42,
    )

    # Scaffold: domain A (helix_a, FORWARD, bp 0→20) → crossover → domain B (helix_b, REVERSE, bp 20→0)
    dom_a = Domain(helix_id=helix_a.id, start_bp=0,  end_bp=20, direction=Direction.FORWARD)
    dom_b = Domain(helix_id=helix_b.id, start_bp=20, end_bp=0,  direction=Direction.REVERSE)
    scaffold = Strand(domains=[dom_a, dom_b], strand_type=StrandType.SCAFFOLD)

    xo = Crossover(
        strand_a_id=scaffold.id, domain_a_index=0,
        strand_b_id=scaffold.id, domain_b_index=1,
        crossover_type=CrossoverType.SCAFFOLD,
    )

    from backend.core.models import CrossoverBases as XB
    cb = XB(crossover_id=xo.id, strand_id=scaffold.id, sequence="TT")

    design = Design(
        helices=[helix_a, helix_b],
        strands=[scaffold],
        crossovers=[xo],
        crossover_bases=[cb],
    )

    # Cluster covering both helices.
    ct = ClusterRigidTransform(
        name="2HB",
        helix_ids=[helix_a.id, helix_b.id],
        pivot=[0.0, 0.0, 0.0],
        translation=[0.0, 0.0, 0.0],
        rotation=[0.0, 0.0, 0.0, 1.0],
    )
    ...
    # Joint at origin (axis_start of helix A).
    r = client.post(
        f"/api/design/cluster/{ct.id}/joint",
        json={"axis_origin": [0.0, 0.0, 0.0], "axis_direction": [0.0, 0.0, 1.0], "name": "XbHinge"},
    )
    assert r.status_code == 200
...
def test_xb_cone_endpoints_after_rotation():
    """
    After a 90° rotation about J, XB cone endpoints must satisfy
      pos_after = R @ (pos_before − J) + J.

    This specifically exercises cross-helix cones where both endpoints are
    __xb__ synthetic beads sitting on the Bézier arc between two anchor nucs.
    """
    cluster_id, J = _setup_2hb_with_xb_joint()

    geo_before = client.get("/api/design/geometry").json()["nucleotides"]
    xb_nucs_before = [n for n in geo_before if n["helix_id"].startswith("__xb_")]
    assert xb_nucs_before, "No __xb__ beads found — CrossoverBases setup failed"

    pairs_before = _cone_pairs(geo_before)
    xb_pairs_before = [p for p in pairs_before
                       if p["from_helix"].startswith("__xb_") or p["to_helix"].startswith("__xb_")]
    assert xb_pairs_before, "No XB cone pairs found"
    ...
    geo_after  = client.get("/api/design/geometry").json()["nucleotides"]
    pairs_after = _cone_pairs(geo_after)
    xb_pairs_after = [p for p in pairs_after
                      if p["from_helix"].startswith("__xb_") or p["to_helix"].startswith("__xb_")]

    assert len(xb_pairs_after) == len(xb_pairs_before), (
        f"XB cone count changed: {len(xb_pairs_before)} → {len(xb_pairs_after)}"
    )
    ...
    for pb, pa in zip(xb_pairs_before, xb_pairs_after):
        from_expected = _apply_rotation(pb["from_pos"], R, J)
        to_expected   = _apply_rotation(pb["to_pos"],   R, J)
        from_err = np.linalg.norm(pa["from_pos"] - from_expected)
        to_err   = np.linalg.norm(pa["to_pos"]   - to_expected)
        ...
        if from_err > max_from_err:
            max_from_err = from_err
            worst_from = {
                "from_helix": pb["from_helix"], "to_helix": pb["to_helix"],
                "is_cross":   pb["is_cross_helix"],
                "orig":       pb["from_pos"].round(6).tolist(),
                "expected":   from_expected.round(6).tolist(),
                "actual":     pa["from_pos"].round(6).tolist(),
    ...
    assert max_from_err < 1e-8, (
        f"XB cone from-endpoint error {max_from_err:.3e} nm\n"
        f"  is_cross={worst_from['is_cross']} "
        f"({worst_from['from_helix']} → {worst_from['to_helix']})\n"
        f"  orig:     {worst_from['orig']}\n"
        f"  expected: {worst_from['expected']}\n"
        f"  actual:   {worst_from['actual']}"
    ...
    assert max_to_err < 1e-8, (
        f"XB cone to-endpoint error {max_to_err:.3e} nm\n"
        f"  is_cross={worst_to['is_cross']} "
        f"({worst_to['from_helix']} → {worst_to['to_helix']})\n"
        f"  orig:     {worst_to['orig']}\n"
        f"  expected: {worst_to['expected']}\n"
        f"  actual:   {worst_to['actual']}"
...
def test_xb_cone_direction_after_rotation():
    """
    After a 90° rotation about J, each XB cone direction vector (to−from) must
    equal R @ original_direction.

    Also asserts the total number of XB beads is preserved, giving an early
    failure if the rotation destroys the crossover topology.
    """
    cluster_id, J = _setup_2hb_with_xb_joint()

    geo_before = client.get("/api/design/geometry").json()["nucleotides"]
    n_xb_before = sum(1 for n in geo_before if n["helix_id"].startswith("__xb_"))
    pairs_before = _cone_pairs(geo_before)
    xb_pairs_before = [p for p in pairs_before
                       if p["from_helix"].startswith("__xb_") or p["to_helix"].startswith("__xb_")]
    ...
    geo_after = client.get("/api/design/geometry").json()["nucleotides"]
    n_xb_after = sum(1 for n in geo_after if n["helix_id"].startswith("__xb_"))
    assert n_xb_after == n_xb_before, f"XB bead count changed: {n_xb_before} → {n_xb_after}"
    ...
    assert max_dir_err < 1e-8, (
        f"XB cone direction error {max_dir_err:.3e} nm\n"
        f"  is_cross={worst['is_cross']} "
        f"({worst['from_helix']} → {worst['to_helix']})\n"
        f"  orig_dir:     {worst['orig_dir']}\n"
        f"  expected_dir: {worst['expected_dir']}\n"
        f"  actual_dir:   {worst['actual_dir']}"
