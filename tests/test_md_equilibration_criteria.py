...
def _pair_positions(distances):
    ...
    return (
        np.asarray(don + acc, dtype=float),
...
def test_no_ions_or_no_dna_is_zero_not_a_crash():
    ...
    assert charge_within_shell(pos, charges, np.array([0]), np.empty(0, int)) == (
        0.0,
        0,
    )
    assert charge_within_shell(pos, charges, np.empty(0, int), np.array([1])) == (
        0.0,
        0,
    )
