...
def _val(text: str, key: str) -> str:
    return next(
        l.split()[1] for l in text.splitlines() if l.lower().startswith(key.lower())
    )
...
def test_soften_is_idempotent_and_a_noop_when_already_soft():
    ...
    already_soft = FAST_CONF.replace(
...
def test_downgrade_factor_can_still_be_forced():
    assert (
        _val(downgrade_gpu_resident(SOFT_RESIDENT_CONF, factor=2), "timestep") == "0.5"
    )
...
def test_probe_detects_pinned_host_exhaustion(tmp_path, monkeypatch):
    ...
    assert (
        namd_runner.gpu_resident_probe(pkg, "seg_fast", "namd3", "0", threads=2)
        is False
    )
...
def test_probe_passes_when_gpu_resident_works(tmp_path, monkeypatch):
    ...
    assert (
        namd_runner.gpu_resident_probe(pkg, "seg_fast", "namd3", "0", threads=2) is True
    )
...
def test_probe_is_cached(tmp_path, monkeypatch):
    ...
    assert len(seen) == 1
    assert (
        json.loads((pkg / namd_runner.GPU_RESIDENT_PROBE_CACHE).read_text())[
            "gpu_resident_ok"
        ]
        is False
    )
...
@pytest.mark.parametrize(
    "boom", [OSError("nope"), subprocess.TimeoutExpired("namd3", 1)]
)
...
def test_downgrade_confs_is_idempotent(tmp_path):
    ...
    assert (
        _val((pkg / "seg_fast.conf").read_text(), "timestep") == "2"
    )  # not halved twice
