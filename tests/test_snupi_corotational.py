...
def test_element_zero_force_at_rest_and_under_rigid_motion():
    ...
    Q = exp_so3(np.array([0.3, -0.5, 0.7]))
    t = np.array([1.0, 2.0, 3.0])
    fr, _ = element_force_tangent(
        Q @ x1 + t, Q @ x2 + t, Q @ np.eye(3), Q @ np.eye(3), ref, K12
    )
...
def _cantilever(Nel=20, Le=1.0, EA=1e5, GJ=1e4, EI=1e3):
    ...
    elems = [
        (
            k,
            k + 1,
            element_reference(X0[k], X0[k + 1], np.eye(3), np.eye(3)),
            local_beam_stiffness_12(Le, EA, GJ, EI, EI),
        )
        for k in range(Nel)
    ]
...
def test_small_load_recovers_linear_euler_bernoulli():
    ...
    F = 0.01
    fe = np.zeros(6 * (Nel + 1))
    fe[6 * Nel] = F  # transverse tip load (x)
...
def test_corotational_integration_on_real_design_converges():
    ...
    cor = predict_shape(
        d, nonlinear=True, with_rmsf=False, material="snupi", corotational=True
    )
    fp = predict_shape(
        d, nonlinear=True, with_rmsf=False, material="snupi", corotational=False
    )
    ...
    assert (
        np.ptp(Pc, axis=0).max() < 3.0 * np.ptp(Pf, axis=0).max()
    )  # physically bounded (no blow-up)
