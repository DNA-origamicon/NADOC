...
from backend.core.duplex_cluster import (
    _duplex_domain_refs, _owning_parent_cluster, conjugate_world_pose_into_parent_rest,
)
from backend.core.models import (
    ClusterRigidTransform, Direction, Domain, Helix, OverhangSpec, Strand, StrandType, Vec3,
)


def _quat(axis, deg):
    a = np.asarray(axis, float); a = a / np.linalg.norm(a)
    h = math.radians(deg) / 2.0; s = math.sin(h)
...
def _seed(*, ovhg_rotation, ovhg_translation, parent):
    base = _demo_design()
    L = 16
    h = Helix(id="dc_h", axis_start=Vec3(x=0.0, y=0.0, z=0.0),
              axis_end=Vec3(x=0.0, y=0.0, z=L * BDNA_RISE_PER_BP),
              phase_offset=0.0, length_bp=L, grid_pos=(0, 0))
    s = Strand(id="dc_s", strand_type=StrandType.STAPLE, domains=[
        Domain(helix_id="dc_h", start_bp=0, end_bp=3, direction=Direction.FORWARD),
        Domain(helix_id="dc_h", start_bp=4, end_bp=11, direction=Direction.FORWARD,
               overhang_id="oh_a")])
    ov = OverhangSpec(id="oh_a", helix_id="dc_h", strand_id="dc_s", label="OHA",
                      sequence="ACGTACGT", rotation=ovhg_rotation, translation=ovhg_translation)
    return base.model_copy(update={
        "helices": [h], "strands": [s],
        "overhangs": [ov], "cluster_transforms": [parent]})


def _ovhg_beads(nucs):
    return {n["bp_index"]: np.asarray(n.get("backbone_position") or n.get("base_position"), float)
            for n in nucs if n.get("overhang_id") == "oh_a"}


def test_conjugation_reproduces_overlay_geometry_with_nonidentity_parent():
    parent = ClusterRigidTransform(
        id="P", name="part", helix_ids=["dc_h"], rotation=_quat([0.3, 1.0, 0.2], 33.0),
        translation=[4.0, -2.0, 1.5], pivot=[1.0, 0.5, 0.0])
    ...
    cleared = _seed(ovhg_rotation=[0, 0, 0, 1], ovhg_translation=[0, 0, 0], parent=parent)
    ...
    child = ClusterRigidTransform(
        id="C", name="Duplex 1", helix_ids=[helix_id], domain_ids=refs,
        parent_cluster_id=par.id, rotation=q_c, pivot=piv_c, translation=t_c)
...
def test_materialize_is_geometry_neutral_and_round_trips():
    ...
    from backend.core.duplex_cluster import (
        materialize_duplex_cluster, dematerialize_duplex_cluster, duplex_cluster_for,
    )
    parent = ClusterRigidTransform(
        id="P", name="part", helix_ids=["dc_h"], rotation=_quat([0.2, 1.0, 0.4], 41.0),
        translation=[-3.0, 5.0, 2.0], pivot=[0.5, 0.0, 1.0])
    ...
    assert np.allclose(bspec.rotation, r_w, atol=1e-5) and np.allclose(bspec.translation, t_w, atol=1e-5)
...
def _xf(quat, pivot, trans):
    ...
    piv = np.asarray(pivot, float); t = np.asarray(trans, float)
...
def test_duplex_cluster_segment_axis_follows_child_composition():
    ...
    from backend.core.duplex_cluster import materialize_duplex_cluster, duplex_cluster_for

    ident = ClusterRigidTransform(id="P", name="p", helix_ids=["dc_h"],
                                  rotation=[0, 0, 0, 1], translation=[0, 0, 0], pivot=[0, 0, 0])
    rest = _seed(ovhg_rotation=[0, 0, 0, 1], ovhg_translation=[0, 0, 0], parent=ident)
    rs, re = _oh_segment(deformed_helix_axes(rest))

    parent = ClusterRigidTransform(id="P", name="p", helix_ids=["dc_h"],
                                   rotation=_quat([0.2, 1.0, 0.3], 34.0),
                                   translation=[3.0, -1.0, 2.0], pivot=[0.5, 0.0, 1.0])
    d = _seed(ovhg_rotation=_quat([0, 1, 0], 25.0), ovhg_translation=[0.0, 1.3, 0.4], parent=parent)
...
def test_headless_materialize_duplex_cluster_and_oracle():
    ...
    parent = ClusterRigidTransform(
        id="P", name="part", helix_ids=["dc_h"], rotation=_quat([0.1, 1.0, 0.3], 28.0),
        translation=[2.0, -1.0, 4.0], pivot=[0.0, 1.0, 0.5])
    d = _seed(ovhg_rotation=_quat([0, 1, 0], 30.0), ovhg_translation=[0.0, 1.4, 0.2],
              parent=parent)
...
def test_validator_flags_uncleared_duplex_pose_and_bad_parent():
    ...
    parent = ClusterRigidTransform(id="P", name="part", helix_ids=["dc_h"],
                                   rotation=[0, 0, 0, 1], translation=[0, 0, 0], pivot=[0, 0, 0])
    d = _seed(ovhg_rotation=_quat([0, 1, 0], 20.0), ovhg_translation=[0.0, 1.0, 0.0], parent=parent)
    mat, _cid = materialize_duplex_cluster(d, "oh_a")
    # Clean materialization validates.
    assert not [r for r in validate_design(mat).results
                if not r.ok and "Cluster hierarchy" in r.message]
    # Re-introduce a stale OverhangSpec pose on the cluster's driver → flagged.
    bad = mat.model_copy(update={"overhangs": [
        o.model_copy(update={"rotation": _quat([1, 0, 0], 10.0)}) if o.id == "oh_a" else o
        for o in mat.overhangs]})
    msgs = [r.message for r in validate_design(bad).results
            if not r.ok and "Cluster hierarchy" in r.message]
    assert msgs and "double-transform" in msgs[0]


@pytest.mark.skipif(not _REAL_FIXTURE.exists(),
...
def test_real_fixture_materialize_beads_neutral_and_no_stray_axis():
    ...
    oh_helices = {dm.helix_id for s in d.strands for dm in s.domains
    ...
    def _snapshot(des):
        ...
        beads = {(n.get("overhang_id"), n["bp_index"], n["direction"]):
                 np.asarray(n.get("backbone_position") or n.get("base_position"), float)
                 for n in nucs if n.get("overhang_id") in (drv, dvn)}
        shafts, centreline = {}, {}
        for a in ax:
            centreline[a["helix_id"]] = (np.asarray(a["start"], float), np.asarray(a["end"], float))
            for oid, seg in (a.get("ovhg_axes") or {}).items():
                if oid in (drv, dvn):
                    shafts[oid] = (np.asarray(seg["start"], float), np.asarray(seg["end"], float))
    ...
    for oid, (ss, ee) in s1.items():
        ...
        assert obeads and min(float(np.linalg.norm((ss + ee) / 2.0 - b)) for b in obeads) < 2.0, oid
...
def test_conjugation_is_identity_when_parent_is_identity():
    ...
    ident = ClusterRigidTransform(id="P", name="part", helix_ids=["dc_h"],
                                  rotation=[0, 0, 0, 1], translation=[0, 0, 0], pivot=[0, 0, 0])
    ...
    q_c, piv_c, t_c = conjugate_world_pose_into_parent_rest(r_w, pivot_world, t_w, ident)
    # R_child == R_world; and applying (q_c, piv_c, t_c) at pivot 0 == the world overlay.
    assert np.allclose(q_c, r_w, atol=1e-6) or np.allclose(q_c, -np.asarray(r_w), atol=1e-6)
