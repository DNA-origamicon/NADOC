...
def test_skipped_square_design_relaxes_and_produces_intact(tmp_path):
    ...
    _skip_if_no_engine()
    design = build_sq_skip_design(
        square_cells(2, 3), 120, 48
    )  # SQ + crossovers + skips
...
def test_full_scale_converges_3x6x400(tmp_path):
    ...
    result = iterate_sq_skips(
        ...
        initial_period=48,
        max_iterations=5,
        production_steps=8_000_000,
        screen_steps=2_000_000,
        max_production_rounds=6,
        timeout=36000.0,
        backend="CUDA",
        **hox.STANDARD_RELAX_PARAMS,
    )
