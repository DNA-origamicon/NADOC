...
def _design_with_two_cluster_ops():
    ...
    ct = ClusterRigidTransform(
        id="A", helix_ids=["h0"],
        translation=[2.0, 0.0, 0.0], rotation=[0, 0, 0, 1], pivot=[0, 0, 0],
    )
    op0 = ClusterOpLogEntry(
        cluster_id="A", translation=[1.0, 0.0, 0.0],
        rotation=[0, 0, 0, 1], pivot=[0, 0, 0],
    )
    op1 = ClusterOpLogEntry(
        cluster_id="A", translation=[2.0, 0.0, 0.0],
        rotation=[0, 0, 0, 1], pivot=[0, 0, 0],
    )
...
def test_edit_latest_cluster_op_updates_live_pose():
    ...
    out = edit_cluster_op_entry(d, 1, entry, {
        "translation": [9.0, 0.0, 0.0], "rotation": [0, 0, 0, 1], "pivot": [0, 0, 0],
    })
    # Log entry rewritten...
...
def test_edit_earlier_cluster_op_preserves_latest_pose():
    ...
    out = edit_cluster_op_entry(d, 0, entry, {
        "translation": [5.0, 0.0, 0.0], "rotation": [0, 0, 0, 1], "pivot": [0, 0, 0],
    })
...
def test_edit_cluster_op_does_not_mutate_input_design():
    d = _design_with_two_cluster_ops()
    edit_cluster_op_entry(d, 1, d.feature_log[1], {
        "translation": [9.0, 0.0, 0.0], "rotation": [0, 0, 0, 1], "pivot": [0, 0, 0],
    })
...
def test_edit_cluster_op_missing_cluster_is_404():
    ...
    ghost = ClusterOpLogEntry(
        cluster_id="GONE", translation=[1, 0, 0], rotation=[0, 0, 0, 1], pivot=[0, 0, 0],
    )
    with pytest.raises(FeatureEditError) as ei:
        edit_cluster_op_entry(d, 1, ghost, {
            "translation": [1, 0, 0], "rotation": [0, 0, 0, 1], "pivot": [0, 0, 0],
        })
...
def _design_with_one_bend():
    op = DeformationOp(
        id="d1", type="bend", plane_a_bp=0, plane_b_bp=10,
        affected_helix_ids=["h0"], cluster_ids=[],
...
def test_edit_deformation_rewrites_op_and_rebuilds_set():
    ...
    entry = d.feature_log[0]
    out = edit_deformation_entry(d, 0, entry, {
        "type": "bend", "plane_a_bp": 0, "plane_b_bp": 20,
...
def test_edit_deformation_preserves_op_id():
    d = _design_with_one_bend()
    out = edit_deformation_entry(d, 0, d.feature_log[0], {
        "type": "bend", "plane_a_bp": 0, "plane_b_bp": 15,
        "affected_helix_ids": ["h0"], "params": {"curvature_deg_per_bp": 0.5},
    })
...
def test_edit_deformation_bad_type_is_400():
    d = _design_with_one_bend()
    with pytest.raises(FeatureEditError) as ei:
        edit_deformation_entry(d, 0, d.feature_log[0], {
            "type": "wobble", "plane_a_bp": 0, "plane_b_bp": 10, "params": {},
        })
    assert ei.value.status == 400
...
def test_edit_deformation_missing_planes_is_400():
    ...
    with pytest.raises(FeatureEditError) as ei:
        edit_deformation_entry(d, 0, d.feature_log[0], {
            "type": "bend", "params": {"curvature_deg_per_bp": 1.0},
        })
    assert ei.value.status == 400
...
def test_edit_deformation_no_snapshot_is_409():
    ...
    with pytest.raises(FeatureEditError) as ei:
        edit_deformation_entry(d, 0, entry, {
            "type": "bend", "plane_a_bp": 0, "plane_b_bp": 10,
            "affected_helix_ids": ["h0"], "params": {"curvature_deg_per_bp": 1.0},
        })
...
def test_edit_deformation_does_not_mutate_input():
    d = _design_with_one_bend()
    edit_deformation_entry(d, 0, d.feature_log[0], {
        "type": "bend", "plane_a_bp": 0, "plane_b_bp": 99,
        "affected_helix_ids": ["h0"], "params": {"curvature_deg_per_bp": 3.0},
    })
