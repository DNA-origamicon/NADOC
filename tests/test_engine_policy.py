...
def test_no_protein_gpu_free_picks_oxdna_cuda():
    r = recommend_engine(has_proteins=False, gpu_busy=False, n_nucleotides=1328, free_cores=16)
...
def test_no_protein_gpu_busy_picks_lammps_cpu_with_factor():
    r = recommend_engine(has_proteins=False, gpu_busy=True, gpu_hog_name="namd3",
                         n_nucleotides=14172, free_cores=12)
...
def test_proteins_force_oxdna_even_when_gpu_busy():
    r = recommend_engine(has_proteins=True, gpu_busy=True, gpu_hog_name="namd3",
                         n_nucleotides=5000, free_cores=8)
    assert r["engine"] == "oxdna" and r["backend"] == "CUDA"
...
def test_proteins_gpu_free_no_dialog():
    r = recommend_engine(has_proteins=True, gpu_busy=False, n_nucleotides=5000, free_cores=8)
