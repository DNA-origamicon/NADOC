...
def _bead(index, aa, chain, res, x, prev):
    return ProteinBead(index=index, aa=aa, chain_id=chain, res_seq=res,
                       pos_nm=np.array([x, 0.0, 0.0]), prev_index=prev)
...
def _chain_block(n, x0=0.0, step=0.4, chain="A"):
    ...
    return [_bead(i, "A", chain, i + 1, x0 + step * i, i - 1 if i else -1) for i in range(n)]
...
def test_hybrid_header_five_fields():
    ...
    assert header == [str(n_prot + n_dna), str(n_dna_strands + 1),
                      str(n_dna), str(n_prot), str(n_dna_strands)]
...
def test_anchor_trap_on_centroid_bead_in_oxdna_units():
    ...
    beads = _chain_block(3, x0=0.0, step=1.0)            # x = 0,1,2 ; centroid bead = index 1 (x=1)
...
def test_binder_terminus_picks_nearest_end(monkeypatch):
    ...
    att = types.SimpleNamespace(target=types.SimpleNamespace(overhang_id="ov", attach_end="free_end"))
    geometry = [
        {"strand_id": "bndr", "is_five_prime": True, "is_three_prime": False,
         "backbone_position": [0.0, 0, 0], "helix_id": "h0", "bp_index": 5, "direction": "FORWARD"},
        {"strand_id": "bndr", "is_five_prime": False, "is_three_prime": True,
         "backbone_position": [5.0, 0, 0], "helix_id": "h0", "bp_index": 0, "direction": "FORWARD"},
    ]
    # anchor sits at x=0.2 → nearer the 5′ terminus (bp 5)
    monkeypatch.setattr(mod, "resolve_overhang_anchor",
                        lambda *a, **k: (__import__("numpy").array([0.2, 0, 0]), None), raising=False)
    # resolve_overhang_anchor is imported inside the fn; patch the source module too
    import backend.core.protein as protein_mod
    monkeypatch.setattr(protein_mod, "resolve_overhang_anchor",
...
def test_protein_forces_free_proteins_get_anchor_traps():
    ...
    atts = [types.SimpleNamespace(target=types.SimpleNamespace()),
...
def _spec(**kw):
    ...
    base = dict(name="2_md_relax", kind="md_relax", sim_type="MD", steps=1000, backend="CPU")
    base.update(kw)
...
def test_render_hybrid_emits_dnanm_and_parfile():
    from backend.core.oxdna_protocol import render_stage_input
    txt = render_stage_input(_spec(interaction="DNANM", parfile="anm.par"), "t.top", "c.dat")
...
def test_render_hybrid_relax_emits_relax_type():
    ...
    txt = render_stage_input(
        _spec(interaction="DNANM_relax", parfile="anm.par", relax_type="harmonic_force"),
        "t.top", "c.dat")
...
def test_render_hybrid_mc_gets_refresh_vel():
    ...
    from backend.core.oxdna_protocol import render_stage_input
    mc = _spec(name="1_mc", kind="mc", sim_type="MC", interaction="DNANM_relax", parfile="anm.par")
...
def _protein_design():
    ...
    from backend.core.models import (
        ProteinAsset, ProteinAtom, ProteinAttachment, ProteinTargetFree,
    )
    d = make_6hb_design()
    for s in d.strands:
        n = sum(abs(dom.end_bp - dom.start_bp) + 1 for dom in s.domains)
        s.sequence = ("ACGT" * (n // 4 + 1))[:n]
    atoms = [ProteinAtom(serial=i, name="CA", element="C", res_name="ALA",
                         chain_id="A", res_seq=i + 1, x=2.0 + 0.38 * i, y=2.0, z=2.0)
             for i in range(5)]
    asset = ProteinAsset(name="p", atoms=atoms, center_of_mass=[2.5, 2.0, 2.0])
    d.protein_assets = [asset]
    d.protein_attachments = [ProteinAttachment(asset_id=asset.id, target=ProteinTargetFree())]
    return d
...
def test_prepare_writes_hybrid_files_with_offset_traps(tmp_path):
    ...
    geometry = _geometry_for_design(design)
    specs = build_relaxation_stages(mc_steps=50, md_relax_steps=50, equil_steps=50, protein=True)
    ...
    mt_particles = [int(ln.split("=")[1]) for ln in forces.splitlines()
...
@pytest.mark.skipif(__import__("backend.core.oxdna_runner", fromlist=["find_oxdna_anm"])
                    .find_oxdna_anm() is None, reason="ANM-oxDNA fork binary not built")
def test_prepared_hybrid_job_runs_on_fork(tmp_path):
    ...
    design = _protein_design()
    geometry = _geometry_for_design(design)
    specs = build_relaxation_stages(mc_steps=100, md_relax_steps=100, equil_steps=50,
                                    backend="CPU", protein=True)
    job = new_oxdna_job(design_name="prot", stages=[s.to_status() for s in specs])
    prepare_oxdna_job(design, geometry, job, tmp_path, specs)
    jd = job.job_dir(tmp_path)
    ...
    inp = render_stage_input(
        spec, str((jd / "topology.top").resolve()), str((jd / "conf.dat").resolve()),
        forces_name=str((jd / "forces.txt").resolve()),
        parfile_name=str((jd / "anm.par").resolve()))
    (stage / "input.txt").write_text(inp)
    r = subprocess.run([find_oxdna_anm(), str(stage / "input.txt")],
                       cwd=stage, capture_output=True, text=True, timeout=180)
...
def test_read_configuration_skips_protein_lead(tmp_path):
    ...
    from backend.api.crud import _geometry_for_design
    from backend.physics.oxdna_interface import read_configuration_full, write_configuration
    from backend.physics.oxdna_protein import hybrid_configuration_text
    ...
    k = next(iter(ref))
    assert np.allclose(got[k]["backbone_position"], ref[k]["backbone_position"], atol=1e-5)
...
def test_protein_display_transform_recovers_known_rigid_motion(tmp_path):
    ...
    from backend.core.protein_cg import ProteinBead
    from backend.physics.oxdna_protein import (
        build_protein_blocks, hybrid_configuration_text, protein_display_transforms,
    )
    ...
    relaxed_beads = [ProteinBead(index=b.index, aa=b.aa, chain_id=b.chain_id,
                                 res_seq=b.res_seq, pos_nm=R0 @ b.pos_nm + t0,
                                 prev_index=b.prev_index) for b in design_beads]
