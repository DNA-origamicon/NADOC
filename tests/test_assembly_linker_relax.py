    ...
    a = Assembly(
        instances=[
            PartInstance(
                id="inst-A",
                name="PartA",
                source=PartSourceInline(design=d_a),
                fixed=a_fixed,
            ),
            PartInstance(
                id="inst-B",
...
def _live_anchor(inst_id: str, oh_id: str, attach: str):
    ...
    return _world_anchor_axial(
        design, inst, oh_id, attach, _find_overhang_domain(design, oh_id)
    )
...
def test_relax_is_translation_only():
    ...
    T_before = np.array(
        assembly_state.get_or_404().instances[1].transform.values
    ).reshape(4, 4)
    ...
    assert T_after[:3, :3] == pytest.approx(
        T_before[:3, :3], abs=1e-9
...
def test_relax_collapses_both_connector_arcs():
    ...
    assert post["a"] == pytest.approx(0.0, abs=1e-6), (
        f"fixed-side arc not closed: {post}"
    )
    assert post["b"] == pytest.approx(0.0, abs=1e-6), (
        f"moved-side arc not closed: {post}"
    )
...
def test_relax_logs_one_entry_and_undo_restores_pose():
    ...
    assert (
        sum(e.op_kind == "assembly-overhang-connection-relax" for e in asm.feature_log)
        == 1
    )
...
def test_linker_geometry_emits_bridge_nucleotides():
    ...
    for linker_type, attach in (
        ("ds", ("root", "free_end")),
        ("ss", ("free_end", "free_end")),
    ):
        _seed()
        r = client.post(
            "/api/assembly/overhang-connections",
        ...
        assert len(bridge) > 0, (
            f"[{linker_type}] no bridge nucleotides emitted on __lnk__ helix"
        )
...
def test_linker_complement_phase_matches_tilted_overhang():
    ...
    t_b = Mat4x4(
        values=[1, 0, 0, 10, 0, c, -s, 0, 0, s, c, 0, 0, 0, 0, 1]
    )  # tilt about X + offset
    d_a = _design_with_real_oh("oh-A_5p", "ACGTACGT")
    d_b = _design_with_real_oh("oh-B_3p", "GGGGCCCC")
    assembly_state.set_assembly(
        Assembly(
            instances=[
                PartInstance(
                    id="inst-A", name="A", source=PartSourceInline(design=d_a)
                ),
                PartInstance(
                    id="inst-B",
                    name="B",
    ...
    expected = {
        ...
        for i in range(len(arrs["bp_indices"]))
        if int(arrs["directions"][i]) == comp_dir_int
    }
    ...
    for bp in sorted(shared):
        assert actual[bp] == pytest.approx(expected[bp], abs=1e-6), (
            f"complement bp {bp} at wrong phase"
        )
...
def test_relax_missing_connection_404():
    _seed()
    assert (
        client.get("/api/assembly/overhang-connections/nope/relax-status").status_code
        == 404
    )
    assert (
        client.post("/api/assembly/overhang-connections/nope/relax").status_code == 404
    )
