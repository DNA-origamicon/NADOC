...
def _energy(rows: int) -> str:
    ...
    return "".join(
        f"{i * EVERY * 0.005:14.4f}  -1.24  0.29  -0.95 \n" for i in range(rows)
    )
...
def test_torn_checkpoint_falls_back_to_last_complete_trajectory_frame(stage):
    (stage / "trajectory.dat").write_text(
        _frame(100_000) + _frame(200_000) + _frame(300_000)
    )
...
def test_recovered_conf_is_written_to_a_separate_file(stage):
    ...
    assert (stage / "last_conf.dat").read_text() == "garbage", (
        "must not touch last_conf"
    )


def test_truncated_final_trajectory_frame_is_skipped(stage):
    (stage / "trajectory.dat").write_text(
        _frame(100_000) + _frame(200_000) + "t = 300000\nb = 1 1 1\n"
    )
    found = last_complete_trajectory_frame(stage / "trajectory.dat", N)
    assert found is not None
    _text, step = found
    assert step == 200_000, (
        "the half-written tail frame must not be offered as a restart point"
    )
...
def test_diverged_attempt_is_skipped_in_favour_of_the_previous_one(stage):
    ...
    (stage / "trajectory.r1.dat").write_text(
        _frame(100_000) + _frame(200_000)
    )  # good attempt
    ...
    conf, consumed, note = resume_point(stage, _spec(), N)
    assert conf.read_text().startswith("t = 200000"), (
        "must come from the pre-divergence attempt"
    )
...
def test_cheap_and_authoritative_agree_within_one_frame_interval(stage):
    (stage / "trajectory.r1.dat").write_text(
        "".join(_frame(s) for s in range(100_000, 600_000, 100_000))
    )
