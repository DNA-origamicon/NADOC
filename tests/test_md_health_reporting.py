...
def _result(**over) -> HealthCheckResult:
    ...
    base = dict(
        passed=True, blocking=False, reason="",
        c1_paired_fraction=0.95, c1_mean_ang=9.1, c1_p90_ang=10.0,
        wc_ref_relative_fraction=0.88, wc_mean_hbond_ang=3.0,
        broken_bp_count=7, charge_within_shell_e=-12.5,
        per_frame_ran=True,
    )
    base.update(over)
...
class TestSampleConstruction:
    def test_from_result_carries_the_per_frame_diagnostics(self) -> None:
        s = MdHealthSample.from_result(_result(), "310K NPT production", "seg1", blocking=False)
    ...
    def test_blocking_is_explicit_not_copied(self) -> None:
        ...
        assert MdHealthSample.from_result(hard, "s", "g", blocking=False).blocking is False
        assert MdHealthSample.from_result(hard, "s", "g", blocking=True).blocking is True

    def test_diagnostics_records_a_failure_verbatim(self) -> None:
        s = MdHealthSample.from_result(
            _result(broken_bp_count=None, charge_within_shell_e=None,
                    per_frame_ran=False, diagnostics_error="frame 812: truncated DCD"),
            "s", "g", blocking=False)
    ...
    def test_a_probe_that_skipped_the_loop_leaves_diagnostics_unset(self) -> None:
        ...
        s = MdHealthSample.from_result(
            _result(broken_bp_count=None, charge_within_shell_e=None, per_frame_ran=False),
            "s", "g", blocking=False)
    ...
    def test_roundtrips_through_disk(self, tmp_path: Path) -> None:
        ...
        job.health_samples.append(
            MdHealthSample.from_result(_result(), "310K NPT production", "seg1", blocking=False))
        job.health_probe = {"enabled": True, "interval_s": 300.0, "last_at": 1.0,
    ...
    def test_an_old_sample_without_diagnostics_still_loads(self, tmp_path: Path) -> None:
        ...
        raw["health_samples"] = [{
            "wall_time": 1.0, "stage": "s", "segment": "g",
            "c1_paired_fraction": 0.9, "passed": True,
        }]
...
class TestJsonlHasSegment:
    ...
    def test_an_all_null_record_does_not_count_as_recorded(self, tmp_path: Path) -> None:
        ...
        p = tmp_path / "metrics.jsonl"
        self._write(p, [{"segment": "seg1", "temperature_k": None, "pressure_bar": None,
                         "ns_per_day": None, "volume_ang3": None}])
        assert namd_runner._jsonl_has_segment(p, "seg1") is False

    def test_a_real_failure_does_count_so_it_is_not_retried_forever(self, tmp_path: Path) -> None:
        p = tmp_path / "health.jsonl"
        self._write(p, [{"segment": "seg1", "c1_paired_fraction": None,
    ...
    def test_a_torn_final_line_is_ignored(self, tmp_path: Path) -> None:
        p = tmp_path / "health.jsonl"
        p.write_text(json.dumps({"segment": "seg1", "c1_paired_fraction": 0.9}) + "\n"
                     + '{"segment": "seg2", "c1_pai')
        assert namd_runner._jsonl_has_segment(p, "seg1") is True
...
class TestHealthProbeBookkeeping:
    ...
    def test_adopted_is_a_durable_fact_not_a_transient_reason(self, tmp_path: Path) -> None:
    ...
    def test_never_raises(self, tmp_path: Path) -> None:
        ...
        broken = SimpleNamespace(job_id="x", health_probe=None,
...
class TestInflightTick:
    ...
    def _spec(self):
        return SimpleNamespace(name="seg1", stage="310K NPT production",
                               min_c1_paired=0.90, min_wc_ref_relative=0.85)
    ...
    def _wire(self, tmp_path: Path, monkeypatch, result):
        ...
        tick = namd_runner._make_inflight_health_tick(job, self._spec(), pkg, out, tmp_path)
    ...
    def test_too_few_frames_is_not_reported_as_a_failure(self, tmp_path: Path, monkeypatch) -> None:
        job, tick, _ = self._wire(tmp_path, monkeypatch, HealthCheckResult(
    ...
    def test_a_real_error_is_published_not_swallowed(self, tmp_path: Path, monkeypatch) -> None:
        job, tick, _ = self._wire(tmp_path, monkeypatch, HealthCheckResult(
            passed=False, error="PSF or PDB not found in pkg"))
        asyncio.run(tick())
        assert job.health_samples == []
    ...
    def test_started_at_is_the_probe_clock_not_the_job_clock(
        ...
        out.mkdir(parents=True)
        namd_runner._make_inflight_health_tick(job, self._spec(), tmp_path / "pkg", out, tmp_path)
        assert job.health_probe["started_at"] > job.created_at

    def test_a_new_segment_resets_the_counters(self, tmp_path: Path, monkeypatch) -> None:
        """Otherwise a stale last_at from the previous segment answers for this one."""
        monkeypatch.setattr(namd_runner, "_INFLIGHT_HEALTH_INTERVAL_S", 300.0)
        job = _job_for_probe(tmp_path)
        job.health_probe = {"enabled": True, "interval_s": 300.0, "started_at": 1.0,
                            "last_tick_at": 2.0, "last_at": 3.0,
                            "last_error": "an old failure", "reason": "an old reason"}
        out = tmp_path / "pkg" / "output"
        out.mkdir(parents=True)
        namd_runner._make_inflight_health_tick(job, self._spec(), tmp_path / "pkg", out, tmp_path)
    ...
    def test_the_probe_is_published_before_it_first_runs(self, tmp_path: Path, monkeypatch) -> None:
    ...
    def test_disabled_sampling_is_published_as_such(self, tmp_path: Path, monkeypatch) -> None:
...
class TestAdoptedRunIsGuarded:
    ...
    def test_ticks_while_the_adopted_process_lives(self, tmp_path: Path, monkeypatch) -> None:
        alive = [True, True, False]
        monkeypatch.setattr(namd_runner, "_segment_process_running",
                            lambda _n: alive.pop(0) if alive else False)
        ticks = []
        rc = asyncio.run(namd_runner._wait_for_segment_process(
            "seg1", poll=0.0, guard_dir=tmp_path, on_tick=lambda: ticks.append(1)))
        assert rc == 0
        assert ticks, "an adopted segment must still be sampled"

    def test_disk_guard_aborts_an_adopted_run(self, tmp_path: Path, monkeypatch) -> None:
        ...
        monkeypatch.setattr(namd_runner, "_kill_process_group", lambda pid: killed.append(pid))
        rc = asyncio.run(namd_runner._wait_for_segment_process(
            "seg1", poll=0.0, guard_dir=tmp_path))
        assert rc == DISK_ABORT_RC
        assert killed == [4242], "filling the disk is worse than killing the orphan"

    def test_a_raising_probe_cannot_kill_the_run_it_watches(self, tmp_path: Path, monkeypatch) -> None:
        alive = [True, False]
        monkeypatch.setattr(namd_runner, "_segment_process_running",
                            lambda _n: alive.pop(0) if alive else False)
    ...
    def test_without_a_guard_dir_it_is_the_old_plain_wait(self, tmp_path: Path, monkeypatch) -> None:
        alive = [True, False]
        monkeypatch.setattr(namd_runner, "_segment_process_running",
                            lambda _n: alive.pop(0) if alive else False)
...
def stub_health(monkeypatch, tmp_path):
    ...
    def _install(n_frames: int, bad: int | None = None):
        ...
        monkeypatch.setitem(sys.modules, "MDAnalysis",
                            SimpleNamespace(Universe=lambda *a, **k: uni))
        ...
        monkeypatch.setattr(H, "c1_metrics_from_dcd", lambda *a, **k: {
            "paired_fraction": 0.95, "mean_c1_ang": 9.0, "p90_c1_ang": 10.0,
            "max_c1_ang": 11.0, "n_pairs": 1, "frame": n_frames - 1})
        monkeypatch.setattr(H, "wc_metrics_from_dcd", lambda *a, **k: {
            "absolute_paired_fraction": 0.9, "ref_relative_paired_fraction": 0.9,
            "mean_hbond_proxy_ang": 3.0, "p90_max_hbond_proxy_ang": 3.5, "n_pairs": 1})
        monkeypatch.setattr(H, "_shell_selections", lambda _u: (_np_arange(1), _np_arange(1)))
        monkeypatch.setattr(H, "wc_hbond_atoms", lambda _u: (_np_arange(1), _np_arange(1)))
        ...
        monkeypatch.setattr(H, "wc_frame_metrics",
                            lambda *a, **k: {"ref_relative_paired_fraction": 0.9})
        return uni
...
class TestPerFrameDiagnostics:
    ...
    def test_a_torn_frame_drops_one_frame_not_the_whole_series(self, stub_health) -> None:
    ...
    def test_the_verdict_is_unaffected_by_a_diagnostics_failure(self, stub_health) -> None:
        from backend.core.md_health import run_health_check
...
class TestTierAContract:
    def test_per_frame_defaults_to_true(self) -> None:
        ...
        assert inspect.signature(run_health_check).parameters["per_frame"].default is True
...
class TestEarlyStopShortSeries:
    def test_wc_plateaued_tolerates_a_short_chunk(self) -> None:
        ...
        assert wc_plateaued(flat[1:]) is True, "one fewer leading frame must not flip it"
