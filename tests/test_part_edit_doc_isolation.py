...
def test_part_editor_docs_do_not_clobber_each_other():
    ...
    insts = {i["name"]: i["id"] for i in v1_instances(client.get("/api/assembly", headers=_h(DOC_A)).json())}
    inst_a, inst_b = insts["A"], insts["B"]

    # Part A editor: fetch source from the assembly doc, import into its OWN doc.
    src_a = client.get(f"/api/assembly/instances/{inst_a}/design", headers=_h(DOC_A)).json()["design"]
    assert client.post("/api/design/import", json={"content": json.dumps(src_a)}, headers=_h(DOC_PA)).status_code == 200
    # Part B editor: same, into a different doc.
    src_b = client.get(f"/api/assembly/instances/{inst_b}/design", headers=_h(DOC_A)).json()["design"]
    assert client.post("/api/design/import", json={"content": json.dumps(src_b)}, headers=_h(DOC_PB)).status_code == 200
...
def test_save_back_reaches_assembly_doc_without_touching_editor_docs(tmp_path, monkeypatch):
    ...
    for inst, doc in ((inst_a, DOC_PA), (inst_b, DOC_PB)):
        src = client.get(f"/api/assembly/instances/{inst}/design", headers=_h(DOC_A)).json()["design"]
        client.post("/api/design/import", json={"content": json.dumps(src)}, headers=_h(doc))
    ...
    got_a = client.get(f"/api/assembly/instances/{inst_a}/design", headers=_h(DOC_A)).json()["design"]
    got_b = client.get(f"/api/assembly/instances/{inst_b}/design", headers=_h(DOC_A)).json()["design"]
