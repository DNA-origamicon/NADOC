...
def test_rise_per_bp():
    ...
    positions = nucleotide_positions(helix)
    fwd = sorted(
        [p for p in positions if p.direction == Direction.FORWARD],
...
def test_rise_accumulated():
    ...
    helix = make_z_helix(length)
    fwd = sorted(
        [p for p in nucleotide_positions(helix) if p.direction == Direction.FORWARD],
        key=lambda p: p.bp_index,
    )
...
def test_phase_offset():
    ...
    fwd0 = next(
        p
        for p in nucleotide_positions(h0)
        if p.bp_index == 0 and p.direction == Direction.FORWARD
    )
    fwdpi2 = next(
        p
        for p in nucleotide_positions(hpi2)
        if p.bp_index == 0 and p.direction == Direction.FORWARD
    )
...
def test_base_pair_bead_distance():
    ...
    """
    backbone_pair_dist = (
        2.0 * HELIX_RADIUS * math.sin(BDNA_MINOR_GROOVE_ANGLE_RAD / 2.0)
    )
    ...
    for bp, strands in by.items():
        dist = np.linalg.norm(
            strands[Direction.FORWARD].base_position
            - strands[Direction.REVERSE].base_position
        )
        assert abs(dist - expected) < 1e-9, (
...
def _make_unequal_bundle():
    ...
    short_h = make_z_helix(100).model_copy(
        update={
            "id": "h_short",
...
def _bend_op(plane_a, plane_b, angle_deg, direction_deg=0.0):
    ...
    return DeformationOp(
        ...
        params=BendParams(
            curvature_deg_per_bp=angle_deg / span, direction_deg=direction_deg
        ),
    )
...
def test_single_bend_reduces_to_arc_formula():
    ...
    for p in (0, 37, 100, N):
        theta = p * angle_rad / N
        expect = (
            radius * (1 - math.cos(theta)) * world_dir
            + radius * math.sin(theta) * tangent0
        )
...
def test_single_twist_is_straight_axial_spin():
    ...
    N, total = 200, 45.0
    design = Design(helices=[make_z_helix(N)], deformations=[_twist_op(0, N, total)])
    for p in (0, 80, N):
        ...
        expect_R = _rot_around_axis(
            np.array([0.0, 0.0, 1.0]), math.radians(total) * p / N
        )
...
def test_overlapping_bend_twist_composes_not_drops():
    ...
    N = 200
    bend_only = Design(helices=[make_z_helix(N)], deformations=[_bend_op(0, N, 90.0)])
    twist_only = Design(helices=[make_z_helix(N)], deformations=[_twist_op(0, N, 60.0)])
    combined = Design(
        helices=[make_z_helix(N)],
...
def test_scalar_and_vectorised_frames_agree_for_overlap():
    ...
    design = Design(
        helices=[make_z_helix(N)],
        deformations=[_bend_op(0, N, 80.0, direction_deg=30.0), _twist_op(0, N, 55.0)],
    )
...
def test_combined_matches_fine_step_integration():
    ...
    design = Design(
        helices=[make_z_helix(N)],
        deformations=[_bend_op(0, N, angle, direction_deg=0.0), _twist_op(0, N, total)],
    )
...
def _js_const(path: str, name: str) -> float:
    ...
    m = re.search(
        rf"^\s*(?:export\s+)?const\s+{name}\s*=\s*(.+?)\s*(?://.*)?$", src, re.M
    )
    assert m, (
        f"{name} not found in {path} — did the module move or the constant rename?"
    )
    expr = m.group(1).strip().rstrip(";").replace("Math.", "")
    assert re.fullmatch(r"[-\d.eE+*/() sqrtPIabspow,]+", expr), (
        f"{name}: unparsed {expr!r}"
    )
...
def test_frontend_bdna_constants_match_the_backend():
    ...
    assert _js_const(js, "HONEYCOMB_COL_PITCH") == pytest.approx(
        HONEYCOMB_COL_PITCH, abs=1e-12
    )
    assert _js_const(js, "HONEYCOMB_ROW_PITCH") == pytest.approx(
        HONEYCOMB_ROW_PITCH, abs=1e-12
    )
        ...
        for helix in design.helices:
            ...
            for (bp, d), idxs in rows.items():
                ...
                for k, i in enumerate(idxs):
                    ck = _compute_nuc_geometry_copy(
                        design, helix.id, bp, d, k, len(idxs)
                    )
                    copies_checked += 1
                    assert np.allclose(
...
def test_honeycomb_crossover_geometry_does_not_drift_along_a_helix():
    ...
    for k in range(0, 210 - 21, 21):
        ...
        assert abs(d) < 1e-9, (
            f"azimuth drifted {d:+.6f} deg over the 21-bp repeat at bp {k}"
        )
...
def test_the_atomistic_representation_is_periodic_over_the_21bp_repeat_too():
    ...
    relocated = {
        x.half_a.index for x in design.crossovers if x.half_a.helix_id == helix.id
    }
    relocated |= {
        x.half_b.index for x in design.crossovers if x.half_b.helix_id == helix.id
    }
