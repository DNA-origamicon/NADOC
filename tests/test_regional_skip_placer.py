...
def _first_helix_with_budget(design, budget_per_helix, want=3):
    for h in sorted(design.helices, key=lambda h: h.id):
        if len(core_candidates(design, h)) >= want * 4 and budget_per_helix.get(h.id, 0) >= want:
            return h
...
def test_deviation_attracts(sq_design):
    ...
    assert any(ls.bp_index == target_bp for ls in mods[h.id]), "deviation spike not targeted"
...
def test_strain_repels(sq_design):
    ...
    assert all(ls.bp_index != spike_bp for ls in mods[h.id]), "placed onto a strain hotspot"
...
def test_deterministic(sq_design):
    budget = budget_from_uniform_period(sq_design, 24)
    dev = {(h.id, core_candidates(sq_design, h)[0]): 5.0
           for h in sq_design.helices if core_candidates(sq_design, h)}
...
def test_budget_matches_uniform_density(sq_design):
    ...
    for hid, count in budget.items():
        assert len(mods.get(hid, [])) == min(count, len(core_candidates(
...
def test_build_regional_skip_design_preserves_count_and_resequences():
    ...
    from backend.api.skip_twist_tuning import (
        build_regional_skip_design, build_sq_skip_design,
    )
    from backend.core.loop_skip_calculator import sq_lattice_periodic_skips
    ...
    period = 24
    uniform_total = sum(len(s) for s in sq_lattice_periodic_skips(base, period).values())

    regional = build_regional_skip_design(base, period, {}, {})   # empty fields => even spread
...
def test_build_explicit_skip_from_design_lays_exact_pattern():
    ...
    from backend.api.skip_twist_tuning import build_explicit_skip_from_design, build_sq_skip_design
    from backend.physics.oxdna_interface import designed_pair_complementarity
...
def test_twist_profile_endpoint_matches_scalar_and_is_monotonic():
    ...
    assert all(b >= a - 1e-6 for a, b in zip(vals, vals[1:]))   # monotonic non-decreasing
...
def test_redistribute_follows_overtwist_and_is_deletion_only():
    ...
    for h in base.helices:
        ...
        if len(cands) < 10:
            continue
        lo, hi = cands[0], cands[-1]; rng = (hi - lo) or 1
        frac = [(ls.bp_index - lo) / rng for ls in placed]
        mid = sum(1 for f in frac if 0.4 <= f <= 0.6)      # the over-wound fifth (20% of span)
...
def test_redistribute_staggers_across_helices():
    ...
    mods = redistribute_by_twist_profile(base, budget, [(0.0, 0.0), (10.0, 0.0)],
                                         gain=0.5, base=1.0)   # flat → pure stagger test
    first_local = []
...
def test_redistribute_flat_profile_spreads_evenly():
    ...
    mods = redistribute_by_twist_profile(base, budget, [(0.0, 0.0), (10.0, 0.0)],
                                         gain=20.0, base=1.0, min_spacing=4)
    for h in base.helices:
        placed = sorted(ls.bp_index for ls in mods.get(h.id, []))
        if len(placed) < 3:
            continue
        gaps = [b - a for a, b in zip(placed, placed[1:])]
        assert max(gaps) <= 3 * (sum(gaps) / len(gaps))    # no clustering — gaps stay even-ish
...
def _bundle_positions(kink_z=0.0):
    ...
    for bp in range(n):
        ...
        for hid, y, d in (("A", 1.0, "fwd"), ("B", -1.0, "rev")):
            out.append({"helix_id": hid, "bp_index": bp, "direction": d,
...
def test_backbone_strain_field_aggregates_per_bp(monkeypatch):
    ...
    monkeypatch.setattr(health, "oxdna_backbone_site", lambda pos, a1, a3: pos)  # identity
    U = OXDNA_LENGTH_UNIT
    full_map = {
        a: {"backbone_position": np.array([0.0, 0, 0]), "a1": 0, "a3": 0},
        b: {"backbone_position": np.array([R0 * U, 0, 0]), "a1": 0, "a3": 0},          # a-b ≈ R0 → 0 strain
        c: {"backbone_position": np.array([R0 * U + 2.0 * U, 0, 0]), "a1": 0, "a3": 0},  # b-c = 2.0 units
    }
    field = health.backbone_strain_field(object(), full_map)
    assert field[("H", 0)] == pytest.approx(0.0, abs=1e-6)        # only the relaxed bond
    assert field[("H", 2)] == pytest.approx(2.0 - R0, abs=1e-6)   # only the stretched bond
