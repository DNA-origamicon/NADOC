...
def test_displaced_nucleotide_flags_backbone_and_hidden(design, tmp_path):
    ...
    frame[key]["backbone_position"] = frame[key]["backbone_position"] + np.array(
        [5.0, 0, 0]
    )
    ...
    assert not r["ok"]
    assert (
        r["n_rigid_stamp_violations"] == 0
    )  # displacement is not a template-integrity bug
    ...
    stretched = [
        b for b in r["invalid_bonds"] if b["class"] in ("backbone", "bridge", "linker")
    ]
    assert stretched
...
def test_backbone_closure_connects_and_preserves_rigid(design, tmp_path):
    ...
    open_m = build_atomistic_model(
        design, nuc_pos_override=npo, axis_override=axo, close_backbone=False
    )
    closed = build_atomistic_model(
        design, nuc_pos_override=npo, axis_override=axo, close_backbone=True
    )
    ...
    def _bb_max(m):
        ...
        return max(
            ...
            for i, j in m.bonds
            if _bond_class(m.atoms[i], m.atoms[j]) == "backbone"
        )

    assert _bb_max(closed) < _bb_max(
        open_m
    )  # closure shortened the worst backbone stick
    ...
    assert [(a.x, a.y, a.z) for a in d_default.atoms] == [
        (a.x, a.y, a.z) for a in d_off.atoms
    ]
...
def test_base_geometry_detects_collapse(design, tmp_path):
    ...
    fwd_c1 = {
        (a.helix_id, a.bp_index): (a.x, a.y, a.z)
        for a in model.atoms
        if a.name == "C1'" and a.direction == "FORWARD"
    }
    crushed = []
    for a in model.atoms:
        if (
    ...
    bg2 = _base_geometry(
        AtomisticModel(atoms=crushed, bonds=model.bonds), wc_collapse_nm=WC_COLLAPSE_NM
    )
...
def test_wc_helix_imbalance_detector(design, tmp_path):
    ...
    for a in model.atoms:
        if (
            ...
            out.append(a)
    bg2 = _base_geometry(
        AtomisticModel(atoms=out, bonds=model.bonds),
...
def test_strand_identity_preserved_nadoc_to_atomistic(design, tmp_path):
    ...
    key_strand = {
        (d.helix_id, bp, d.direction.value): s.id
    ...
    assert Counter(a.strand_id for a in ref.atoms) == Counter(
        a.strand_id for a in disp.atoms
    )


def test_display_topology_hash_guards_against_design_drift(
    ...
    job.save(ws)
    sd = job.stage_dir(ws, "3_equil")
    sd.mkdir(parents=True, exist_ok=True)
    write_configuration(
        design, _geometry_for_design(design), sd / "last_conf.dat", box_nm=200.0
    )
    ...
    assert (
        model["topology_hash"] == disp["topology_hash"]
    )  # same topology as the positions
    assert len(model["atoms"]) == disp["n_atoms"]  # serial space matches the flat array
    assert all(
        {"serial", "element", "strand_id", "residue"} <= set(a)
        for a in model["atoms"][:5]
    )
...
def test_audit_trajectory_frames_clean(design, tmp_path):
    ...
    assert (
        len(r["frames"]) == r["summary"]["n_audited"] == 4
    )  # all audited (≤ max_audit)
    s = r["summary"]
...
def test_audit_trajectory_frames_catches_bad_frame(design, tmp_path):
    ...
    fwd = {
        (n["helix_id"], n["bp_index"]): n["backbone_position"]
        for n in bad
...
def test_audit_trajectory_frames_explicit_indices(design, tmp_path):
    ...
    r = audit_trajectory_frames(
        design, [("prod", "production", str(traj))], str(ref), frame_indices=[0, 2, 99]
...
def test_trajectory_audit_route(design, tmp_path, monkeypatch):
    ...
    stage = OxdnaStageStatus(
        name="4_production", kind="production", steps=10, status="done"
    )
    job = new_oxdna_job("sim", [stage], design_source_path="sim.nadoc")
    job.save(ws)
    sd = job.stage_dir(ws, "4_production")
    sd.mkdir(parents=True, exist_ok=True)
    geo = _geometry_for_design(design)
    ...
    resp2 = TestClient(app).post(
        f"/api/oxdna/jobs/{job.job_id}/trajectory-audit", json={"frame_indices": [0]}
    )
...
def test_display_atomistic_audit_route(design, tmp_path, monkeypatch):
    ...
    assert (
        r["n_rigid_stamp_violations"] == 0
    )  # the placer is correct on the ideal frame
