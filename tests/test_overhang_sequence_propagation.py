...
def _seed(*, scaffold_sequenced: bool, seq_a="ACGTACGT", seq_b="TTTTGGGG"):
    ...
    def _oh(hid, sid, oid, x, seq):
        ...
        spec = OverhangSpec(
            id=oid, helix_id=hid, strand_id=sid, label=oid.upper(), sequence=seq
        )  # ctor-time seq → whole sub-domain sized to len
        return h, s, spec
    ...
    scaf = Strand(
        ...
        domains=[
            Domain(helix_id="scaf_h", start_bp=0, end_bp=7, direction=Direction.FORWARD)
        ],
        sequence=("AAAACCCC" if scaffold_sequenced else None),
    )
    return _demo_design().model_copy(
        update={
            "helices": [ha, hb, scaf_h],
            "strands": [sa, sb, scaf],
            "overhangs": [oa, ob],
        }
    )
...
def test_connect_linker_propagates_real_complement_when_scaffold_sequenced():
    ...
    design_state.set_design(_seed(scaffold_sequenced=True))
    r = client.post(
...
def test_connect_linker_noop_when_scaffold_unsequenced():
    ...
    design_state.close_session()
    design_state.set_design(_seed(scaffold_sequenced=False))
    r = client.post(
        "/api/design/overhang-connections",
        json={
            "overhang_a_id": "oh_a",
...
def test_patch_overhang_sequence_reassigns_complement():
    ...
    design_state.close_session()
    design_state.set_design(_seed(scaffold_sequenced=True, seq_a="AAAAAAAA"))
    client.post(
        "/api/design/overhang-connections",
        json={
            "overhang_a_id": "oh_a",
