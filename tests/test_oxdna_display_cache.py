...
def test_distinct_keys_do_not_collide():
    ...
    H._display_out_put(
        ("cts", "k", 0, ("strand", 0.28, 0.2, 1.3, 15)),
        {"vertices": [0.0], "faces": [0]},
    )
    assert H._display_out_get(("cta", "k", 0)) == [0.0]
    assert H._display_out_get(("cta", "k", 1)) == [1.0]
    assert H._display_out_get(("cts", "k", 0, ("strand", 0.28, 0.2, 1.3, 15)))[
        "faces"
    ] == [0]
...
def test_element_accounting_lists_and_surfaces():
    ...
    assert (
        H._out_payload_elems({"vertices": [0], "faces": [0], "vertex_rmsf": [0, 1]})
        == 4
    )
    assert (
        H._out_payload_elems({"vertices": [0], "faces": [], "vertex_colors": [1, 2, 3]})
        == 4
    )
