...
def _design_one_staple() -> Design:
    h = Helix(id="hA", axis_start=Vec3(x=0.0, y=0.0, z=0.0),
              ...
              phase_offset=0.0, length_bp=16, grid_pos=(0, 0))
    st = Strand(id="st", strand_type=StrandType.STAPLE, sequence="ACGTACGTACGTACGT",
                domains=[Domain(helix_id="hA", start_bp=0, end_bp=15,
...
def test_valid_extension_not_flagged():
    ...
    d = d.model_copy(update={"extensions": [
...
def test_validate_flags_dangling_strand_and_bad_sequence():
    ...
    d = d.model_copy(update={"extensions": [
        StrandExtension(strand_id="ghost", end="three_prime", modification="cy3"),
        StrandExtension(strand_id="st", end="five_prime", sequence="TTXZ"),
    ]})
    msgs = [r.message for r in validate_design(d).results
            if not r.ok and "Strand extension" in r.message]
...
def test_headless_add_extension_fluorophore_and_oracle():
    ...
    assert_extension_present(design_state.get_design(), ext.id,
                             strand_id="st", end="three_prime", modification="cy3")
...
def test_headless_add_extension_sequence():
    ...
    ext = design_state.get_design().extensions[0]
    assert_extension_present(design_state.get_design(), ext.id,
                             strand_id="st", end="five_prime", sequence="TTTT")
