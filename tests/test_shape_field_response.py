...
def test_deflection_is_monotone_in_field_magnitude():
    ...
    anchors = _anchor_low_z(ref)
    small = field_response_profile(
        _deflect(ref, field_dir, 0.8, anchors), ref, field_dir, anchors
    )
    large = field_response_profile(
        _deflect(ref, field_dir, 3.0, anchors), ref, field_dir, anchors
    )
...
def test_copy_aware_keys_kept_distinct():
    ...
    anchors = [(1, 5, "forward")]
    fld = [
        _pos(0, 5, "forward", (1.0, 0, 0), copy=0),
    ...
    keys = {
        (e["helix_id"], e["bp_index"], e["direction"], e["copy"]) for e in r["per_nt"]
    }
...
def test_identical_engines_perfect_cosine_unit_ratio():
    ...
    field_dir = (1.0, 0.0, 0.0)
    anchors = _anchor_low_z(ref)
    a = field_response_profile(
        _deflect(ref, field_dir, 2.0, anchors), ref, field_dir, anchors
    )
    b = field_response_profile(
        _deflect(ref, field_dir, 2.0, anchors), ref, field_dir, anchors
    )
...
def test_opposite_deflection_anti_correlated():
    ...
    anchors = _anchor_low_z(ref)
    a = field_response_profile(
        _deflect(ref, (1, 0, 0), 2.0, anchors), ref, (1, 0, 0), anchors
    )
    b = field_response_profile(
        _deflect(ref, (-1, 0, 0), 2.0, anchors), ref, (-1, 0, 0), anchors
    )
...
def test_orthogonal_deflection_zero_cosine():
    ...
    b = field_response_profile(
        _deflect(ref, (0, 1, 0), 2.0, anchors), ref, (0, 1, 0), anchors
    )
    c = compare_field_response(a, b)
...
def test_magnitude_ratio_scales_with_compliance():
    ...
    field_dir = (1.0, 0.0, 0.0)
    anchors = _anchor_low_z(ref)
    stiff = field_response_profile(
        _deflect(ref, field_dir, 1.0, anchors), ref, field_dir, anchors
    )
    soft = field_response_profile(
        _deflect(ref, field_dir, 3.0, anchors), ref, field_dir, anchors
    )
...
def test_compare_no_shared_free_is_none():
    ...
    b = field_response_profile(
        _deflect(ref2, (1, 0, 0), 2.0, anch2), ref2, (1, 0, 0), anch2
    )
    c = compare_field_response(a, b)
