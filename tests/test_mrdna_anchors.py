...
def _base_anchor(design):
    ...
    prov = [p for p in _strand_nucleotide_provenance(design) if p["helix_id"] is not None]
    p = prov[len(prov) // 2]
    return [{"kind": "base", "helix_id": p["helix_id"], "bp": p["bp"],
...
def test_dry_run_writes_restraint_file_for_scope(tmp_path):
    ...
    m.simulate(output_name="anchortest", directory=str(tmp_path),
...
def test_real_arbd_anchored_beads_hold(tmp_path):
    ...
    m.simulate(output_name="anchorhold", directory=str(tmp_path),
               num_steps=2000.0, timestep=200e-6, output_period=1000.0, gpu=0)
    ...
    u = mda.Universe(str(tmp_path / "anchorhold.psf"),
                     str(tmp_path / "output" / "anchorhold.dcd"))
    u.trajectory[-1]
