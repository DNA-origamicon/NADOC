...
def _straight_core(n_helix: int = 2, n_bp: int = 12, spacing: float = 2.5,
                   rise: float = 0.34):
    ...
    for h in range(n_helix):
        for bp in range(n_bp):
            for d in ("forward", "reverse"):
                rec = {"helix_id": f"h{h}", "bp_index": bp, "direction": d,
...
def _nma_rmsf(n_helix: int = 2, n_bp: int = 12, base: float = 0.9):
    ...
    return [{"helix_id": f"h{h}", "bp_index": bp, "rmsf_nm": base + 0.01 * bp}
            for h in range(n_helix) for bp in range(n_bp)]
...
def test_core_mask_drops_ssdna_ends():
    ...
    frame += [{"helix_id": "h0", "bp_index": 99, "direction": "forward",
               "backbone_position": [0.0, 0.0, 40.0]},
              {"helix_id": "h1", "bp_index": 99, "direction": "reverse",
...
def test_rmsf_remap_is_directionless_and_drops_none():
    rmsf = [{"helix_id": "h0", "bp_index": 0, "rmsf_nm": 0.9},
...
def test_field_passthrough_and_none_default():
    frame, reference = _straight_core()
    sentinel = {"anchored_max_drift_nm": 0.0, "free_proj_along_field_nm": 3.0,
                "passed": True, "per_nt": []}
    assert build_cando_shape_source(frame, reference)["field"] is None
    assert build_cando_shape_source(frame, reference, field=sentinel)["field"] is sentinel
...
def test_cando_bundle_yields_cross_engine_agreement():
    ...
    ox_frame = [{**p, "backbone_position": [p["backbone_position"][0] + 0.2,
                                            *p["backbone_position"][1:]]} for p in frame]
    ox_rmsf = [{"helix_id": f"h{h}", "bp_index": bp, "direction": d, "rmsf_nm": 0.9 + 0.01 * bp}
               for h in range(2) for bp in range(12) for d in ("forward", "reverse")]
    oxdna = {"engine": "oxdna",
             "descriptors": compute_shape_descriptors(ox_frame),
             "rmsf": ox_rmsf, "shape_frame": ox_frame, "field": None}
    ...
    assert by_eng["cando"]["shape_rmsd_nm"] is not None       # rigid 0.2nm shift → finite RMSD
    assert by_eng["cando"]["shape_rmsd_nm"] == pytest.approx(0.0, abs=1e-6)  # rigid → Kabsch ~0
    ox_rmsf_ag = by_eng["oxdna"]["rmsf"]
    assert ox_rmsf_ag is not None
    assert ox_rmsf_ag["pearson"] == pytest.approx(1.0, abs=1e-9)  # identical rmsf ramp → r=1
