...
def test_force_equals_negative_energy_gradient_finite_difference():
    ...
    for d in range(3):
        ...
        m[d] -= h
        grad[d] = (
            es.pair_energy(np.linalg.norm(p - xi), prm)
            - es.pair_energy(np.linalg.norm(m - xi), prm)
        ) / (2 * h)
...
def test_inter_helix_pairs_exclude_same_helix_and_respect_cutoff():
    ...
    hid = []
    for k in range(3):
        pos.append([0.34 * k, 0.0, 0.0])
        hid.append("h0")
        pos.append([0.34 * k, 2.0, 0.0])
        hid.append("h1")
    pos = np.array(pos)
    ...
    assert len(pairs) >= 3
    pos_far = pos.copy()
    pos_far[1::2, 1] = 3.0  # push h1 to 3.0 nm
