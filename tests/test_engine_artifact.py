...
def _gpu(present, names=None):
    return {
        "present": present,
        "names": names or (["RTX 2080"] if present else []),
        "arch": "75",
    }
...
def test_validate_accepts_archive_with_namd3(tmp_path):
    p = _make_tar(
        ...
        inner_names=[
            "NAMD_3.0.2_Linux-x86_64-multicore-CUDA/namd3",
...
def test_validate_missing_file():
    v = art.validate_namd_archive(
        "/no/such/NAMD_3.0.2_Linux-x86_64-multicore.tar.gz", _gpu(False)
    )
    assert v["valid"] is False
...
def test_parse_arbd_filename_accepts_and_rejects():
    assert (
        art.parse_arbd_filename("/d/arbd-may24-beta.tar.gz")["filename"]
        == "arbd-may24-beta.tar.gz"
    )
...
def test_validate_arbd_warns_without_gpu(tmp_path):
    p = _make_tar(
        str(tmp_path), "arbd-may24-beta.tar.gz", inner_names=["arbd/CMakeLists.txt"]
    )
...
def test_validate_arbd_rejects_non_source(tmp_path):
    p = _make_tar(
        str(tmp_path), "arbd-blob.tar.gz", inner_names=["arbd-blob/README.txt"]
    )
    v = art.validate_arbd_archive(p, _gpu(True))
    assert v["valid"] is False
...
def test_install_arbd_raises_when_cmake_fails(tmp_path, monkeypatch):
    ...
    p = _make_tar(
        str(tmp_path),
        "arbd-may24-beta.tar.gz",
        inner_names=["arbd-may24-beta/CMakeLists.txt"],
    )
    monkeypatch.setenv("HOME", str(home))
    monkeypatch.setattr(art, "gpu_info", lambda: _gpu(True))
...
def test_install_arbd_binary_copies_built_onto_path(tmp_path, monkeypatch):
    ...
    assert out == dest
    assert os.path.isfile(dest) and os.access(
        dest, os.X_OK
    )  # really copied + executable
    assert rec.msgs[-1]["type"] == "complete"
