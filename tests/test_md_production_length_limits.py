...
class TestProductionCaps:
    ...
    def test_caps_are_shared_not_per_model(self, model) -> None:
        ...
        assert _build(model, length_ns=MAX_PRODUCTION_NS).length_ns == pytest.approx(
            MAX_PRODUCTION_NS
        )
...
class TestForecastMeasuresTheRealVolume:
    ...
    def test_forecast_of_an_absent_target_still_measures_a_real_volume(
        self, tmp_path
    ) -> None:
...
class TestThroughputEstimate:
    ...
    @pytest.mark.parametrize(
        "n_atoms,length_ns", [(0, 100.0), (62_677, 0.0), (-1, 10.0)]
    )
...
class TestMeasuredThroughputBeatsTheModel:
    ...
    @pytest.mark.parametrize(
        "bad", [(0.0, 2.0, "s"), (220.0, 0.0, "s"), (-5.0, 2.0, "s")]
    )
...
class TestBenchmarkLineParsing:
    ...
    def test_reads_the_older_days_per_ns_format(self, tmp_path) -> None:
        ...
        log = self._log(
            tmp_path, "Benchmark time: 0.002345 s/step 0.027123 days/ns 16.00 MB\n"
        )
    ...
    def test_takes_the_last_benchmark_line(self, tmp_path) -> None:
        ...
        log = self._log(
            tmp_path,
            "Benchmark time: 1 s/step 100 ns/day 0 MB\n"
