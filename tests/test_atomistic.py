...
def test_p_atom_at_backbone_position():
    ...
    for atom in p_atoms:
        ...
        assert dist < 1.5, (
            f"P atom at ({atom.x:.3f},{atom.y:.3f},{atom.z:.3f}) is {dist:.3f} nm from backbone bead — seems wrong"
        )
...
def test_psf_atom_line_has_charmm_fields():
    ...
    for line in lines:
        ...
        if in_natom:
            ...
            parts = line.split()
            assert len(parts) >= 8, (
                f"PSF NATOM line has only {len(parts)} fields: {line!r}"
            )
            count += 1
...
def test_pdb_atom_records_fixed_width_large_design():
    ...
    for line in atom_lines:
        assert len(line) == 80, f"ATOM line wrong length ({len(line)}): {line!r}"
