...
def test_json_shim_adds_loops_drops_scaf_colors():
    cad = {"name": "x", "vstrands": [
        {"num": 0, "row": 1, "col": 2, "scaf": [], "stap": [], "loop": [], "skip": [],
         "stap_colors": [[0, 123]], "scaf_colors": [[0, 9]]},
    ]}
...
def _synthetic_design(nA=5, nB=4, x_sep=3.0, rise=0.34):
    ...
    keys, pos = [], []
    for bp in range(nA):
        keys.append(("A", bp)); pos.append([0.0, 0.0, bp * rise])
    for bp in range(nB):
        keys.append(("B", bp)); pos.append([x_sep, 0.0, bp * rise])
    labels = [
        {"snupi_chain_index": 0, "num": 0, "helix_id": "A", "direction": "FORWARD",
         "bases": [{"cadnano_base": bp, "global_bp": bp, "duplex": True} for bp in range(nA)]},
        {"snupi_chain_index": 1, "num": 1, "helix_id": "B", "direction": "REVERSE",
         "bases": [{"cadnano_base": bp, "global_bp": bp, "duplex": True} for bp in range(nB)]},
    ]
...
def test_topological_match_salvages_boundary_swap():
    ...
    for h in ("A", "D", "B", "C"):
        x, y = xy[h]
        for bp in range(5):
            keys.append((h, bp)); pos.append([x, y, bp * rise])
    pos = np.array(pos)
    labels = [
        {"snupi_chain_index": i, "num": i, "helix_id": h, "direction": "FORWARD",
         "bases": [{"cadnano_base": bp, "global_bp": bp, "duplex": True} for bp in range(5)]}
        for i, h in enumerate(("A", "D", "B", "C"))
    ]
    snupi_all = _rigid(pos)
    A_pos, D_pos, B_pos, C_pos = (snupi_all[:5], snupi_all[5:10],
                                  snupi_all[10:15], snupi_all[15:20])
    nodes = _make_snupi_nodes([
        ("H1", A_pos, [1, 2, 3, 4, 5]),                        # clean
        ("H2", D_pos, [1, 2, 3, 4, 5]),                        # clean
        ("H3", B_pos[:4], [1, 2, 3, 4]),                       # B missing its last bp
        ("H4", np.vstack([C_pos, B_pos[4]]), [1, 2, 3, 4, 5, 6]),  # C + B's stray bp
    ])
    m = match_nodes(nodes, keys, pos, labels=labels)
    assert m.ok, m.reason
...
def test_topological_match_rejects_uncoverable_mismatch():
    ...
    labels = [{"snupi_chain_index": 0, "num": 0, "helix_id": "A", "direction": "FORWARD",
               "bases": [{"cadnano_base": bp, "global_bp": bp, "duplex": True} for bp in range(5)]}]
    snupi_A = _rigid(pos)
...
def test_rigid_body_fraction_flags_rigid_vs_elastic():
    ...
    axial = (P - P.mean(0))[:, 2]
    bend = np.zeros((30, 3)); bend[:, 0] = axial ** 2
...
def test_reconstruct_pearson_valid_matrix():
    ...
    N, M, nr = 6, 12, 6
    ev = np.sort(rng.uniform(1, 50, M + nr)); vec = rng.standard_normal((M + nr, 6 * N))
...
def test_self_consistency_roundtrips_to_machine_precision():
    ...
    N, M, nr = 5, 10, 6
    ev = np.sort(rng.uniform(1, 100, M + nr)); vec = rng.standard_normal((M + nr, 6 * N))
    rmsf = reconstruct_rmsf(ev, vec, N, n_rigid=nr, kbt=KBT_300K)
    C = reconstruct_pearson_correlation(ev, vec, N, n_rigid=nr)
    mat = {"eigenvalues": ev, "eigenvectors": vec,
           "rmsf": rmsf, "pearson_correlation": np.tril(C)}   # SNUPI stores lower-tri
...
def test_correlation_agreement_handles_lower_triangular_snupi():
    ...
    full = rng.standard_normal((n, n)); full = (full + full.T) / 2
