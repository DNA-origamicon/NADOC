...
from backend.core.surface import (
    compute_surface, smooth_mesh, adaptive_grid_spacing, cg_surface_mesh, make_cg_bead,
    CG_BEAD_RADIUS_NM,
)
...
def fine_surface(design, grid_spacing=0.20, probe_radius=0.28, radius_inflate=1.30, smooth=15):
    ...
    mesh = compute_surface(model.atoms, grid_spacing=gs, probe_radius=probe_radius,
                           radius_scale=1.2 * radius_inflate)
    return smooth_mesh(mesh, iterations=smooth)


def vectorized_fine_surface(design, grid_spacing=0.20, probe_radius=0.28, radius_inflate=1.30, smooth=15):
...
def coarse_surface(design, grid_spacing=0.20, probe_radius=0.28, smooth=15,
    ...
    return cg_surface_mesh(beads, grid_spacing=grid_spacing, probe_radius=probe_radius,
                           smooth=smooth, bead_radius=bead_radius)


def _voltroncore() -> Design:
    if not _VOLTRONCORE_JSON.exists():
        pytest.skip("VoltronCore design.json not present (workspace/ is not synced across machines)")
...
def test_coarse_vs_fine_deviation(builder, base):
    ...
    assert 0.15 < h["mean"] < 0.45, f"coarse-vs-fine mean {h['mean']:.3f} nm off characterization"
...
def test_vectorized_fine_surface_matches_exact_on_deformed():
    ...
    design = design.model_copy(update={"deformations": [DeformationOp(
        type="bend", plane_a_bp=20, plane_b_bp=64,
...
def test_invariants_detect_envelope_perturbation():
    ...
    assert h["mean"] > 0.02, "grid perturbation did not move surface_hausdorff off identity"
