...
def test_write_hmr_psf_heavy_residues_scale_up_and_skip_hmr(tmp_path: Path):
    ...
    assert (
        n == 0
    )  # no H repartitioned — the only DNA residue is heavy, water is skipped

    m = _masses(dst.read_text())
    assert (
        abs(m[1] - 12.011 * 8.0) < 1e-3
    )  # carbon scaled x8 from physical (NOT lightened)
    ...
    assert [len(l) for l in _PSF.splitlines()] == [
        len(l) for l in dst.read_text().splitlines()
    ]
...
def test_segment_conf_soft_gets_resident_even_with_fast_off():
    ...
    soft = next(s for s in segs if s.soft)
    conf = M._segment_conf(
        soft, "S", (100.0, 90.0, 80.0), mgh_extrabonds=True, fast=False
    )
    assert "GPUresident        on" in conf
    assert "timestep           1" in conf
...
def test_segment_conf_small_system_stays_offload():
    ...
    soft = next(s for s in segs if s.soft)
    conf = M._segment_conf(
        soft, "S", (100.0, 90.0, 80.0), mgh_extrabonds=True, fast=False, n_atoms=32_566
    )
...
def test_unknown_atom_count_does_not_block_resident():
    ...
    soft = next(s for s in segs if s.soft)
    conf = M._segment_conf(
        soft, "S", (100.0, 90.0, 80.0), mgh_extrabonds=True, fast=False, n_atoms=None
    )
    assert "GPUresident        on" in conf
