...
from backend.core.md_protocols import (SegmentSpec, build_production_conf,
                                       build_reseed_conf, package_npt_allowed)
...
def _prod_spec(steps: int = 500_000) -> SegmentSpec:
    return SegmentSpec(
        name="S_01_production_2ns_k0", stage="2 ns production", percent=100.0,
        steps=steps, temp=300.0, damping=5.0, scale=None, npt=True,
        previous="S_00_reseed", reinit=False,
    )


def _conf(npt: bool) -> str:
    return build_production_conf(_prod_spec(), "S", BOX, True, fast=True,
                                 timestep_fs=4.0, structure_psf="S_hmr.psf", npt=npt)
...
def test_default_is_npt_so_existing_callers_are_unchanged():
    assert "langevinPiston     on" in build_production_conf(
        _prod_spec(), "S", BOX, True, fast=True, timestep_fs=4.0,
...
def test_package_npt_allowed_reads_the_solvation_record(tmp_path):
    carved = _pkg(tmp_path / "a", {"carved": True, "npt_allowed": False,
                                   "water_shell_nm": 1.2, "padding_nm": 1.2})
    full = _pkg(tmp_path / "b", {"carved": False, "npt_allowed": True,
                                 "water_shell_nm": 0.0, "padding_nm": 1.2})
...
def test_xst_sampling_is_capped_so_a_long_run_still_resolves_300_ps():
    ...
    long_run = build_production_conf(
        _prod_spec(steps=50_000_000), "S", BOX, True, fast=True, timestep_fs=4.0,
        structure_psf="S_hmr.psf")
    xst = int(next(l.split()[1] for l in long_run.splitlines()
                   if l.startswith("xstFreq")))
