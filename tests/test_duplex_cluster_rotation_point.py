...
from backend.core.duplex_cluster import (
    materialize_duplex_cluster, duplex_cluster_for, duplex_cluster_rotation_points,
...
def _ovhg_beads(des, ids):
    return {(n.get("overhang_id"), n["bp_index"], n["direction"]):
            np.asarray(n.get("backbone_position") or n.get("base_position"), float)
            for n in _geometry_for_design(des) if n.get("overhang_id") in ids}
...
def test_moving_pivot_to_any_rotation_point_is_geometry_neutral():
    ...
    for p in duplex_cluster_rotation_points(d, cl):
        ...
        assert max(float(np.linalg.norm(before[k] - after[k])) for k in before) < 1e-6, p["kind"]
...
def test_rotate_about_centroid_vs_root_differ():
    ...
    def spun(pivot):
        ...
        m2 = m.model_copy(update={"cluster_transforms": [
            c2 if x.id == c.id else x for x in m.cluster_transforms]})
        return _ovhg_beads(m2, {drv})
    a = spun(pts["centroid"])
    b = spun(next(p["point"] for p in duplex_cluster_rotation_points(d, cl)
