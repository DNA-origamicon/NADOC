...
def _prepared_job(ws: Path, status="queued"):
    ...
    job.segments.append(
        MdSegmentStatus(
            name="D_01_stage", stage="s", percent=100.0, steps=10, status="pending"
        )
    )
    job.save(ws)
...
def test_forces_attach_to_a_prepared_job_without_re_prepping(tmp_path, monkeypatch):
    ...
    r = asyncio.run(
        routes_md.set_md_job_forces(
            job.job_id,
            routes_md.JobForcesRequest(
    ...
    with pytest.raises(HTTPException) as e:
        asyncio.run(
            routes_md.set_md_job_forces(
                ...
                routes_md.JobForcesRequest(
                    anchors=[
                        {"kind": "strand", "id": design.strands[0].id, "atoms": ["CA"]}
                    ]
                ),
            )
        )
