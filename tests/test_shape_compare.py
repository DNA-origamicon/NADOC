...
def test_rmsf_cando_directionless_vs_ensemble_per_strand_correlates():
    ...
    cando = [
        {"helix_id": 0, "bp_index": i, "rmsf_nm": v}
    ...
    for i, v in enumerate([1.1, 2.9, 2.2, 4.8, 4.1]):
        ensemble.append(
            {"helix_id": 0, "bp_index": i, "direction": "forward", "rmsf_nm": v + 0.05}
        )
        ensemble.append(
            {"helix_id": 0, "bp_index": i, "direction": "reverse", "rmsf_nm": v - 0.05}
        )
    out = compare_descriptors(
        _bundle("oxdna", rmsf=ensemble), _bundle("cando", rmsf=cando)
    )
...
def test_shape_rmsd_ignores_rigid_pose_but_catches_shape():
    ...
    rigid = compare_descriptors(
        _bundle("cando", shape_frame=moved), _bundle("oxdna", shape_frame=ref)
    )
    ...
    bent = compare_descriptors(
        _bundle("cando", shape_frame=sheared), _bundle("oxdna", shape_frame=ref)
    )
...
def test_shape_rmsd_absent_frame_is_none():
    out = compare_descriptors(
        _bundle("cando", shape_frame=None), _bundle("oxdna", shape_frame=_grid_frame())
    )
