...
class TestHelixAtCell:
    ...
    def test_hc_phase_offset_forward_cell(self, client):
        ...
        assert (
            abs(h["phase_offset"] - math.radians(90.0 + BDNA_TWIST_PER_BP_DEG / 2))
            < 1e-5
        )
    ...
    def test_hc_phase_offset_reverse_cell(self, client):
        ...
        h = r.json()["design"]["helices"][0]
        assert (
            abs(h["phase_offset"] - math.radians(60.0 + BDNA_TWIST_PER_BP_DEG / 2))
            < 1e-5
        )
    ...
    def test_hc_twist_is_commensurate_with_the_21bp_repeat(self, client):
        ...
        assert abs(21 * h["twist_per_bp_rad"] - 4 * math.pi) < 1e-12, (
            "21 bp must be 2 turns"
        )
    ...
    def test_hc_custom_length_bp(self, client):
        ...
        _make_hc_design(client)
        r = client.post(
            "/api/design/helix-at-cell", json={"row": 0, "col": 0, "length_bp": 84}
        )
        assert r.status_code == 201
        h = r.json()["design"]["helices"][0]
    ...
    def test_populate_strands_visible_in_geometry(self, client):
        ...
        design = r.json()["design"]
        scaf_id = next(
            s["id"] for s in design["strands"] if s["strand_type"] == "scaffold"
        )
        stpl_id = next(
            s["id"] for s in design["strands"] if s["strand_type"] == "staple"
        )
    ...
    def test_adjacent_to_offset_neighbor(self, client):
        ...
        ref.axis_end = Vec3(
            x=ref.axis_end.x + DX, y=ref.axis_end.y + DY, z=Z0 + 29 * RISE
        )
        ref.bp_start = 16
        ref.length_bp = 30
...
class TestScaffoldDomainPaint:
    def _add_cell(self, client, row, col, length_bp=42):
        r = client.post(
            "/api/design/helix-at-cell",
            json={"row": row, "col": col, "length_bp": length_bp},
        )
        assert r.status_code == 201
