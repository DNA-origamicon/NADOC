...
def test_reference_dependency_is_caught():
    ...
    infos = [
        ...
        _snap(
            modified={"hA"}, targets={"hA"}, reconstructable=True
        ),  # continuation onto hA
    ]
...
def test_transitive_closure():
    ...
    infos = [
        _snap(added={"hA"}, reconstructable=True),  # K
        _snap(
            added={"hB"}, targets={"hA"}, reconstructable=True
        ),  # L deps on K, makes hB
        _snap(
            modified={"hB"}, targets={"hB"}, reconstructable=True
        ),  # M deps on L's hB
    ]
...
def test_survivor_between_two_dependents():
    ...
    infos = [
        ...
        _snap(
            added={"hC"}, targets=set(), reconstructable=True
        ),  # 2 independent survivor
...
def test_structural_targets_include_added_crossover_endpoint_refs():
    ...
    h2 = Helix(
        id="h_survivor",
        axis_start=Vec3(x=1, y=0, z=0),
        axis_end=Vec3(x=1, y=0, z=10),
        length_bp=10,
    )
        ...
        crossovers=[
            Crossover(
                ...
                half_a=HalfCrossover(
                    helix_id="h_removed", index=5, strand=Direction.FORWARD
                ),
                half_b=HalfCrossover(
                    helix_id="h_survivor", index=5, strand=Direction.REVERSE
                ),
            )
        ]
    )
    ...
    assert structural_reference_targets(pre, post, added, modified) >= {
        "h_removed",
        "h_survivor",
    }
