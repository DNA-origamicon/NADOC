...
def _helix_ids_in_cols(design, col_lo: int, col_hi: int):
    return [
        h.id for h in design.helices if h.grid_pos and col_lo <= h.grid_pos[1] <= col_hi
    ]
...
def test_obb_contains_cluster_geometry_snugly():
    ...
    with hb.scratch_session(LatticeType.SQUARE):
        ...
        assert np.all(np.abs(coords) <= obb.half + 1e-6), (
            "a helix endpoint escapes the OBB"
        )
...
def test_obb_is_equivariant(rotvec, translation):
    ...
    with hb.scratch_session(LatticeType.SQUARE):
        ...
        assert np.allclose(obb1.half, obb0.half, atol=1e-3), (
            "OBB extents changed under a rigid pose"
        )
        # axes rotate with g
        for i in range(3):
...
def test_align_two_parallel_bars_snaps_edge_onto_edge():
    ...
    with hb.scratch_session(LatticeType.SQUARE):
        ...
        assert np.allclose((pa0 + pa1) / 2, (pb0 + pb1) / 2, atol=1e-3), (
            "midpoints not snapped"
        )
...
def test_align_edge_to_angled_world_line_rotates_and_snaps():
    ...
    with hb.scratch_session(LatticeType.SQUARE):
        ...
        quat, _, _ = align_edge_transform(
            design, cid, src, target_line=(point, direction)
        )
...
def test_align_to_rotated_bar_exercises_full_rotation():
    ...
    with hb.scratch_session(LatticeType.SQUARE):
        ...
        hb.transform_cluster(
            b, translation=[0.0, 0.0, 0.0], rotation=q, pivot=b_obb.center.tolist()
        )
        src = ("w", 1, 1)
        tgt = ("w", 1, 1)
        hb.align_cluster_edge(a, src, target_edge=(b, tgt))
...
def test_align_auto_flips_to_minimal_rotation():
    ...
    with hb.scratch_session(LatticeType.SQUARE):
        ...
        ang = Rotation.from_quat(quat).magnitude()
        assert ang < np.radians(1.0), (
            "auto-flip should make this a ~zero rotation, not 180°"
        )
        hb.align_cluster_edge(cid, src, target_line=(point, direction))
        d = design_state.get_or_404()
        assert_edges_collinear(d, cid, src, target_line=(point, direction))
...
def test_hull_prism_axis_corner_pivots_at_corner_along_face_normal():
    ...
    with hb.scratch_session(LatticeType.SQUARE):
        ...
        origin, direction = hull_prism_axis(
            design, cid, corner=(1, 1, 1), face=("w", 1)
        )
        assert np.allclose(origin, obb.corner(1, 1, 1), atol=1e-6)
        normal = obb.face_normal(("w", 1))
        assert (
            abs(abs(float(np.dot(direction, normal / np.linalg.norm(normal)))) - 1.0)
            < 1e-6
        )
...
def test_place_cluster_joint_on_posed_cluster_round_trips_local_frame():
    ...
    with hb.scratch_session(LatticeType.SQUARE):
        ...
        hb.transform_cluster(
            cid, translation=[5.0, -3.0, 2.0], rotation=q, pivot=obb.center.tolist()
        )
        edge = ("w", 1, 1)
...
def test_obb_sweep_rom_double_wall_matches_analytic():
    ...
    rom = obb_sweep_rom(rod, [top, bottom], [0, 0, 0], [0, 0, 1], pad=0.0, step_deg=0.5)
...
def test_obb_sweep_rom_no_obstacle_is_full_limit():
    ...
    assert (
        abs(
            obb_sweep_rom(rod, [], [0, 0, 0], [0, 0, 1], min_deg=-90, max_deg=90)
            - 180.0
        )
        < 1e-6
    )
...
def test_cluster_range_of_motion_no_obstacle_full_swing():
    ...
    with hb.scratch_session(LatticeType.SQUARE):
        ...
        assert (
            abs(
                cluster_range_of_motion(
                    design, cid, axis, min_angle_deg=-90, max_angle_deg=90
                )
                - 180.0
            )
            < 2.0
...
def test_cluster_range_of_motion_obstacle_reduces_and_is_monotonic():
    ...
    with hb.scratch_session(LatticeType.SQUARE):
        ...
        hb.transform_cluster(
            b, translation=(sep_u * 4).tolist(), rotation=[0, 0, 0, 1], pivot=[0, 0, 0]
        )
        ...
        hb.transform_cluster(
            b, translation=(sep_u * 1).tolist(), rotation=[0, 0, 0, 1], pivot=[0, 0, 0]
        )
...
def test_rank_joint_candidates_orders_by_rom_and_door_jamb():
    ...
    with hb.scratch_session(LatticeType.SQUARE):
        ...
        assert roms[0] - roms[-1] > 5.0, (
            "B should make some hinges much freer than others"
        )
...
def test_recommend_hinge_corner_vs_midpoint_anchor():
    ...
    with hb.scratch_session(LatticeType.SQUARE):
        ...
        assert (
            min(np.linalg.norm(o_corner - p_lo), np.linalg.norm(o_corner - p_hi)) < 1e-6
        )
...
def test_recommend_hinge_target_rom_filter():
    ...
    with hb.scratch_session(LatticeType.SQUARE):
        ...
        hb.transform_cluster(
            b,
            translation=(sep / np.linalg.norm(sep) * 2).tolist(),
            rotation=[0, 0, 0, 1],
            pivot=[0, 0, 0],
        )
        d2 = design_state.get_or_404()
