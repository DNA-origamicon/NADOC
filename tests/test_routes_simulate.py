...
import backend.api.routes_simulate as rs
...
def _patch(monkeypatch, *, activity, contention, active_jobs=(), free=16, ox_pids=None):
    ...
    monkeypatch.setattr(md_vram, "gpu_contention_summary", lambda act, own_pids=(): contention)
...
def test_recommendation_shape_and_free_gpu(monkeypatch):
    _patch(monkeypatch,
           ...
           contention={"available": True, "busy": False, "processes": [],
                       "free_mb": 12000, "total_mb": 12000, "util_pct": 3})
...
def test_recommendation_external_busy_gpu_picks_lammps(monkeypatch):
    _patch(monkeypatch,
           activity={"free_mb": 2000, "total_mb": 12000, "util_pct": 95,
                     "processes": [{"pid": 999, "name": "namd3", "mem_mb": 8000}]},
           contention={"available": True, "busy": True,
                       "processes": [{"pid": 999, "name": "namd3", "mem_mb": 8000}],
                       "free_mb": 2000, "total_mb": 12000, "util_pct": 95})
...
def test_recommendation_own_gpu_job_is_busy_with_eta(monkeypatch):
    ...
    _patch(monkeypatch,
           ...
           contention={"available": True, "busy": False, "processes": [],
                       "free_mb": 6000, "total_mb": 12000, "util_pct": 60},
           active_jobs=[{"engine": "md", "resource_class": "gpu", "status": "running",
                         "eta_seconds": 425.0}])
    b = client.get("/api/simulate/recommendation").json()
