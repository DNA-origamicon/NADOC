...
def test_tether_anchors_resolve_to_real_beads():
    ...
    for t in duplex_cluster_tethers(d, cl):
        m = _bead(
            nucs, t["moving"]["helix_id"], t["moving"]["bp"], t["moving"]["direction"]
        )
        f = _bead(
            nucs, t["fixed"]["helix_id"], t["fixed"]["bp"], t["fixed"]["direction"]
        )
