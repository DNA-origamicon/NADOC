"""Explicit-solvent cell geometry, ion census, and ensemble configuration."""
...
def _capture_solvate_cmd(monkeypatch, tmp_path):
    """Drive _gmx_solvate with GROMACS stubbed and return its solvate command."""
    calls: list[list[str]] = []
    ...
    ns._gmx_solvate(pdb, 1.2, tmp_path)
    solvate = next(c for c in calls if "solvate" in c)
    return solvate


def test_solvation_always_fills_box_without_shell_flag(monkeypatch, tmp_path):
    cmd = _capture_solvate_cmd(monkeypatch, tmp_path)
