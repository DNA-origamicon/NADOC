...
# ── Binding relax (unified direct relax: close the driven tip↔root chord) ──────
# The unified `relax_overhang_binding` now closes the DRIVEN overhang's stretched
# tip↔root chord by swinging the driver's overhang duplex (writing the driver's
# OverhangSpec.rotation) + cluster kinematics — it requires an APPLIED, relocated,
# multi-domain binding. That behavior is pinned end-to-end in
# tests/test_direct_connection_unified.py (joint-rotate / translate / swing-persists
# / same-body), tests/test_headless_build.py (closes-chord / pose-only / same-body),
